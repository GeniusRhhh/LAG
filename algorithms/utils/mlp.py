...
class MLPLayer(nn.Module):
    def __init__(self, input_dim, hidden_size, activation_id):
        ...
        fc_h = []
        for j in range(len(self._size) - 1):
            fc_h += [nn.Linear(self._size[j], self._size[j + 1]), active_func, nn.LayerNorm(self._size[j + 1])]
        self.fc = nn.Sequential(*fc_h)
...
class MLPBase(nn.Module):
    def __init__(self, obs_space, hidden_size, activation_id, use_feature_normalization):
        ...
        self.obs_flattener = build_flattener(obs_space)
        self.input_dim = self.obs_flattener.size
        if self._use_feature_normalization:
            self.feature_norm = nn.LayerNorm(self.input_dim)
        self.mlp = MLPLayer(self.input_dim, self._hidden_size, self._activation_id)
