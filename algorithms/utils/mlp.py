...
class MLPLayer(nn.Module):
    def __init__(self, input_dim, hidden_size, activation_id):
        ...
        fc_h = []
        for j in range(len(self._size) - 1):
            layer = nn.Linear(self._size[j], self._size[j + 1])
            nn.init.xavier_uniform_(layer.weight)  # 修改为Xavier初始化
            nn.init.zeros_(layer.bias)  # 偏置初始化为0
            fc_h += [layer, active_func, nn.LayerNorm(self._size[j + 1])]
        self.fc = nn.Sequential(*fc_h)
...
class MLPBase(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self._use_feature_normalization:
            x = self.feature_norm(x)
        x = self.mlp(x)
        logging.debug(f"MLP output: min={x.min().item():.4f}, max={x.max().item():.4f}")
        return x
