...
def _get_enemy_action_simple(self, env, agent_id: str) -> Tuple[int, int, int]:
    ...
    if is_verification_scenario and hasattr(self, 'enemy_target_headings'):
        speed_map = self.enemy_target_headings.get('_target_speeds_mps')
        if isinstance(speed_map, dict):
            value = speed_map.get(agent_id)
            if value is not None:
                try:
                    target_speed_mps = float(value)
                except Exception:
                    target_speed_mps = None

    if target_speed_mps is not None and tas_mps is not None:
        spd_cmd = self._speed_error_to_spd_cmd(target_speed_mps - tas_mps)
...
def _get_intercept_action(self, env, agent_id: str) -> Tuple[int, int, int]:
    ...
    effective_role_map = responsibility_role_map if effective_responsibility_map else {}
