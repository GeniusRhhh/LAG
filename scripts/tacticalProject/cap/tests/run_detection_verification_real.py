...
# 速度常量 (fps)
FPS_NORMAL = 984.252  # 普通速度 300 m/s
FPS_LOW = 900.0       # 低速
FPS_HIGH = 1500.0     # 高速
...
def _front_reference_distance_for_scenario(scenario: EnemyScenario) -> float:
    """Front reference distance used to freeze scripted enemy maneuvers across compare runs."""
    try:
        own_center_y = 50.0
        return max(0.0, float(scenario.initial_distance) - own_center_y)
    except Exception:
        return 250.0


def _script_reference_distance_for_scenario(scenario: EnemyScenario, time_s: float) -> float:
    """Reference distance driven only by the scenario script, not by friendly behavior."""
    try:
        ...
        closing_speed = 0.381
    return max(0.0, _front_reference_distance_for_scenario(scenario) - closing_speed * float(time_s))
...
def run_scenario(scenario: EnemyScenario, max_steps: int = 3000, mode: str = 'proposed', seed: int = 1) -> dict:
    ...
    env = None
    prev_enemy_simple = os.environ.get('CAP_ENEMY_SIMPLE')
    ...
    try:
        ...
        patrol_task._awacs_control_time_s = 0.0
        patrol_task._awacs_control_reference_distance_km = _front_reference_distance_for_scenario(scenario)
        patrol_task._focus_awacs_loss_windows_s = tuple(getattr(scenario, 'awacs_loss_windows', ()) or ())
        patrol_task._focus_awacs_force_control = bool(getattr(scenario, 'awacs_loss_windows', None))
        ...
        while step < max_steps:
            ...
            scripted_distance = _script_reference_distance_for_scenario(scenario, time_s)
            patrol_task._awacs_control_time_s = float(time_s)
            patrol_task._awacs_control_reference_distance_km = float(scripted_distance)

            # 应用敌机机动
            enemy_target_headings = apply_enemy_maneuver(
                env, scenario, scripted_distance, time_s, enemy_target_headings)
            patrol_task.enemy_target_headings = enemy_target_headings
            ...
            if isinstance(enemy_target_headings.get('_target_altitudes_ft'), dict):
                ...
                if altitude_debug and step % 50 == 0 and scenario.scenario_id == 'V03':
                    logging.info(
                        f"[V6诊断 主循环] step={step} real_distance={current_distance:.1f}km "
                        f"script_distance={scripted_distance:.1f}km"
                    )
                    logging.info(
        ...
        try:
            ...
            pass
        if prev_enemy_simple is None:
            os.environ.pop('CAP_ENEMY_SIMPLE', None)
        else:
            os.environ['CAP_ENEMY_SIMPLE'] = prev_enemy_simple
    return results
