...
def maybe_log_guidance_snapshot(patrol_task, step: int, dt: float) -> None:
    ...
    try:
        ...
        logging.info(
            "[协同验收快照] "
            f"gate_pass={int(snapshot.get('prelaunch_gate_pass_count', 0))} "
            f"gate_block={int(snapshot.get('prelaunch_gate_block_count', 0))} "
        ...
        logging.info(
            ...
            f"pass={int(snapshot.get('prelaunch_gate_pass_count', 0))} "
            f"block={int(snapshot.get('prelaunch_gate_block_count', 0))} "
            f"relay_attempt={int(snapshot.get('relay_attempt_count', 0))} "
            f"relay_success={int(snapshot.get('relay_success_count', 0))} "
...
def log_final_guidance_summary(patrol_task) -> None:
    ...
    try:
        ...
        logging.info(
            f"  发射前门禁: pass={int(snapshot.get('prelaunch_gate_pass_count', 0))} "
            f"block={int(snapshot.get('prelaunch_gate_block_count', 0))} "
            f"pass_rate={float(snapshot.get('prelaunch_gate_pass_rate', 0.0)):.2%}"
        )
        logging.info(
            f"  接力制导: attempts={int(snapshot.get('relay_attempt_count', 0))} "
            f"success={int(snapshot.get('relay_success_count', 0))} "
            f"success_rate={float(snapshot.get('relay_success_rate', 0.0)):.2%} "
        ...
        logging.info(
            ...
            f"relay_attempt={int(snapshot.get('relay_attempt_count', 0))} "
            f"relay_success={int(snapshot.get('relay_success_count', 0))} "
            f"relay_rate={float(snapshot.get('relay_success_rate', 0.0)):.2%} "
            f"midcourse_peak={int(snapshot.get('active_guided_missile_peak', 0))}"
        )
    except Exception as exc:
