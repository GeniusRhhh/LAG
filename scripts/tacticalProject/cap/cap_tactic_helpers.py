...
def _build_dynamic_control_context(self, ctx: StateContext, current_time: float) -> Dict[str, object]:
    guidance = {}
    if hasattr(self, "get_guidance_verification_snapshot"):
        try:
            guidance = self.get_guidance_verification_snapshot() or {}
        except Exception:
            guidance = {}
    nearest_zone = str(getattr(ctx, "nearest_target_zone", "") or "").strip().upper()
    if not nearest_zone:
        if bool(ctx.has_hostile_in_high_zone):
            nearest_zone = "HIGH"
        elif bool(ctx.has_hostile_in_medium_zone):
            nearest_zone = "MEDIUM"
        else:
            nearest_zone = "LOW"
    return {
        "has_awacs_info": bool(ctx.has_awacs_info),
        "has_hostile_in_high_zone": bool(ctx.has_hostile_in_high_zone),
        "has_hostile_in_medium_zone": bool(ctx.has_hostile_in_medium_zone),
        "nearest_target_zone": nearest_zone,
        "is_missile_incoming": bool(ctx.is_missile_incoming),
        "rwr_level": int(getattr(ctx, "rwr_level", 0) or 0),
        "ecm_active": bool(getattr(ctx, "ecm_active", False)),
        "radar_threat_level": float(getattr(ctx, "radar_threat_level", 0.0) or 0.0),
        "closure_rate_mps": float(getattr(ctx, "closure_rate_mps", 0.0) or 0.0),
        "time_to_merge_s": float(getattr(ctx, "time_to_merge_s", float("inf")) or float("inf")),
        "nearest_target_speed_mps": float(getattr(ctx, "nearest_target_speed_mps", 0.0) or 0.0),
        "nearest_altitude_delta_km": float(getattr(ctx, "nearest_altitude_delta_km", 0.0) or 0.0),
        "mission_time": float(getattr(ctx, "mission_time", current_time) or current_time),
        "hostile_alive_count": int(getattr(ctx, "hostile_alive_count", 0) or 0),
        "stable_ready_count": int(len(getattr(self, "_stable_tracking_ready_announced", set()) or set())),
        "radar_track_count": int(sum(len(self.cap_radar.get_tracks(aid) or {}) for aid in getattr(self.cap_radar, "_radar_tracks", {}))),
        "threat_count": int(len(self._get_picture_threats(current_time=current_time, purpose="decision"))),
        "active_relay": bool(guidance.get("active_relay", False)),
        "active_guided_missile_peak": int(guidance.get("active_guided_missile_peak", 0) or 0),
        "gate_pass_count": int(guidance.get("prelaunch_gate_unique_pass_count", guidance.get("prelaunch_gate_pass_count", 0)) or 0),
        "gate_block_count": int(guidance.get("prelaunch_gate_unique_block_count", guidance.get("prelaunch_gate_block_count", 0)) or 0),
        "stable_tracking_required_s": float(getattr(self, "_stable_tracking_window_s", 5.0) or 5.0),
        "relay_entry_km": max(float(getattr(getattr(self, "ranges", DEFAULT_RANGES), "MAR", DEFAULT_RANGES.MAR)) + 10.0, 45.0),
    }


def _refresh_dynamic_control_ranges(self, env, ctx: StateContext, current_time: float) -> Dict[str, float]:
    if not hasattr(self, "_dynamic_control_ranges"):
        self._dynamic_control_ranges = None
    if not hasattr(self, "_dynamic_control_context"):
        self._dynamic_control_context = {}
    if not hasattr(self, "_dynamic_control_update_time"):
        self._dynamic_control_update_time = -9999.0
    ...
    total_radar_tracks = sum(len(self.cap_radar.get_tracks(aid)) for aid in getattr(self.cap_radar, "_radar_tracks", {}))
    if total_radar_tracks <= 0 or not np.isfinite(float(ctx.min_threat_distance)):
        return dict(self._locked_compression or self._dynamic_control_ranges or {})

    dynamic_ctx = _build_dynamic_control_context(self, ctx, current_time)
    previous_ranges = self._dynamic_control_ranges or self._locked_compression
    dynamic_ranges = DEFAULT_RANGES.get_dynamic_ranges(
        float(ctx.min_threat_distance),
        context=dynamic_ctx,
    ...
    self._dynamic_control_ranges = dict(dynamic_ranges)
    self._dynamic_control_context = dict(dynamic_ctx)
    self._dynamic_control_update_time = float(current_time)
    self._dynamic_control_update_distance = float(ctx.min_threat_distance)
    self._compression_locked_at_distance = float(ctx.min_threat_distance)
    self._locked_compression = dict(dynamic_ranges)
    self.ranges = ControlRanges(**dynamic_ranges)
    if hasattr(self, "missile_adapter") and self.missile_adapter is not None:
        self.missile_adapter.ranges = self.ranges
    if hasattr(self, "tactic_executor") and self.tactic_executor is not None:
        self.tactic_executor.ranges = self.ranges
    if hasattr(self, "cap_state_machine") and self.cap_state_machine is not None:
        self.cap_state_machine.ranges = self.ranges
    return dict(dynamic_ranges)


def _get_active_range_dict(self) -> Dict[str, float]:
    active = getattr(self, "_dynamic_control_ranges", None) or getattr(self, "_locked_compression", None)
    if active:
        return dict(active)
    ranges = getattr(self, "ranges", DEFAULT_RANGES)
    if hasattr(ranges, "get_all"):
        return dict(ranges.get_all())
    return dict(DEFAULT_RANGES.get_all())
...
def _log_dynamic_control_ranges_once(self, initial_distance: float):
    ...
    dynamic_context = dict(getattr(self, "_dynamic_control_context", {}) or {})
    dynamic_ranges = _get_active_range_dict(self) or DEFAULT_RANGES.get_dynamic_ranges(initial_distance, context=dynamic_context)
    log.info(f"[节点参数] 理论节点(km): {base_text} | 二次进攻: {second_text}")
    if not dynamic_ranges:
        log.info(f"[节点参数] 初始距离={initial_distance:.1f}km < MAR，直接进入紧急规避区")
        return
    dyn_text = (
        f"NLT={dynamic_ranges['NLT']:.1f} MELD={dynamic_ranges['MELD']:.1f} MTR={dynamic_ranges['MTR']:.1f} "
        f"LR={dynamic_ranges['LR']:.1f} TR={dynamic_ranges['TR']:.1f} DOR={dynamic_ranges['DOR']:.1f} "
        f"DR={dynamic_ranges['DR']:.1f} MAR={dynamic_ranges['MAR']:.1f} "
        f"MTR'={dynamic_ranges['MTR_PRIME']:.1f} LR'={dynamic_ranges['LR_PRIME']:.1f} TR'={dynamic_ranges['TR_PRIME']:.1f}"
    )
    log.info(
        "[节点参数] 初始距离=%.1fkm | dynamic_context=%s | 动态节点(km): %s",
        initial_distance,
...
def _get_second_attack_subnode(self, distance: float, ranges_dict: Dict[str, float] = None) -> Optional[str]:
    ...
    active_ranges = ranges_dict or _get_active_range_dict(self)
    if not current_ranges.is_second_attack_zone(distance):
        return None

    def get_val(key: str) -> float:
        if active_ranges and key in active_ranges:
            return float(active_ranges[key])
        return float(getattr(current_ranges, key, getattr(DEFAULT_RANGES, key)))
...
def _build_pair_node_status(self, env) -> str:
    ...
    for tag, pair in pair_defs.items():
        ...
        current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
        node = current_ranges.get_current_node(dist, _get_active_range_dict(self))
...
def _log_situation(self, env, ctx: StateContext, cap_state: CAPState, current_time: float):
    ...
    if not self._initial_nodes_logged:
        ...
        self._log_dynamic_control_ranges_once(ctx.min_threat_distance)
    ...
    # 获取当前节点 (传入动态压缩后的范围)
    active_ranges = _get_active_range_dict(self)
    ...
    second_node = self._get_second_attack_subnode(ctx.min_threat_distance, active_ranges)
    if second_node:
...
def _update_mission_evaluation(self, env, current_time: float):
    ...
    enemy_total = sum(1 for aid in env.agents if str(aid).startswith('B'))
    enemy_alive = sum(1 for aid in env.agents if str(aid).startswith('B') and env.agents[aid].is_alive)
    kills = max(0, enemy_total - enemy_alive)
    missiles_fired = 0
    ...
    if missile_adapter is not None and hasattr(missile_adapter, 'get_summary_snapshot'):
        try:
            ...
            missiles_fired = int(missile_summary.get('launch_count', 0) or 0)
        except Exception:
            missiles_fired = 0
...
def _update_tactic_selection(self, env, current_time: float):
    ...
    if hasattr(self, "_refresh_dynamic_control_ranges"):
        try:
            ...
            pass
    current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
    dynamic_ranges = _get_active_range_dict(self)
    ...
    for pair_name, pair_info in pair_defs.items():
        ...
        shooter_distance = float(np.linalg.norm(friendly_positions[shooter][:2] - np.array([threat.x, threat.y], dtype=float)))
        pair_node = current_ranges.get_current_node(shooter_distance, dynamic_ranges)
        pair_node = self._lock_template_node(shooter, threat.track_id, pair_node)
        target_status = self._get_picture_target_actionability(
            threat.track_id,
            current_time=current_time,
            purpose="decision",
        ) if hasattr(self, "_get_picture_target_actionability") else {}
        target_zone = str(target_status.get("target_zone", "") or "").upper()
        if not target_zone:
            target_zone = "HIGH" if 0.0 <= float(getattr(threat, "y", 300.0)) < 100.0 else (
                "MEDIUM" if 100.0 <= float(getattr(threat, "y", 300.0)) < 200.0 else "LOW"
            )
        is_second_attack = current_ranges.is_second_attack_zone(shooter_distance)
        gate_profile = self._get_prelaunch_gate_profile(
            threat.track_id,
            current_time,
            shooter_id=shooter,
            distance_km=shooter_distance,
            is_second_attack=is_second_attack,
            target_zone=target_zone,
        ) if hasattr(self, "_get_prelaunch_gate_profile") else {}
        stable_required_s = float(gate_profile.get("required_seconds", getattr(self, "_stable_tracking_window_s", 5.0)))
        first_window = (
            float(getattr(current_ranges, "TR", DEFAULT_RANGES.TR)),
            float(getattr(current_ranges, "LR", DEFAULT_RANGES.LR)),
        )
        second_window = (
            float(getattr(current_ranges, "TR_PRIME", DEFAULT_RANGES.TR_PRIME)),
            float(getattr(current_ranges, "LR_PRIME", DEFAULT_RANGES.LR_PRIME)),
        )
        survival_window = (
            float(getattr(current_ranges, "DOR", DEFAULT_RANGES.DOR)),
            float(getattr(current_ranges, "DR", DEFAULT_RANGES.DR)),
            float(getattr(current_ranges, "MAR", DEFAULT_RANGES.MAR)),
        )
        shooter_snapshot = self._get_last_maneuver_snapshot(shooter) if hasattr(self, "_get_last_maneuver_snapshot") else {}
        support_snapshot = self._get_last_maneuver_snapshot(support) if (support and hasattr(self, "_get_last_maneuver_snapshot")) else {}
        pair_assignment = TacticAssignment(
            ...
            stage_reason=(
                f"ctx_dist={float(ctx.min_threat_distance):.1f}km "
                f"pair_dist={shooter_distance:.1f}km node={pair_node} zone={target_zone} "
                f"awacs={'Y' if ctx.has_awacs_info else 'N'} "
                f"missile_incoming={'Y' if ctx.is_missile_incoming else 'N'} "
                f"rwr={int(getattr(ctx, 'rwr_level', 0) or 0)}"
            ),
            tactic_reason=(
                f"pair={pair_name} target={threat.track_id} tactic={pair_tactic.value} "
                f"src={threat_source} enemy_phase={str(target_status.get('enemy_phase', '') or '-')}"
            ),
            maneuver_reason=(
                f"shooter={shooter} support={support or '-'} "
                f"distance={shooter_distance:.1f}km node={pair_node} geometry=nearest_shooter"
            ),
            parameter_reason=(
                f"stable_required_s={stable_required_s:.1f} "
                f"gate_phase={str(gate_profile.get('phase_name', '') or '-') } "
                f"first_window=[TR:{first_window[0]:.1f},LR:{first_window[1]:.1f}] "
                f"second_window=[TR':{second_window[0]:.1f},LR':{second_window[1]:.1f}] "
                f"survival=[DOR:{survival_window[0]:.1f},DR:{survival_window[1]:.1f},MAR:{survival_window[2]:.1f}] "
                f"closure_rate={float(getattr(ctx, 'closure_rate_mps', 0.0) or 0.0):.1f}mps "
                f"time_to_merge={float(getattr(ctx, 'time_to_merge_s', float('nan'))):.1f}s "
                f"alt_delta={float(getattr(ctx, 'nearest_altitude_delta_km', 0.0) or 0.0):.2f}km "
                f"target_speed={float(getattr(ctx, 'nearest_target_speed_mps', 0.0) or 0.0):.1f}mps "
                f"dynamic_ranges={json.dumps(dynamic_ranges, ensure_ascii=False, sort_keys=True)}"
            ),
            decision_snapshot={
                "pair_name": pair_name,
                "target_id": threat.track_id,
                "tactic": pair_tactic.value,
                "shooter_id": shooter,
                "support_id": support,
                "threat_source": threat_source,
                "pair_node": pair_node,
                "target_zone": target_zone,
                "shooter_distance_km": shooter_distance,
                "stable_required_seconds": stable_required_s,
                "gate_phase_name": str(gate_profile.get("phase_name", "") or ""),
                "ctx_distance_km": float(getattr(ctx, "min_threat_distance", float("nan"))),
                "ctx_has_awacs": bool(getattr(ctx, "has_awacs_info", False)),
                "ctx_rwr_level": int(getattr(ctx, "rwr_level", 0) or 0),
                "ctx_missile_incoming": bool(getattr(ctx, "is_missile_incoming", False)),
                "ctx_closure_rate_mps": float(getattr(ctx, "closure_rate_mps", 0.0) or 0.0),
                "ctx_time_to_merge_s": float(getattr(ctx, "time_to_merge_s", float("nan"))),
                "ctx_nearest_altitude_delta_km": float(getattr(ctx, "nearest_altitude_delta_km", 0.0) or 0.0),
                "ctx_nearest_target_speed_mps": float(getattr(ctx, "nearest_target_speed_mps", 0.0) or 0.0),
                "dynamic_ranges": dict(dynamic_ranges),
                "shooter_maneuver_snapshot": shooter_snapshot,
                "support_maneuver_snapshot": support_snapshot,
            },
        )
