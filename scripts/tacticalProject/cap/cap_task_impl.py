...
try:
    from . import cap_task_action_helpers as _capaction
except ImportError:
    import cap_task_action_helpers as _capaction
try:
...
try:
    _CAP_SINGLE_SHIP_BACKSHOT_BLOCK_DEG = float(os.environ.get('CAP_SINGLE_SHIP_BACKSHOT_BLOCK_DEG', '120'))
except Exception:
    _CAP_SINGLE_SHIP_BACKSHOT_BLOCK_DEG = 120.0
try:
    _CAP_SINGLE_SHIP_SIDE_RELAY_REQUIRED_DEG = float(os.environ.get('CAP_SINGLE_SHIP_SIDE_RELAY_REQUIRED_DEG', '70'))
except Exception:
    _CAP_SINGLE_SHIP_SIDE_RELAY_REQUIRED_DEG = 70.0
try:
    _CAP_SINGLE_SHIP_SIDE_WITH_RELAY_MAX_DEG = float(os.environ.get('CAP_SINGLE_SHIP_SIDE_WITH_RELAY_MAX_DEG', '85'))
except Exception:
    _CAP_SINGLE_SHIP_SIDE_WITH_RELAY_MAX_DEG = 85.0
try:
    _CAP_SINGLE_SHIP_RELAY_MAX_HDG_ERR_DEG = float(os.environ.get('CAP_SINGLE_SHIP_RELAY_MAX_HDG_ERR_DEG', '45'))
except Exception:
    _CAP_SINGLE_SHIP_RELAY_MAX_HDG_ERR_DEG = 45.0
try:
    _CAP_SINGLE_SHIP_RELAY_MIN_TRACK_Q = float(os.environ.get('CAP_SINGLE_SHIP_RELAY_MIN_TRACK_Q', '0.25'))
except Exception:
    _CAP_SINGLE_SHIP_RELAY_MIN_TRACK_Q = 0.25
try:
    _CAP_SINGLE_SHIP_RELAY_MAX_DIST_KM = float(os.environ.get('CAP_SINGLE_SHIP_RELAY_MAX_DIST_KM', '120'))
except Exception:
    _CAP_SINGLE_SHIP_RELAY_MAX_DIST_KM = 120.0
...
def _normalize_heading_error_deg(delta_deg: float) -> float:
...
@dataclass
class CorridorSideState:
    hold_heading_deg: float = 0.0
    switch_band_km: float = 1.0
    freeze_until_s: float = -1e9
    recreate_generation: int = 0
    last_switch_time_s: float = -1e9
...
class ManeuverIntent:
    ...
    raw_command: Optional[Tuple[int, int, int]] = None
    min_dwell_s: float = 0.0
    heading_deadband_deg: float = 0.0
    speed_floor_cmd: int = 0
    alt_floor_cmd: Optional[int] = None
...
class CAPTask(CAPTaskRuntimeMixin, MultipleCombatTask):
    ...
    def _get_alive_awacs_tracks(self, env) -> dict:
        ...
        try:
            current_time = None
            for attr_name in ("_awacs_control_time_s", "_detection_focus_time_s"):
                attr_value = getattr(self, attr_name, None)
                if attr_value is not None:
                    current_time = float(attr_value)
                    break
            if current_time is None:
                current_time = float(getattr(self, "step_count", 0)) * 0.2
        except Exception:
            current_time = float(getattr(self, "step_count", 0)) * 0.2

        try:
            current_distance = None
            for attr_name in ("_awacs_control_reference_distance_km", "_detection_focus_reference_distance_km"):
                attr_value = getattr(self, attr_name, None)
                if attr_value is not None:
                    current_distance = float(attr_value)
                    break
        ...
        if bool(getattr(self, "_focus_awacs_force_control", False)):
            loss_windows = tuple(getattr(self, "_focus_awacs_loss_windows_s", ()) or ())
            for start_s, end_s in loss_windows:
                if float(start_s) <= current_time < float(end_s):
                    return {}
        else:
            scenario = getattr(self, "enemy_scenario", None)
            if scenario is not None and hasattr(scenario, "is_awacs_available"):
                try:
                    if not bool(scenario.is_awacs_available(current_time, current_distance)):
                        return {}
                except Exception:
                    pass
        try:
    ...
    def _format_sim_timestamp(self, env=None, current_time: float = None) -> str:
        step = int(getattr(env, 'current_step', getattr(self, 'step_count', 0))) if env is not None else int(getattr(self, 'step_count', 0))
        if current_time is None:
            dt = float(getattr(env, 'time_interval', 0.2)) if env is not None else 0.2
            current_time = float(step) * dt
        return get_sim_log_prefix(step=step, time_s=float(current_time))
    ...
    def _log_key_event(
        ...
        **fields,
        ...
        segments = [f"{key}={value}" for key, value in fields.items() if value is not None]
        message = f"{prefix} [{title}] [{agent_id}]"
        if segments:
            message = f"{message} " + " | ".join(segments)
        logging.log(level, message)

    def _get_cap_phase_name(self, agent_id: str) -> str:
        phase_name = str(self._get_bridge_phase_name(agent_id) or '').strip()
        if phase_name:
            return phase_name
        state = getattr(getattr(self, 'cap_state_machine', None), 'state', None)
        if state is None:
            return ""
        return str(getattr(state, 'value', state) or "")

    def _get_agent_tactic(self, agent_id: str) -> str:
        assignments = getattr(self, '_tactic_assignments_by_agent', None)
        if isinstance(assignments, dict):
            assignment = assignments.get(agent_id)
            if assignment is not None:
                tactic = getattr(assignment, 'tactic', None)
                value = getattr(tactic, 'value', None)
                if value:
                    return str(value)
                if tactic is not None:
                    return str(tactic)
        return str(self._get_bridge_tactic_name(agent_id) or '')

    def _normalize_angle_diff(self, angle_diff: float) -> float:
        return ((float(angle_diff) + 180.0) % 360.0) - 180.0

    def _compute_corridor_heading(
        self,
        *,
        lane_error_km: float,
        north_error_km: float,
        floor_escape_active: bool = False,
        force_south: bool = False,
        hold_heading: float = None,
    ) -> Tuple[float, str]:
        if floor_escape_active or force_south:
            base_heading = 180.0
            correction_deg = float(np.clip(lane_error_km * 0.45, -6.0, 6.0))
            return self._apply_corridor_heading(base_heading, correction_deg), "south_recovery"
        if abs(north_error_km) >= 8.0:
            ...
            return self._apply_corridor_heading(base_heading, correction_deg), "depth_rejoin"
        if hold_heading is None or not np.isfinite(float(hold_heading)):
            hold_heading = 0.0 if north_error_km <= 0.0 else 180.0
        else:
            hold_heading = self._normalize_corridor_side_heading(hold_heading)
        correction_deg = float(np.clip(lane_error_km * 0.8, -8.0, 8.0))
        return self._apply_corridor_heading(hold_heading, correction_deg), "corridor_hold"

    def _apply_corridor_heading(self, base_heading: float, correction_deg: float) -> float:
        base_heading = float(base_heading) % 360.0
        correction_deg = float(correction_deg)
        if base_heading < 90.0 or base_heading > 270.0:
            return float((base_heading - correction_deg) % 360.0)
        return float((base_heading + correction_deg) % 360.0)

    def _normalize_corridor_side_heading(self, heading: float) -> float:
        if not np.isfinite(float(heading)):
            return 0.0
        heading = float(heading) % 360.0
        north_delta = abs(float(self._normalize_angle_diff(heading)))
        south_delta = abs(float(self._normalize_angle_diff(heading - 180.0)))
        return 0.0 if north_delta <= south_delta else 180.0

    def _next_maneuver_intent_id(self, agent_id: str, source: str) -> str:
        if not hasattr(self, "_maneuver_intent_seq"):
            self._maneuver_intent_seq = {}
        next_seq = int(self._maneuver_intent_seq.get(agent_id, 0)) + 1
        self._maneuver_intent_seq[agent_id] = next_seq
        current_time_s = float(getattr(self, "step_count", 0)) * 0.2
        return f"{agent_id}:{source}:{current_time_s:.1f}:{next_seq}"

    def _get_maneuver_state(self, agent_id: str) -> Dict[str, object]:
        if not hasattr(self, "_maneuver_intent_state"):
            self._maneuver_intent_state = {}
        return self._maneuver_intent_state.setdefault(
            agent_id,
            {
                "active_source": "",
                "active_intent_id": "",
                "active_priority": -10**9,
                "active_until_s": -1e9,
                "last_target_heading_deg": float("nan"),
                "last_selected_step": -10**9,
                "last_command": None,
            },
        )

    def _store_maneuver_snapshot(self, agent_id: str, snapshot: Dict[str, Any]) -> None:
        if not hasattr(self, "_last_maneuver_snapshot_by_agent"):
            self._last_maneuver_snapshot_by_agent = {}
        self._last_maneuver_snapshot_by_agent[str(agent_id)] = dict(snapshot or {})

    def _get_last_maneuver_snapshot(self, agent_id: str) -> Dict[str, Any]:
        snapshots = getattr(self, "_last_maneuver_snapshot_by_agent", None)
        if not isinstance(snapshots, dict):
            return {}
        snapshot = snapshots.get(str(agent_id), {})
        return dict(snapshot) if isinstance(snapshot, dict) else {}

    def _store_template_action_plan(
        self,
        agent_id: str,
        *,
        tactic: str,
        ...
        if not hasattr(self, "_last_template_action_plan_by_agent"):
            self._last_template_action_plan_by_agent = {}
        snapshot = {
            "tactic": str(tactic or ""),
            "node": str(node or ""),
            "target_id": str(target_id or ""),
            "distance_km": float(distance_km) if np.isfinite(distance_km) else float("nan"),
            "base_heading_deg": float(base_heading_deg) % 360.0,
            "target_heading_deg": float(target_heading_deg) % 360.0,
            "alt_cmd": int(alt_cmd),
            "hdg_cmd": int(hdg_cmd),
            "spd_cmd": int(spd_cmd),
        ...
        self._last_template_action_plan_by_agent[str(agent_id)] = snapshot

    def _get_template_action_plan(self, agent_id: str) -> Dict[str, Any]:
        plans = getattr(self, "_last_template_action_plan_by_agent", None)
        if not isinstance(plans, dict):
            return {}
        plan = plans.get(str(agent_id), {})
        return dict(plan) if isinstance(plan, dict) else {}

    def _get_friendly_recreate_context(self, agent_id: str) -> Dict[str, float]:
        ...
        return {
            "generation": int(recreate_state.get("generation", 0) or 0),
            "posture_hold_until_s": float(recreate_state.get("posture_hold_until_s", -1e9) or -1e9),
            "formation_rebind_until_s": float(recreate_state.get("formation_rebind_until_s", -1e9) or -1e9),
            "recover_until_s": float(recreate_state.get("recover_until_s", -1e9) or -1e9),
        }

    def _get_corridor_side_state(
        self,
        *,
        hold_state: Optional[Dict[str, object]],
        state_key: str,
        fallback_heading: float,
        switch_band_km: float,
    ) -> CorridorSideState:
        normalized_fallback = self._normalize_corridor_side_heading(fallback_heading)
        if not isinstance(hold_state, dict):
            return CorridorSideState(hold_heading_deg=normalized_fallback, switch_band_km=float(switch_band_km))
        state = hold_state.get(state_key)
        if isinstance(state, CorridorSideState):
            state.switch_band_km = float(switch_band_km)
            return state
        state = CorridorSideState(
            hold_heading_deg=self._normalize_corridor_side_heading(
                hold_state.get("active_hold_heading_deg", hold_state.get(state_key, normalized_fallback))
            ),
            switch_band_km=float(switch_band_km),
            freeze_until_s=float(hold_state.get("corridor_side_freeze_until_s", -1e9) or -1e9),
            recreate_generation=int(hold_state.get("corridor_recreate_generation", 0) or 0),
            last_switch_time_s=float(hold_state.get("corridor_last_switch_time_s", -1e9) or -1e9),
        )
        hold_state[state_key] = state
        hold_state["active_hold_heading_deg"] = state.hold_heading_deg
        return state

    def _build_heading_intent(
        self,
        *,
        agent_id: str,
        source: str,
        ...
        return ManeuverIntent(
            intent_id=self._next_maneuver_intent_id(agent_id, source),
            source=str(source),
            producer=str(producer),
            priority=int(priority),
            desired_heading_deg=float(desired_heading_deg) % 360.0,
            min_dwell_s=float(min_dwell_s),
            heading_deadband_deg=float(max(0.0, heading_deadband_deg)),
            speed_floor_cmd=int(max(0, speed_floor_cmd)),
            alt_floor_cmd=None if alt_floor_cmd is None else int(alt_floor_cmd),
            metadata=dict(metadata or {}),
        )

    def _build_raw_command_intent(
        self,
        *,
        agent_id: str,
        source: str,
        ...
        return ManeuverIntent(
            ...
            producer=str(producer),
            priority=int(priority),
            raw_command=tuple(int(x) for x in raw_command),
            min_dwell_s=float(min_dwell_s),
            metadata=dict(metadata or {}),
        )

    def _resolve_corridor_hold_heading(
        self,
        *,
        hold_state: Optional[Dict[str, object]],
        state_key: str,
        ...
        side_state = self._get_corridor_side_state(
            hold_state=hold_state,
            state_key=state_key,
            fallback_heading=fallback_heading,
            switch_band_km=switch_band_km,
        )
        if int(side_state.recreate_generation) != int(recreate_generation):
            side_state.recreate_generation = int(recreate_generation)
            side_state.freeze_until_s = max(float(side_state.freeze_until_s), float(freeze_until_s))

        side_state.switch_band_km = float(switch_band_km)
        side_state.freeze_until_s = max(float(side_state.freeze_until_s), float(freeze_until_s))

        if float(current_time_s) >= float(side_state.freeze_until_s):
            desired_heading = float(side_state.hold_heading_deg)
            if desired_heading == 0.0:
                if north_error_km >= float(side_state.switch_band_km):
                    desired_heading = 180.0
            elif north_error_km <= -float(side_state.switch_band_km):
                desired_heading = 0.0
            if desired_heading != float(side_state.hold_heading_deg):
                side_state.hold_heading_deg = float(desired_heading)
                side_state.last_switch_time_s = float(current_time_s)

        if isinstance(hold_state, dict):
            hold_state[state_key] = side_state
            hold_state["active_hold_heading_deg"] = float(side_state.hold_heading_deg)
            hold_state["corridor_side_freeze_until_s"] = float(side_state.freeze_until_s)
            hold_state["corridor_recreate_generation"] = int(side_state.recreate_generation)
            hold_state["corridor_last_switch_time_s"] = float(side_state.last_switch_time_s)
        return float(side_state.hold_heading_deg)

    def _compute_no_chase_corridor_heading(
        self,
        *,
        ...
        if hold_heading is None or not np.isfinite(float(hold_heading)):
            ...
            hold_heading = float(hold_heading) % 360.0

        if abs(north_error_km) >= 8.0:
            base_heading = 0.0 if north_error_km < 0.0 else 180.0
            correction_deg = float(np.clip(lane_error_km * 0.55, -12.0, 12.0))
            target_heading = self._apply_corridor_heading(base_heading, correction_deg)
            return target_heading, "depth_rejoin"

        depth_hold_heading = self._normalize_corridor_side_heading(hold_heading)
        correction_deg = float(np.clip(lane_error_km * 0.75, -14.0, 14.0))
        target_heading = self._apply_corridor_heading(depth_hold_heading, correction_deg)
        hold_delta = abs(float(self._normalize_angle_diff(current_heading - depth_hold_heading)))
        if abs(lane_error_km) < 4.0 and abs(north_error_km) < 4.0 and hold_delta <= 35.0:
            return depth_hold_heading, "depth_hold"
        return target_heading, "lateral_rejoin"

    def _get_formation_mean_battlefield_pos(self, env, agent_id: str) -> Optional[Tuple[float, float]]:
        members = self._get_alive_formation_agents(env, agent_id)
        if len(members) < 2:
            return None
        xs = []
        ys = []
        for member_id in members:
            try:
                member_x_km, member_y_km = self._get_battlefield_pos(env, member_id)
            except Exception:
                continue
            xs.append(float(member_x_km))
            ys.append(float(member_y_km))
        if len(xs) < 2:
            return None
        return float(np.mean(xs)), float(np.mean(ys))

    def _heading_to_battlefield_point(self, env, agent_id: str, target_x_km: float, target_y_km: float) -> float:
        current_x_km, current_y_km = self._get_battlefield_pos(env, agent_id)
        return float(np.degrees(np.arctan2(float(target_x_km) - float(current_x_km), float(target_y_km) - float(current_y_km))) % 360.0)

    def _get_alive_formation_agents(self, env, agent_id: str) -> List[str]:
        formation = getattr(self, 'formation', None)
        if formation is None:
            aircraft = getattr(env, 'agents', {}).get(agent_id)
            return [agent_id] if aircraft is not None and getattr(aircraft, 'is_alive', False) else []
        formation_id = formation.get_formation_id(agent_id)
        members = formation.formations.get(formation_id, []) if formation_id >= 0 else []
        alive = []
        for member in members:
            aircraft = getattr(env, 'agents', {}).get(member.agent_id)
            if aircraft is not None and getattr(aircraft, 'is_alive', False):
                alive.append(member.agent_id)
        return alive

    def _get_formation_alive_count(self, env, agent_id: str) -> int:
        return len(self._get_alive_formation_agents(env, agent_id))
    ...
    def _get_nominal_lane_anchor_km(self, agent_id: str) -> Tuple[float, float]:
        formation = getattr(self, 'formation', None)
        if formation is not None:
            ...
            for member in members:
                if member.agent_id == agent_id:
                    return float(member.x_rel), float(member.y_rel)
        return (0.0, 0.0)

    def _get_cap_hold_depth_km(self, agent_id: str, aggressive: bool = False, target_zone: str = "") -> float:
        zone = str(target_zone or "").upper()
        if zone == "HIGH":
            base_depth = 112.0
        elif zone == "MEDIUM":
            base_depth = 122.0
        elif zone in ("LOW", "OUTSIDE", "UNKNOWN"):
            base_depth = 116.0
        else:
            base_depth = 116.0
        if aggressive:
            base_depth = max(110.0, min(base_depth, 116.0))
        return float(np.clip(base_depth, 108.0, 128.0))

    def _get_cap_no_chase_hold_depth_km(self, agent_id: str, reason: str = "") -> float:
        ...
        if formation is not None:
            try:
                formation_id = formation.get_formation_id(agent_id)
                members = formation.formations.get(formation_id, []) if formation_id >= 0 else []
                formation_alive = max(1, len(members))
            except Exception:
                formation_alive = 2
        hold_depth_km = 96.0
        if any(tag in reason_text for tag in ("enemy_regroup", "winchester", "low_zone", "latched_defense", "pair_guard")):
            hold_depth_km = 92.0
        if "bvr_floor_overrun" in reason_text:
            hold_depth_km = min(hold_depth_km, 88.0 if formation_alive >= 2 else 84.0)
        if any(tag in reason_text for tag in ("enemy_regroup_north", "enemy_regroup_no_chase", "low_zone_hold_line")):
            hold_depth_km = min(hold_depth_km, 88.0 if formation_alive >= 2 else 84.0)
        return float(np.clip(hold_depth_km, 84.0, 104.0))

    def _is_cap_no_chase_preserve_reason(self, reason: str) -> bool:
        reason_text = str(reason or "")
        return any(
            tag in reason_text
            for tag in (
    ...
    def _get_cap_bvr_entry_floor_km(self, agent_id: str) -> float:
        return 40.0 if str(agent_id).startswith('A') else 26.0

    def _get_cap_bvr_release_floor_km(self, agent_id: str) -> float:
        return 52.0 if str(agent_id).startswith('A') else 30.0

    def _get_cap_bvr_preempt_floor_km(self, agent_id: str) -> float:
        return 48.0 if str(agent_id).startswith('A') else 30.0

    def _resolve_preserve_target(self, env, target=None) -> Dict[str, object]:
        info = {"track_id": "", "x_km": float("nan"), "y_km": float("nan"), "source": "none"}
        if target is None:
            return info
        track_id = str(getattr(target, 'track_id', '') or '').strip()
        if hasattr(target, 'x') and hasattr(target, 'y'):
            try:
                info["x_km"] = float(target.x)
                info["y_km"] = float(target.y)
                info["source"] = "picture_track"
        ...
        if hasattr(target, 'get_position'):
            try:
                ...
                info["x_km"] = float(pos[1]) / 1000.0
                info["y_km"] = float(pos[0]) / 1000.0
                info["source"] = "aircraft"
                info["track_id"] = track_id or str(getattr(target, 'uid', '') or '')
                return info
            except Exception:
                pass
        if not track_id:
            track_id = str(target or "").strip()
        if track_id:
            resolved_id = track_id
            if hasattr(env, 'resolve_real_agent_id'):
                try:
                    candidate = env.resolve_real_agent_id(track_id)
                    if candidate:
                        resolved_id = str(candidate)
                except Exception:
                    resolved_id = track_id
            aircraft = getattr(env, 'agents', {}).get(resolved_id) or getattr(env, 'agents', {}).get(track_id)
            if aircraft is not None and getattr(aircraft, 'is_alive', False):
                try:
                    pos = aircraft.get_position()
                    info["x_km"] = float(pos[1]) / 1000.0
                    info["y_km"] = float(pos[0]) / 1000.0
                    info["source"] = "env_agent"
                    info["track_id"] = track_id
                    return info
                except Exception:
                    pass
    ...
    def _normalize_preserve_distance_km(self, env, agent_id: str, target_info: Dict[str, object], distance: float = None) -> float:
        if distance is not None and np.isfinite(distance):
            value = float(distance)
            return value / 1000.0 if value > 1000.0 else value
        target_x_km = float(target_info.get("x_km", float("nan")))
        target_y_km = float(target_info.get("y_km", float("nan")))
        if np.isfinite(target_x_km) and np.isfinite(target_y_km):
            current_x_km, current_y_km = self._get_battlefield_pos(env, agent_id)
            return float(np.hypot(target_x_km - current_x_km, target_y_km - current_y_km))
        return float("nan")

    def _get_agent_nearest_hostile_target_info(self, env, agent_id: str) -> Tuple[Dict[str, object], float]:
        info = {"track_id": "", "x_km": float("nan"), "y_km": float("nan"), "source": "none"}
        current_x_km, current_y_km = self._get_battlefield_pos(env, agent_id)
        hostile_prefix = 'B' if str(agent_id).startswith('A') else 'A'
        best_target = None
        best_distance_km = float("inf")

        for other_id, other_aircraft in getattr(env, 'agents', {}).items():
            other_id = str(other_id)
            if not other_id.startswith(hostile_prefix) or not getattr(other_aircraft, 'is_alive', False):
                continue
            try:
                other_x_km, other_y_km = self._get_battlefield_pos(env, other_id)
                distance_km = float(np.hypot(other_x_km - current_x_km, other_y_km - current_y_km))
            except Exception:
                continue
            if distance_km < best_distance_km:
                best_distance_km = distance_km
                best_target = other_aircraft

        if best_target is None or not np.isfinite(best_distance_km):
            return info, float("nan")
        return self._resolve_preserve_target(env, target=best_target), float(best_distance_km)

    def _get_friendly_close_block_context(
        self,
        env,
        agent_id: str,
        launch_target_id: str = "",
        launch_distance_km: float = None,
    ) -> Dict[str, object]:
        current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
        floor_km = max(
            float(getattr(current_ranges, "MAR", DEFAULT_RANGES.MAR)),
            float(getattr(current_ranges, "TR_PRIME", DEFAULT_RANGES.TR_PRIME)) - 3.0,
        )
        block_distance_km = float(launch_distance_km) if launch_distance_km is not None and np.isfinite(launch_distance_km) else float("nan")
        block_target_id = str(launch_target_id or "")
        block_source = "launch_target"
        live_nearest_info = {"track_id": "", "x_km": float("nan"), "y_km": float("nan"), "source": "none"}
        live_nearest_distance_km = float("nan")
        if str(agent_id).startswith('A'):
            try:
                live_nearest_info, live_nearest_distance_km = self._get_agent_nearest_hostile_target_info(env, agent_id)
            except Exception:
                live_nearest_info = {"track_id": "", "x_km": float("nan"), "y_km": float("nan"), "source": "none"}
                live_nearest_distance_km = float("nan")
            if np.isfinite(live_nearest_distance_km) and (
                not np.isfinite(block_distance_km) or float(live_nearest_distance_km) < float(block_distance_km)
            ):
                block_distance_km = float(live_nearest_distance_km)
                block_target_id = str(live_nearest_info.get("track_id") or block_target_id)
                block_source = "live_nearest"
        return {
            "floor_km": float(floor_km),
            "distance_km": float(block_distance_km) if np.isfinite(block_distance_km) else float("nan"),
            "target_id": block_target_id,
            "source": block_source,
            "live_nearest_target_id": str(live_nearest_info.get("track_id") or ""),
            "live_nearest_distance_km": float(live_nearest_distance_km) if np.isfinite(live_nearest_distance_km) else float("nan"),
        }

    def _get_preserve_target_zone(self, target_info: Dict[str, object], current_time: float = None) -> str:
        track_id = str(target_info.get("track_id", "") or "").strip()
        if track_id:
            try:
                status = self._get_picture_target_actionability(track_id, current_time=current_time, purpose="decision")
                zone = str(status.get("target_zone", "") or "").upper()
                if zone:
                    return zone
            except Exception:
                pass
        target_y_km = float(target_info.get("y_km", float("nan")))
        if not np.isfinite(target_y_km):
            return "UNKNOWN"
        if 0.0 <= target_y_km < 100.0:
            return "HIGH"
        if 100.0 <= target_y_km < 200.0:
            return "MEDIUM"
        if 200.0 <= target_y_km <= 300.0:
            return "LOW"
        return "OUTSIDE"

    def _build_cap_bvr_hard_escape_command(self, env, agent_id: str, target_info: Dict[str, object], distance_km: float, reason: str = "") -> ManeuverIntent:
        current_x_km, current_y_km = self._get_battlefield_pos(env, agent_id)
        lane_x_km, _ = self._get_nominal_lane_anchor_km(agent_id)
        entry_floor_km = float(self._get_cap_bvr_entry_floor_km(agent_id))
        release_floor_km = float(self._get_cap_bvr_release_floor_km(agent_id))
        if not hasattr(self, '_cap_bvr_escape_state'):
            self._cap_bvr_escape_state = {}
        escape_state = self._cap_bvr_escape_state.setdefault(str(agent_id), {})

        floor_escape_active = bool(escape_state.get('floor_escape_active', False))
        if np.isfinite(distance_km):
            if floor_escape_active:
                floor_escape_active = float(distance_km) < release_floor_km
            else:
                floor_escape_active = float(distance_km) <= entry_floor_km
        else:
            floor_escape_active = False
        escape_state['floor_escape_active'] = floor_escape_active

        lane_error_km = float(current_x_km - lane_x_km)
        south_heading = float((180.0 + np.clip(lane_error_km * 0.45, -8.0, 8.0)) % 360.0)
        desired_heading = south_heading
        target_x_km = float(target_info.get("x_km", float("nan")))
        target_y_km = float(target_info.get("y_km", float("nan")))
        if np.isfinite(target_x_km) and np.isfinite(target_y_km):
            threat_bearing = float(np.degrees(np.arctan2(target_x_km - current_x_km, target_y_km - current_y_km)) % 360.0)
            away_heading = float((threat_bearing + 180.0) % 360.0)
            desired_heading = float((south_heading + np.clip(self._normalize_angle_diff(away_heading - south_heading), -16.0, 16.0)) % 360.0)
        return self._build_heading_intent(
            agent_id=agent_id,
            source="bvr_hard_escape",
            producer="_build_cap_bvr_hard_escape_command",
            priority=120,
            desired_heading_deg=desired_heading,
            min_dwell_s=5.0,
            heading_deadband_deg=8.0,
            speed_floor_cmd=6 if floor_escape_active or distance_km <= 35.0 else 5,
            alt_floor_cmd=7,
            metadata={
                "reason": str(reason or ""),
                "distance_km": float(distance_km) if np.isfinite(distance_km) else float("nan"),
                "target_id": str(target_info.get("track_id", "") or ""),
                "mode": "bvr_hard_escape",
                "floor_escape_active": bool(floor_escape_active),
                "entry_floor_km": float(entry_floor_km),
                "release_floor_km": float(release_floor_km),
                "lane_error_km": float(lane_error_km),
                "south_heading_deg": float(south_heading),
            },
        )

    def _build_cap_multi_ship_defensive_hold_command(
        self,
        env,
        agent_id: str,
        ...
        current_x_km, current_y_km = self._get_battlefield_pos(env, agent_id)
        lane_x_km, _ = self._get_nominal_lane_anchor_km(agent_id)
        target_zone = self._get_preserve_target_zone(target_info, current_time=current_time)
        hold_depth_km = self._get_cap_hold_depth_km(agent_id, aggressive=False, target_zone=target_zone)
        reason_text = str(reason or "")
        no_chase_reason = bool(self._is_cap_no_chase_preserve_reason(reason_text))
        if any(tag in reason_text for tag in ("enemy_regroup", "winchester", "latched_defense", "low_zone", "pair_guard", "bvr_floor_overrun")):
            hold_depth_km = self._get_cap_hold_depth_km(agent_id, aggressive=True, target_zone=target_zone)

        if not hasattr(self, '_cap_multi_ship_hold_state'):
            self._cap_multi_ship_hold_state = {}
        formation_label = self._get_formation_label(agent_id)
        hold_state = self._cap_multi_ship_hold_state.setdefault(formation_label, {})

        hold_profile = "no_chase" if no_chase_reason else f"zone:{target_zone}"
        captured_hold_depth_km = float(hold_state.get("captured_hold_depth_km", float("nan")))
        captured_lane_x_km = float(hold_state.get("captured_lane_x_km", float("nan")))
        captured_hold_heading = float(hold_state.get("captured_hold_heading_deg", float("nan")))
        recreate_ctx = self._get_friendly_recreate_context(agent_id)
        recreate_freeze_until_s = max(
            float(recreate_ctx.get("posture_hold_until_s", -1e9)),
            float(recreate_ctx.get("formation_rebind_until_s", -1e9)),
        )
        if hold_state.get("hold_profile") != hold_profile or not np.isfinite(captured_hold_depth_km):
            if no_chase_reason:
                mean_pos = self._get_formation_mean_battlefield_pos(env, agent_id)
                mean_x_km = float(mean_pos[0]) if mean_pos is not None else float(current_x_km)
                mean_y_km = float(mean_pos[1]) if mean_pos is not None else float(current_y_km)
                captured_hold_depth_km = self._get_cap_no_chase_hold_depth_km(agent_id, reason_text)
                captured_lane_x_km = mean_x_km
                captured_hold_heading = 0.0 if mean_y_km <= captured_hold_depth_km else 180.0
            else:
                captured_hold_depth_km = float(np.clip(hold_depth_km, 40.0, 128.0))
                captured_lane_x_km = float(lane_x_km)
                captured_hold_heading = 0.0 if current_y_km <= captured_hold_depth_km else 180.0
            hold_state["captured_hold_depth_km"] = captured_hold_depth_km
            hold_state["captured_lane_x_km"] = captured_lane_x_km
            hold_state["captured_hold_heading_deg"] = captured_hold_heading
            hold_state["active_hold_heading_deg"] = captured_hold_heading
            hold_state["hold_profile"] = hold_profile
        if no_chase_reason:
            hold_depth_km = captured_hold_depth_km
            if np.isfinite(captured_lane_x_km):
                lane_x_km = captured_lane_x_km

        lane_error_km = float(current_x_km - lane_x_km)
        north_error_km = float(current_y_km - hold_depth_km)
        entry_floor_km = float(self._get_cap_bvr_entry_floor_km(agent_id))
        release_floor_km = float(self._get_cap_bvr_release_floor_km(agent_id))
        floor_escape_active = bool(hold_state.get('floor_escape_active', False))
        if distance_km is not None and np.isfinite(distance_km):
            ...
            floor_escape_active = False
        hold_state['floor_escape_active'] = floor_escape_active

        forced_south_recovery = bool(
            current_y_km >= (hold_depth_km + 6.0)
            or floor_escape_active
            or "bvr_floor_overrun" in reason_text
        )
        shared_lane_error_km = lane_error_km
        shared_north_error_km = north_error_km
        active_hold_heading = self._normalize_corridor_side_heading(captured_hold_heading)
        if no_chase_reason and not forced_south_recovery:
            mean_pos = self._get_formation_mean_battlefield_pos(env, agent_id)
            if mean_pos is not None:
                shared_lane_error_km = float(mean_pos[0] - lane_x_km)
                shared_north_error_km = float(mean_pos[1] - hold_depth_km)
            else:
                shared_lane_error_km = lane_error_km
                shared_north_error_km = north_error_km
            active_hold_heading = self._resolve_corridor_hold_heading(
                hold_state=hold_state,
                state_key="active_hold_heading_deg",
                fallback_heading=captured_hold_heading,
                north_error_km=shared_north_error_km,
                switch_band_km=2.0,
                current_time_s=float(current_time or 0.0),
                freeze_until_s=recreate_freeze_until_s,
                recreate_generation=int(recreate_ctx.get("generation", 0)),
            )
            current_heading = float(self._get_heading(env, agent_id))
            target_heading, corridor_mode = self._compute_no_chase_corridor_heading(
                current_heading=current_heading,
                lane_error_km=shared_lane_error_km,
                north_error_km=shared_north_error_km,
                hold_heading=active_hold_heading,
            )
        else:
            if not forced_south_recovery:
                active_hold_heading = self._resolve_corridor_hold_heading(
                    hold_state=hold_state,
                    state_key="active_hold_heading_deg",
                    fallback_heading=captured_hold_heading,
                    north_error_km=north_error_km,
                    switch_band_km=1.0,
                    current_time_s=float(current_time or 0.0),
                    freeze_until_s=recreate_freeze_until_s,
                    recreate_generation=int(recreate_ctx.get("generation", 0)),
                )
            target_heading, corridor_mode = self._compute_corridor_heading(
                lane_error_km=lane_error_km,
                north_error_km=north_error_km,
                floor_escape_active=floor_escape_active,
                force_south=forced_south_recovery,
                hold_heading=active_hold_heading,
            )
        recovery_mode = bool(corridor_mode != "corridor_hold")
        lateral_rejoin_only = bool(corridor_mode in ("depth_rejoin", "lateral_rejoin") and abs(lane_error_km) >= 3.0)
        intent = self._build_heading_intent(
            agent_id=agent_id,
            source="defensive_corridor_hold",
            producer="_build_cap_multi_ship_defensive_hold_command",
            priority=90,
            desired_heading_deg=target_heading,
            min_dwell_s=8.0,
            heading_deadband_deg=7.0,
            speed_floor_cmd=6 if floor_escape_active else (5 if recovery_mode else 4),
            alt_floor_cmd=7,
            metadata={
                "corridor_mode": corridor_mode,
                "reason": reason_text,
                "corridor_side": "north" if active_hold_heading == 0.0 else "south",
                "recreate_generation": int(recreate_ctx.get("generation", 0)),
                "mode": "defensive_corridor_hold",
                "target_zone": str(target_zone),
                "lane_error_km": float(lane_error_km),
                "north_error_km": float(north_error_km),
                "shared_lane_error_km": float(shared_lane_error_km),
                "shared_north_error_km": float(shared_north_error_km),
                "hold_depth_km": float(hold_depth_km),
                "active_hold_heading_deg": float(active_hold_heading),
                "recovery_mode": bool(recovery_mode),
                "forced_south_recovery": bool(forced_south_recovery),
                "floor_escape_active": bool(floor_escape_active),
                "lateral_rejoin_only": bool(lateral_rejoin_only),
            },
        )
        self._log_key_event(
            env,
            ...
            当前阶段=self._get_cap_phase_name(agent_id),
            当前战术=self._get_agent_tactic(agent_id),
            进入函数="_build_cap_multi_ship_defensive_hold_command",
            执行机动="DEFENSIVE_CORRIDOR_HOLD",
            当前状态=(
                f"pair={self._get_formation_label(agent_id)} zone={target_zone} "
                f"lane={lane_error_km:.1f}km north={north_error_km:.1f}km "
                f"shared_lane={shared_lane_error_km:.1f}km shared_north={shared_north_error_km:.1f}km "
                f"recovery={recovery_mode} south_recovery={forced_south_recovery} "
                f"lateral_rejoin={lateral_rejoin_only} floor_escape={floor_escape_active} "
                f"mode={corridor_mode} hold_hdg={captured_hold_heading:.1f} active_hold_hdg={active_hold_heading:.1f} "
                f"no_chase={no_chase_reason} corridor_side={'north' if active_hold_heading == 0.0 else 'south'} "
                f"recreate_generation={int(recreate_ctx.get('generation', 0))}"
            ),
            目标点=f"x={lane_x_km:.1f}km y={hold_depth_km:.1f}km hdg={target_heading:.1f}",
            当前指令=f"intent={intent.intent_id} producer={intent.producer} priority={intent.priority}",
            退出条件=f"distance>40km and hold capture, reason={reason_text or 'defensive_guard'}",
            是否满足退出="否",
            thread_id=str(threading.get_ident()),
        )
        return intent

    def _build_cap_solo_defensive_hold_command(
        self,
        env,
        agent_id: str,
        target_info: Dict[str, object],
        ...
        reason_text = str(reason or "")
        no_chase_reason = bool(self._is_cap_no_chase_preserve_reason(reason_text))
        aggressive = any(tag in reason_text for tag in ("enemy_regroup", "winchester", "low_zone", "pair_guard", "bvr_floor_overrun"))
        hold_depth_km = self._get_cap_hold_depth_km(agent_id, aggressive=aggressive, target_zone="")
        if not hasattr(self, '_cap_solo_defensive_hold_state'):
            self._cap_solo_defensive_hold_state = {}
        hold_state = self._cap_solo_defensive_hold_state.setdefault(agent_id, {})
        hold_profile = "no_chase" if no_chase_reason else ("aggressive" if aggressive else "normal")
        captured_hold_depth_km = float(hold_state.get("captured_hold_depth_km", float("nan")))
        captured_lane_x_km = float(hold_state.get("captured_lane_x_km", float("nan")))
        captured_hold_heading = float(hold_state.get("captured_hold_heading_deg", float("nan")))
        recreate_ctx = self._get_friendly_recreate_context(agent_id)
        ...
        intent = self._build_heading_intent(
            ...
            producer="_build_cap_solo_defensive_hold_command",
            priority=88,
            desired_heading_deg=target_heading,
            min_dwell_s=8.0,
            heading_deadband_deg=7.0,
            ...
            metadata={
                ...
                "forced_south_recovery": bool(forced_south_recovery),
                "floor_escape_active": bool(floor_escape_active),
                "no_chase_reason": bool(no_chase_reason),
            },
        )
        self._log_key_event(
            env,
            ...
            当前阶段=self._get_cap_phase_name(agent_id),
            当前战术=self._get_agent_tactic(agent_id),
            进入函数="_build_cap_solo_defensive_hold_command",
            执行机动="DEFENSIVE_HOLD",
            当前状态=(
                f"lane={lane_error_km:.1f}km north={north_error_km:.1f}km "
                f"recovery={recovery_mode} floor_escape={floor_escape_active} "
                f"mode={corridor_mode} hold_hdg={captured_hold_heading:.1f} active_hold_hdg={active_hold_heading:.1f} "
                f"no_chase={no_chase_reason} corridor_side={'north' if active_hold_heading == 0.0 else 'south'} "
                f"recreate_generation={int(recreate_ctx.get('generation', 0))}"
            ),
            目标点=f"x={lane_x_km:.1f}km y={hold_depth_km:.1f}km hdg={target_heading:.1f}",
            当前指令=f"intent={intent.intent_id} producer={intent.producer} priority={intent.priority}",
            退出条件=f"distance>40km and solo hold capture, reason={reason_text or 'defensive_guard'}",
            是否满足退出="否",
            thread_id=str(threading.get_ident()),
        )
        return intent
    ...
    def _execute_defensive_preserve_action(
        ...
        current_time = float(getattr(env, 'current_step', getattr(self, 'step_count', 0))) * float(getattr(env, 'time_interval', 0.2))
        target_info = self._resolve_preserve_target(env, target=target)
        distance_km = self._normalize_preserve_distance_km(env, agent_id, target_info, distance=distance)
        escape_target_info = target_info
        escape_distance_km = distance_km
        escape_distance_source = "preserve_target"
        live_nearest_info, live_nearest_distance_km = self._get_agent_nearest_hostile_target_info(env, agent_id)
        if np.isfinite(live_nearest_distance_km) and (
            not np.isfinite(escape_distance_km) or float(live_nearest_distance_km) < float(escape_distance_km)
        ):
            escape_target_info = live_nearest_info
            escape_distance_km = float(live_nearest_distance_km)
            escape_distance_source = "live_nearest"
        preempt_floor_km = float(self._get_cap_bvr_preempt_floor_km(agent_id))
        if np.isfinite(escape_distance_km) and escape_distance_km <= preempt_floor_km:
            intent = self._build_cap_bvr_hard_escape_command(
                env,
                agent_id,
                escape_target_info,
                escape_distance_km,
                reason=f"{reason or 'defensive_guard'}|bvr_floor_overrun|preempt_floor:{preempt_floor_km:.1f}km",
            )
            self._log_key_event(
                env,
                agent_id,
                ...
                当前阶段=self._get_cap_phase_name(agent_id),
                当前战术=self._get_agent_tactic(agent_id),
                进入函数="_execute_defensive_preserve_action",
                执行机动="BVR_STANDOFF",
                当前状态=f"dist={escape_distance_km:.1f}km source={escape_target_info.get('source')} trigger={escape_distance_source}",
                目标点=escape_target_info.get("track_id") or None,
                当前指令=f"intent={intent.intent_id} producer={intent.producer} priority={intent.priority}",
                退出条件=f"distance > {preempt_floor_km:.0f}km",
                是否满足退出="否",
            )
            return intent
        if self._get_formation_alive_count(env, agent_id) >= 2:
            return self._build_cap_multi_ship_defensive_hold_command(
                env,
                agent_id,
                target_info,
        ...
        return self._build_cap_solo_defensive_hold_command(
            env,
            agent_id,
            target_info,
            distance_km=distance_km if np.isfinite(distance_km) else None,
    ...
    def _resolve_maneuver_intent_command(
        self,
        env,
        agent_id: str,
        intents: List[ManeuverIntent],
        *,
        current_time: float,
    ) -> Tuple[int, int, int]:
        valid_intents = [intent for intent in intents if isinstance(intent, ManeuverIntent)]
        if not valid_intents:
            return 7, 8, 3

        valid_intents.sort(key=lambda intent: (int(intent.priority), float(intent.min_dwell_s)), reverse=True)
        selected = valid_intents[0]
        state = self._get_maneuver_state(agent_id)
        active_source = str(state.get("active_source", "") or "")
        active_until_s = float(state.get("active_until_s", -1e9) or -1e9)
        active_priority = int(state.get("active_priority", -10**9) or -10**9)
        if float(current_time) < active_until_s and active_source:
            active_candidate = next((intent for intent in valid_intents if intent.source == active_source), None)
            if active_candidate is not None and int(selected.priority) < (active_priority + 20):
                selected = active_candidate

        if selected.raw_command is not None:
            command = [int(selected.raw_command[0]), int(selected.raw_command[1]), int(selected.raw_command[2])]
            if selected.alt_floor_cmd is not None:
                command[0] = max(int(command[0]), int(selected.alt_floor_cmd))
            command[2] = max(int(command[2]), int(selected.speed_floor_cmd))
            resolved_command = tuple(command)
        else:
            resolved_command = self.build_return_command_with_alt_guard(
                env,
                agent_id,
                float(selected.desired_heading_deg if selected.desired_heading_deg is not None else 0.0),
                intent=selected,
            )

        state["active_source"] = str(selected.source)
        state["active_intent_id"] = str(selected.intent_id)
        state["active_priority"] = int(selected.priority)
        state["active_until_s"] = float(current_time) + float(selected.min_dwell_s)
        state["last_selected_step"] = int(getattr(self, "step_count", 0))
        state["last_command"] = tuple(int(x) for x in resolved_command)
        template_plan = self._get_template_action_plan(agent_id)
        current_heading_deg = float(self._get_heading(env, agent_id))
        current_x_km, current_y_km = self._get_battlefield_pos(env, agent_id)
        try:
            current_alt_m = float(env.agents[agent_id].get_property_value(c.position_h_sl_m))
        except Exception:
            current_alt_m = float("nan")
        try:
            current_speed_mps = float(env.agents[agent_id].get_property_value(c.velocities_vc_mps))
        except Exception:
        ...
        snapshot = {
            ...
            "intent_id": str(selected.intent_id),
            "intent_source": str(selected.source),
            "intent_producer": str(selected.producer),
            "intent_priority": int(selected.priority),
            "min_dwell_s": float(selected.min_dwell_s),
            "heading_deadband_deg": float(selected.heading_deadband_deg),
            "resolved_command": [int(x) for x in resolved_command],
            "selected_heading_deg": (
                float(selected.desired_heading_deg) % 360.0
                if selected.desired_heading_deg is not None
                else float("nan")
            ),
            "current_heading_deg": float(current_heading_deg),
            "current_x_km": float(current_x_km),
            "current_y_km": float(current_y_km),
            "current_alt_m": float(current_alt_m) if np.isfinite(current_alt_m) else float("nan"),
            "current_speed_mps": float(current_speed_mps) if np.isfinite(current_speed_mps) else float("nan"),
            "metadata": metadata,
            "template_plan": template_plan,
        }
        if selected.desired_heading_deg is not None:
            snapshot["heading_error_deg"] = float(
                self._normalize_angle_diff(float(selected.desired_heading_deg) - current_heading_deg)
            )
        self._store_maneuver_snapshot(agent_id, snapshot)

        if getattr(self, "step_count", 0) % 10 == 0:
            log.info(
                "[CAP_MANEUV_INTENT] agent=%s intent_id=%s source=%s producer=%s priority=%d selected=%s thread=%s",
                agent_id,
                selected.intent_id,
                selected.source,
                selected.producer,
                int(selected.priority),
                tuple(int(x) for x in resolved_command),
                threading.get_ident(),
            )
        return tuple(int(x) for x in resolved_command)
    ...
    def build_return_command_with_alt_guard(
        ...
        intent: Optional[ManeuverIntent] = None,
    ) -> Tuple[int, int, int]:
        current_heading = float(self._get_heading(env, agent_id))
        ...
        maneuver_state = self._get_maneuver_state(agent_id)
        if intent is not None and intent.desired_heading_deg is not None:
            previous_heading = float(maneuver_state.get("last_target_heading_deg", float("nan")))
            same_source = str(maneuver_state.get("active_source", "") or "") == str(intent.source)
            if (
                same_source
                and np.isfinite(previous_heading)
                and abs(float(self._normalize_angle_diff(float(target_heading) - previous_heading))) <= float(intent.heading_deadband_deg)
            ):
                target_heading = float(previous_heading)
            maneuver_state["last_target_heading_deg"] = float(target_heading) % 360.0
        else:
            maneuver_state["last_target_heading_deg"] = float(target_heading) % 360.0
        ...
        if intent is not None:
            cmd = list(cmd)
            if intent.alt_floor_cmd is not None:
                cmd[0] = max(int(cmd[0]), int(intent.alt_floor_cmd))
            cmd[2] = max(int(cmd[2]), int(intent.speed_floor_cmd))
            cmd = tuple(cmd)
        return cmd
    ...
    def _find_single_ship_relay_support(self, env, shooter_id: str, target_id: str) -> Optional[Dict[str, float]]:
        """为大离轴单机发射寻找可承担后续照射的友机。"""
        target_ac = env.agents.get(target_id)
        if target_ac is None or not getattr(target_ac, "is_alive", False):
            return None
        if hasattr(self, "_get_picture_target_actionability"):
            ...
            if not bool(target_status.get("actionable", True)):
                return None
        target_x, target_y = self._get_battlefield_pos(env, target_id)
        best_info = None
        best_score = -1e9
        for aid, aircraft in getattr(env, "agents", {}).items():
            aid = str(aid)
            if not aid.startswith("A") or aid == shooter_id:
                continue
            if not getattr(aircraft, "is_alive", False):
                continue
            if self._is_agent_in_defensive_maneuver(env, aid):
                continue
            relay_q = float(self.cap_radar.get_track_quality(aid, target_id))
            if relay_q < float(_CAP_SINGLE_SHIP_RELAY_MIN_TRACK_Q):
                continue
            relay_x, relay_y = self._get_battlefield_pos(env, aid)
            relay_dist = float(np.hypot(target_x - relay_x, target_y - relay_y))
            if relay_dist > float(_CAP_SINGLE_SHIP_RELAY_MAX_DIST_KM):
                continue
            relay_hdg = self._get_heading(env, aid)
            relay_bearing = np.degrees(np.arctan2(target_x - relay_x, target_y - relay_y)) % 360.0
            relay_hdg_err = abs(_normalize_heading_error_deg(relay_bearing - relay_hdg))
            if relay_hdg_err > float(_CAP_SINGLE_SHIP_RELAY_MAX_HDG_ERR_DEG):
                continue
            score = relay_q * 100.0 - relay_dist * 0.30 - relay_hdg_err * 0.45
            if score > best_score:
                best_score = score
                best_info = {
                    "agent_id": aid,
                    "track_quality": float(relay_q),
                    "heading_error": float(relay_hdg_err),
                    "distance_km": float(relay_dist),
                }
        return best_info
    ...
    def get_action(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        if getattr(self, "_detection_focus_mode", False):
            return self._get_detection_focus_action(env, agent_id)
    ...
    def _get_detection_focus_action(self, env, agent_id: str) -> Tuple[int, int, int]:
        """Dedicated frontal-intercept action policy for cooperative-detection validation."""
        ac = env.agents.get(agent_id)
        if not ac or not ac.is_alive:
            return 7, 8, 3
        ...
        current_time = float(self.step_count) * 0.2
        ...
        try:
            ...
            awacs_tracks = {}
        for track in awacs_tracks.values():
        ...
        if not points:
            for threat in self._get_picture_threats(current_time=current_time, purpose="search"):
                if hasattr(threat, "x") and hasattr(threat, "y"):
                    points.append((float(threat.x), float(threat.y)))

        if points:
            sorted_points = sorted(points, key=lambda p: (p[0], p[1]))
            lane_order = {"A0100": 0, "A0200": 1, "A0300": 2, "A0400": 3}
            target_index = min(lane_order.get(agent_id, 0), len(sorted_points) - 1)
            tx, ty = sorted_points[target_index]
            global_tx = float(np.mean([p[0] for p in sorted_points]))
            global_ty = float(np.mean([p[1] for p in sorted_points]))

            # Keep the dedicated validation geometry frontal: advance mainly along +Y,
            # and only use bounded lateral correction to align each aircraft with its lane.
            lateral_error = float(tx - x)
            lateral_correction = float(np.clip(lateral_error * 0.35, -18.0, 18.0))
            lookahead_y = max(float(ty), float(global_ty), float(y) + 80.0)
            aim_x = float(x + lateral_correction)
            aim_y = lookahead_y

            target_hdg = float(np.degrees(np.arctan2(aim_x - x, aim_y - y)) % 360.0)
            ref_dist = float(np.hypot(global_tx - x, global_ty - y))
            if ref_dist >= 250.0:
                spd_cmd = 5
            elif ref_dist >= 180.0:
                spd_cmd = 4
    ...
    def _get_enemy_action_simple(self, env, agent_id: str) -> Tuple[int, int, int]:
        return _capaction._get_enemy_action_simple(self, env, agent_id)
    ...
    def _get_intercept_action(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        if use_awacs_tracks:
            for tid, t in awacs_tracks.items():
                ...
                target_confidences[tid] = float(getattr(t, 'confidence', 1.0) or 1.0)
        else:
            ...
            picture_tracks = list(self._get_picture_threats(current_time=current_time, purpose="search"))
            picture_tracks = sorted(
                picture_tracks,
                key=lambda track: (
                    float(getattr(track, 'confidence', 1.0) or 1.0),
                    float(getattr(track, 'timestamp', -1.0) or -1.0),
                ),
                reverse=True,
            )[:10]
            ...
            for track in picture_tracks:
                if hasattr(track, 'x') and hasattr(track, 'y'):
                    ...
                    target_confidences[track.track_id] = float(getattr(track, 'confidence', 1.0) or 1.0)

        if len(target_positions_raw) > 10:
            sorted_ids = sorted(
                target_positions_raw.keys(),
                key=lambda tid: (
                    float(target_confidences.get(tid, 1.0)),
                    float(target_measure_times.get(tid, -1.0)),
                ),
                reverse=True,
            )[:10]
            target_positions_raw = {tid: target_positions_raw[tid] for tid in sorted_ids}
            target_measure_times = {tid: target_measure_times.get(tid, current_time) for tid in sorted_ids}
            target_confidences = {tid: target_confidences.get(tid, 1.0) for tid in sorted_ids}
        ...
        responsibility_map = {}
        responsibility_role_map = {}
        responsibility_age = None
        try:
            responsibility_map = self.coop_detection.get_responsibility_bearing_map()
            responsibility_role_map = self.coop_detection.get_responsibility_role_map()
            responsibility_age = current_time - float(self.coop_detection.get_responsibility_update_time())
        except Exception:
            responsibility_map = {}
            responsibility_role_map = {}
            responsibility_age = None

        effective_responsibility_map = (
            responsibility_map if responsibility_map and (responsibility_age is None or responsibility_age <= 5.0) else {}
        )
        effective_role_map = (
            responsibility_role_map if effective_responsibility_map else {}
        )
        responsibility_weight_scale = 1.0
        try:
            coop_mode = getattr(self.coop_detection, "mode", None)
            if coop_mode is not None and "SEARCH" in str(coop_mode):
                responsibility_weight_scale = 0.35
        except Exception:
            responsibility_weight_scale = 1.0
        ...
        guidance = self.formation_guidance.compute_guidance(
            ...
            responsibility_map=effective_responsibility_map,
            responsibility_role_map=effective_role_map,
            responsibility_weight_scale=responsibility_weight_scale,
        )
        
        guided_target_points = {fid: g.target_point for fid, g in guidance.items()}
        guided_target_headings = {fid: g.target_heading for fid, g in guidance.items()}

        my_guidance = guidance.get(agent_id)
        spd_cmd = 3  # 默认：保持速度
        target_hdg = current_hdg  # 默认保持当前航向
        opening_trace_enabled = os.environ.get('CAP_OPENING_TRACE', '0').strip().lower() in ('1', 'true', 'yes', 'on')
        opening_trace_window = int(os.environ.get('CAP_OPENING_TRACE_STEPS', '12') or '12')
        opening_trace_active = (
            opening_trace_enabled
            and self.cap_state_machine.state == CAPState.INTERCEPT
            and int(self.step_count) <= max(0, opening_trace_window)
            and agent_id in ('A0100', 'A0200', 'A0300', 'A0400')
        )
        
        if my_guidance:
            target_hdg_raw = guided_target_headings.get(agent_id, my_guidance.target_heading)
            
            # ========== 层6：协调层 - 速度协调（算法2.7）==========
            target_positions_for_speed = dict(guided_target_points)
            # 🔥 V2：传递任务阶段给速度协调算法
            ...
            # 构造目标航向和速度字典
            target_headings = dict(guided_target_headings)
            target_speeds = {}
            ...
            if opening_trace_active:
                resp_meta = responsibility_guidance_meta.get(agent_id, {})
                resp_bearing = resp_meta.get('bearing', None)
                resp_effective_bearing = resp_meta.get('effective_bearing', None)
                resp_weight = resp_meta.get('weight', None)
                enemy_dx = float(enemy_center[0] - x)
                enemy_dy = float(enemy_center[1] - y)
                direct_enemy_hdg = float(np.degrees(np.arctan2(enemy_dx, enemy_dy)) % 360.0)
                log.warning(
                    "[OPENING_TRACE] step=%d t=%.1fs aid=%s cur=%.1f base=%.1f raw=%.1f smooth=%.1f "
                    "enemy_center=(%.1f,%.1f) self=(%.1f,%.1f) direct=%.1f resp=(%s/%s,w=%s) action=%s",
                    int(self.step_count),
                    float(current_time),
                    agent_id,
                    float(current_hdg),
                    float(my_guidance.target_heading),
                    float(target_hdg_raw),
                    float(target_hdg),
                    float(enemy_center[0]),
                    float(enemy_center[1]),
                    float(x),
                    float(y),
                    float(direct_enemy_hdg),
                    "-" if resp_bearing is None else f"{float(resp_bearing):.1f}",
                    "-" if resp_effective_bearing is None else f"{float(resp_effective_bearing):.1f}",
                    "-" if resp_weight is None else f"{float(resp_weight):.2f}",
                    str(action_taken),
                )
        ...
        opening_turn_deadband = 2.0
        if (
            self.cap_state_machine.state == CAPState.INTERCEPT
            and has_awacs_info
            and int(self.step_count) <= 12
        ):
            opening_turn_deadband = 1.0

        if abs(diff) < opening_turn_deadband:
            hdg_cmd = 8  # 保持当前航向（norm_hdg[8]=0）
        else:
            opening_small_turn_bias = (
                self.cap_state_machine.state == CAPState.INTERCEPT
                and has_awacs_info
                and int(self.step_count) <= 12
                and 2.0 <= abs(diff) < 12.0
            )
            if opening_small_turn_bias:
                hdg_cmd = 9 if diff > 0.0 else 7
            else:
                hdg_cmd = int(np.argmin(np.abs(self.norm_hdg - diff_rad)))
        if opening_trace_active:
            log.warning(
                "[OPENING_TRACE_CMD] step=%d t=%.1fs aid=%s target=%.1f diff=%.1f hdg_cmd=%d spd_cmd=%d",
                int(self.step_count),
                float(current_time),
                agent_id,
                float(target_hdg),
    ...
    def _get_engage_action(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        pair_guard_blocked = False
        pair_guard_blocked_targets = set()

        def _accept_target(candidate):
            nonlocal pair_guard_blocked
            if candidate is None:
                return None
            target_id = getattr(candidate, "track_id", None)
            if self._is_agent_target_pair_compatible(
                agent_id,
                target_id,
                env=env,
                current_time=current_time,
                ...
                return candidate
            pair_guard_blocked = True
            if target_id:
                pair_guard_blocked_targets.add(str(target_id))
            if self.step_count % 25 == 0:
                log.info(
                    "[PAIR_TARGET_GUARD] t=%.0fs %s skip=%s reason=cross_pair_guard",
                    current_time,
                    agent_id,
                    target_id or "-",
                )
        ...
        if assign and getattr(assign, 'target_id', None):
            for threat in self._get_picture_threats(current_time=current_time, purpose="decision"):
                if threat.track_id == assign.target_id:
                    target_threat = _accept_target(threat)
                    break

        if target_threat is None:
            target_threat = _accept_target(self._get_nearest_visible_pair_threat(agent_id, x, y))
        ...
        if target_threat is None:
            target_threat = _accept_target(
                self._get_picture_nearest_threat(
                    np.array([x, y, 8.0]),
                    current_time=current_time,
                    purpose="decision",
                )
        ...
        if defensive_engagement_suppressed:
            release_candidate = None
            release_distance = float("inf")
            try:
                release_threats = self._get_picture_threats(current_time=current_time, purpose="decision")
            except Exception:
                release_threats = []
            for candidate in release_threats:
                candidate = _accept_target(candidate)
                if candidate is None:
                    continue
                release_state = self._evaluate_cap_defensive_fire_window(
                    agent_id,
                    getattr(candidate, "track_id", None),
                    env=env,
                    current_time=current_time,
                )
                if not bool(release_state.get("eligible", False)):
                    continue
                candidate_distance = float(np.sqrt((candidate.x - x) ** 2 + (candidate.y - y) ** 2))
                if candidate_distance < release_distance:
                    release_candidate = candidate
                    release_distance = candidate_distance
            if release_candidate is not None:
                defensive_engagement_suppressed = False
                suppressed_until_step = -1
                if isinstance(suppress_store, dict):
                    suppress_store.pop(agent_id, None)
                target_threat = release_candidate
                if self.step_count % 25 == 0:
                    log.info(
                        "[CAP_ENGAGE_RELEASE] t=%.0fs %s target=%s dist=%.1fkm",
                        current_time,
                        agent_id,
                        getattr(release_candidate, "track_id", "-"),
                        release_distance,
                    )

        maneuver_action = None
        maneuver_intents: List[ManeuverIntent] = []
        ...
        friendly_rootcause_return = bool(
            agent_id.startswith('A')
            and self._is_friendly_rootcause_return(agent_id)
        )
        block_bridge_tactical_turn = (
            agent_id.startswith('A')
            and alive_enemy_exists
            and bridge_tactic_name == 'TACTICAL_TURN'
            and bool(self._should_cap_abort_tactical_turn(env, agent_id, current_time=current_time))
        )
        block_bridge_motion = bool(
            agent_id.startswith('A')
            and (defensive_engagement_suppressed or pair_guard_blocked or friendly_rootcause_return)
        )
        if assign and (assign.shooter_id == agent_id or assign.support_id == agent_id):
            try:
                ...
                elif block_bridge_motion:
                    if self.step_count % 25 == 0:
                        log.info(
                            "[CAP_ENGAGE_OWNER_REDIRECT] t=%.0fs %s tactic=%s phase=%s suppress=%s pair_guard=%s rootcause_return=%s targets=%s",
                            current_time,
                            agent_id,
                            getattr(assign.tactic, "value", str(assign.tactic)),
                            self._get_bridge_phase_name(agent_id) or "-",
                            "Y" if defensive_engagement_suppressed else "N",
                            "Y" if pair_guard_blocked else "N",
                            "Y" if friendly_rootcause_return else "N",
                            "/".join(sorted(pair_guard_blocked_targets)) or "-",
                        )
        ...
        if target_threat is None:
            preserve_target = self._get_picture_nearest_threat(
                np.array([x, y, 8.0]),
                current_time=current_time,
                purpose="decision",
            )
            preserve_distance = None
            if preserve_target is not None:
                try:
                    preserve_distance = float(np.sqrt((preserve_target.x - x) ** 2 + (preserve_target.y - y) ** 2))
                except Exception:
                    preserve_distance = None
            if agent_id.startswith('A') and (defensive_engagement_suppressed or pair_guard_blocked):
                if self.step_count % 25 == 0:
                    log.info(
                        "[CAP_ENGAGE_OWNER_REDIRECT] t=%.0fs %s reason=no_legal_target suppress=%s pair_guard=%s targets=%s preserve_target=%s preserve_dist=%.1fkm",
                        current_time,
                        agent_id,
                        "Y" if defensive_engagement_suppressed else "N",
                        "Y" if pair_guard_blocked else "N",
                        "/".join(sorted(pair_guard_blocked_targets)) or "-",
                        getattr(preserve_target, "track_id", "-") if preserve_target is not None else "-",
                        float(preserve_distance) if preserve_distance is not None and np.isfinite(preserve_distance) else float('nan'),
                    )
                maneuver_intents.append(
                    self._execute_defensive_preserve_action(
                        env,
                        agent_id,
                        target=preserve_target,
                        distance=preserve_distance,
                        reason=(
                            "cap_engage_redirect:"
                            + ("suppressed|" if defensive_engagement_suppressed else "")
                            + ("pair_guard|" if pair_guard_blocked else "")
                            + "no_legal_target"
                        ).strip("|"),
                    )
                )
            if maneuver_action is not None:
                maneuver_intents.append(
                    self._build_raw_command_intent(
                        agent_id=agent_id,
                        source="bridge_or_template",
                        producer="_get_engage_action",
                        priority=40,
                        raw_command=tuple(int(x) for x in maneuver_action),
                        min_dwell_s=4.0,
                        metadata={"reason": "target_missing"},
                    )
                )
            if maneuver_intents:
                return self._resolve_maneuver_intent_command(
                    env,
                    agent_id,
                    maneuver_intents,
                    current_time=current_time,
        ...
        if agent_id.startswith('A'):
            hard_bvr_floor_km = float(self._get_cap_bvr_preempt_floor_km(agent_id))
            if np.isfinite(distance) and float(distance) <= hard_bvr_floor_km:
                if self.step_count % 25 == 0:
                    log.info(
                        "[CAP_ENGAGE_OWNER_REDIRECT] t=%.0fs %s reason=hard_bvr_floor target=%s dist=%.1fkm",
                        current_time,
                        agent_id,
                        getattr(target_threat, "track_id", "-"),
                        float(distance),
                    )
                maneuver_intents.append(
                    self._execute_defensive_preserve_action(
                        ...
                        reason=f"cap_engage_redirect:hard_bvr_floor:{float(distance):.1f}km",
                    )
                )
        ...
        if candidate_threats and not defensive_engagement_suppressed:
            ...
            for candidate in candidate_threats:
                ...
                candidate_distance = float(np.sqrt((candidate.x - x) ** 2 + (candidate.y - y) ** 2))
                current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
                candidate_is_second_attack = current_ranges.is_second_attack_zone(candidate_distance)
                candidate_zone = "HIGH" if 0.0 <= float(getattr(candidate, "y", 300.0)) < 100.0 else (
                    "MEDIUM" if 100.0 <= float(getattr(candidate, "y", 300.0)) < 200.0 else "LOW"
                )
                gate_profile = self._get_prelaunch_gate_profile(
                    candidate_id,
                    current_time,
                    shooter_id=agent_id,
                    distance_km=float(candidate_distance),
                    is_second_attack=bool(candidate_is_second_attack),
                    target_zone=candidate_zone,
                )
                stable_ready = self._has_stable_tracking_ready(
                    candidate_id,
                    required_seconds=float(gate_profile.get("required_seconds", self._stable_tracking_window_s)),
                )
                current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
                first_window = (float(getattr(current_ranges, "TR", DEFAULT_RANGES.TR)) - 7.0) <= candidate_distance <= (float(getattr(current_ranges, "LR", DEFAULT_RANGES.LR)) + 4.0)
                second_window = float(getattr(current_ranges, "TR_PRIME", DEFAULT_RANGES.TR_PRIME)) <= candidate_distance <= (float(getattr(current_ranges, "LR_PRIME", DEFAULT_RANGES.LR_PRIME)) + 10.0)
                ...
                reference_distance = 78.0 if first_window else 43.0
                ...
                if score > best_score:
                    ...
                    best_score = score
            current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
            current_in_window = (float(getattr(current_ranges, "TR", DEFAULT_RANGES.TR)) - 4.0) <= launch_distance <= (float(getattr(current_ranges, "LR", DEFAULT_RANGES.LR)) + 4.0)
            best_in_window = (float(getattr(current_ranges, "TR", DEFAULT_RANGES.TR)) - 7.0) <= best_distance <= (float(getattr(current_ranges, "LR", DEFAULT_RANGES.LR)) + 4.0)
            if (
        ...
        current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
        is_second_attack = current_ranges.is_second_attack_zone(launch_distance)
        ...
        if is_second_attack and not defensive_engagement_suppressed:
            ...
            if window_key not in self._second_attack_window_announced:
                ...
                log.info(
                    ...
                    f"距离{launch_distance:.1f}km | MTR'={float(getattr(getattr(self, 'ranges', DEFAULT_RANGES), 'MTR_PRIME', DEFAULT_RANGES.MTR_PRIME)):.0f} "
                    f"LR'={float(getattr(getattr(self, 'ranges', DEFAULT_RANGES), 'LR_PRIME', DEFAULT_RANGES.LR_PRIME)):.0f} "
                    f"TR'={float(getattr(getattr(self, 'ranges', DEFAULT_RANGES), 'TR_PRIME', DEFAULT_RANGES.TR_PRIME)):.0f}"
                )
        ...
        # 导弹发射检查（要求发射前稳定双机跟踪）
        launch_gate_profile = self._get_prelaunch_gate_profile(
            launch_target_threat.track_id,
            current_time,
            shooter_id=agent_id,
            distance_km=float(launch_distance),
            is_second_attack=bool(is_second_attack),
            target_zone="HIGH" if 0.0 <= float(getattr(launch_target_threat, "y", 300.0)) < 100.0 else (
                "MEDIUM" if 100.0 <= float(getattr(launch_target_threat, "y", 300.0)) < 200.0 else "LOW"
            ),
        )
        required_stable_seconds = float(launch_gate_profile.get("required_seconds", self._stable_tracking_window_s))
        can_launch = self._has_stable_tracking_ready(
            launch_target_threat.track_id,
            required_seconds=required_stable_seconds,
        )
        gate_mode = "stable_track"
        ...
        if can_shoot:
            should_launch_req = self.missile_adapter.should_launch(
                ...
                launch_target_threat.track_id,
                launch_distance,
                (is_second_attack or semantic_second_attack or adaptive_attack),
                current_time,
                env,
            )

        friendly_close_block = self._get_friendly_close_block_context(
            env,
            agent_id,
            launch_target_id=getattr(launch_target_threat, "track_id", ""),
            launch_distance_km=float(launch_distance),
        )
        friendly_close_block_km = float(friendly_close_block.get("floor_km", float("nan")))
        close_block_distance_km = float(friendly_close_block.get("distance_km", float("nan")))
        close_block_target_id = str(friendly_close_block.get("target_id", "") or "")
        close_block_source = str(friendly_close_block.get("source", "") or "launch_target")
        if (
            agent_id.startswith('A')
            and can_launch
            and np.isfinite(close_block_distance_km)
            and close_block_distance_km <= friendly_close_block_km
        ):
            can_launch = False
            launch_block_reason = "friendly_safe"
            if self.step_count % 25 == 0:
                log.info(
                    "[FRIENDLY_CLOSE_SHOT_BLOCK] shooter=%s launch_target=%s launch_dist=%.1fkm block_target=%s block_dist=%.1fkm floor=%.1fkm source=%s",
                    agent_id,
                    getattr(launch_target_threat, "track_id", "-"),
                    float(launch_distance),
                    close_block_target_id or "-",
                    close_block_distance_km,
                    friendly_close_block_km,
                    close_block_source,
                )
        ...
        if (
            agent_id.startswith('A')
            and can_launch
            and not is_second_attack
            and launch_distance > 86.0
        ):
            can_launch = False
            launch_block_reason = "long_shot"
            if self.step_count % 25 == 0:
                log.info(
                    "[FRIENDLY_LONG_SHOT_BLOCK] shooter=%s target=%s dist=%.1fkm",
                    agent_id,
                    getattr(launch_target_threat, "track_id", "-"),
                    launch_distance,
                )

        if can_shoot and should_launch_req and not can_launch and launch_block_reason is None:
            launch_block_reason = "stable_not_ready"
        ...
        if can_shoot and should_launch_req and can_launch:
            ...
            if missile_id:
                _capgh._record_prelaunch_gate_event(
                    self,
                    shooter_id=str(agent_id),
                    target_id=str(launch_target_threat.track_id),
                ...
                _capgh._record_prelaunch_gate_event(
                    ...
                    event_type="block",
                    stable_ready=True,
                    reason="launch_exec_fail",
                )
                if self.step_count % 50 == 0:
                    log.info(
                        "[PRELAUNCH_GATE_BLOCK] shooter=%s target=%s stable=%.1fs mode=%s reason=launch_exec_fail",
                        agent_id,
                        launch_target_threat.track_id,
                        hold,
                        gate_mode,
            ...
            _capgh._record_prelaunch_gate_event(
                ...
                current_time=float(current_time),
                event_type="block",
                stable_ready=(str(launch_block_reason or "") != "stable_not_ready"),
                reason=str(launch_block_reason or 'blocked'),
            )
            ...
            if isinstance(block_by_reason, dict):
                reason_key = str(launch_block_reason or 'blocked')
                block_by_reason[reason_key] = int(block_by_reason.get(reason_key, 0)) + 1
            if self.step_count % 50 == 0:
                hold = self._stable_tracking_elapsed.get(launch_target_threat.track_id, 0.0)
                trackers = self._stable_tracking_trackers.get(launch_target_threat.track_id, [])
                if launch_block_reason is not None:
                    log.info(
                        f"[发射门禁阻断] shooter={agent_id} target={launch_target_threat.track_id} "
                        f"stable={hold:.1f}s trackers={trackers} mode={gate_mode} reason={launch_block_reason}"
                    )
                else:
                    log.info(f"⏳ [{agent_id}] 发射保持: {launch_target_threat.track_id} 稳定跟踪{hold:.1f}s/{required_stable_seconds:.1f}s")
                    log.info(
                        f"[发射门禁阻断] shooter={agent_id} target={launch_target_threat.track_id} "
        ...
        if maneuver_action is not None:
            if agent_id.startswith('A') and (defensive_engagement_suppressed or pair_guard_blocked):
                if self.step_count % 25 == 0:
                    log.info(
                        "[CAP_ENGAGE_OWNER_REDIRECT] t=%.0fs %s reason=bridge_motion_blocked suppress=%s pair_guard=%s targets=%s preserve_target=%s preserve_dist=%.1fkm",
                        current_time,
                        agent_id,
                        "Y" if defensive_engagement_suppressed else "N",
                        "Y" if pair_guard_blocked else "N",
                        "/".join(sorted(pair_guard_blocked_targets)) or "-",
                        getattr(target_threat, "track_id", "-") if target_threat is not None else "-",
                        float(distance) if distance is not None and np.isfinite(distance) else float('nan'),
                    )
                maneuver_intents.append(
                    self._execute_defensive_preserve_action(
                        env,
                        agent_id,
                        target=target_threat,
                        distance=float(distance) if distance is not None and np.isfinite(distance) else None,
                        reason=(
                            "cap_engage_redirect:"
                            + ("suppressed|" if defensive_engagement_suppressed else "")
                            + ("pair_guard|" if pair_guard_blocked else "")
                ...
                maneuver_intents.append(
                    self._build_raw_command_intent(
                        ...
                        min_dwell_s=4.0,
                        metadata={
                            "tactic": getattr(assign.tactic, "value", str(getattr(assign, "tactic", ""))) if assign is not None else "",
                            "target_id": getattr(target_threat, "track_id", "") if target_threat is not None else "",
                        },
                    )
                )

        if agent_id.startswith('A') and defensive_engagement_suppressed:
            if self.step_count % 25 == 0:
                log.info(
                    "[CAP_ENGAGE_OWNER_REDIRECT] t=%.0fs %s reason=direct_motion_blocked target=%s dist=%.1fkm",
                    current_time,
                    agent_id,
                    getattr(target_threat, "track_id", "-"),
                    float(distance) if distance is not None and np.isfinite(distance) else float('nan'),
            ...
            maneuver_intents.append(
                self._execute_defensive_preserve_action(
                    ...
                    target=target_threat,
                    distance=float(distance) if distance is not None and np.isfinite(distance) else None,
                    reason="cap_engage_redirect:suppressed|direct_motion_blocked",
                )
            )
        ...
        target_hdg = np.degrees(np.arctan2(dx, dy)) % 360
        maneuver_intents.append(
            self._build_heading_intent(
                agent_id=agent_id,
                source="direct_engage",
                producer="_get_engage_action",
                priority=30,
                desired_heading_deg=target_hdg,
                min_dwell_s=2.0,
                heading_deadband_deg=6.0,
                speed_floor_cmd=3,
                alt_floor_cmd=7,
                metadata={
                    "target_id": getattr(target_threat, "track_id", "") if target_threat is not None else "",
                    "launch_target_id": getattr(launch_target_threat, "track_id", "") if launch_target_threat is not None else "",
                },
            )
        )
        return self._resolve_maneuver_intent_command(
    ...
    def _execute_drag_shoot(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
        active_ranges = getattr(self, "_dynamic_control_ranges", None) or getattr(self, "_locked_compression", None)
        node = current_ranges.get_current_node(distance, active_ranges)
        ...
        if node in ('BEYOND_NLT', 'NLT_MELD'):
            ...
            second_node = self._get_second_attack_subnode(distance, active_ranges)
        ...
        self._store_template_action_plan(
            agent_id,
            tactic="drag_shoot",
            node=node,
            target_id=getattr(target_threat, "track_id", ""),
            distance_km=float(distance),
            base_heading_deg=float(base_hdg),
            target_heading_deg=float(target_hdg),
            alt_cmd=int(alt_cmd),
            hdg_cmd=int(hdg_cmd),
            spd_cmd=int(spd_cmd),
            reason=f"is_shooter={'Y' if is_shooter else 'N'} second_node={self._get_second_attack_subnode(distance, active_ranges) or '-'}",
            extra={
                "current_heading_deg": float(current_hdg),
                "heading_delta_deg": float(diff),
                "is_shooter": bool(is_shooter),
        ...
        self._store_template_action_plan(
            ...
            hdg_cmd=int(hdg_cmd),
            spd_cmd=int(spd_cmd),
            reason=f"role={'lead' if is_lead else 'wing'} node={node}",
            extra={
                "current_heading_deg": float(current_hdg),
                "heading_delta_deg": float(diff),
    ...
    def _execute_pincer_attack(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        # 避免固定角度在不同几何关系下过度/不足。
        current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
        far_ratio = float(np.clip((distance - float(getattr(current_ranges, "MTR", DEFAULT_RANGES.MTR))) / max(float(getattr(current_ranges, "NLT", DEFAULT_RANGES.NLT)) - float(getattr(current_ranges, "MTR", DEFAULT_RANGES.MTR)), 1e-6), 0.0, 1.0))
        near_ratio = float(np.clip((float(getattr(current_ranges, "DOR", DEFAULT_RANGES.DOR)) - distance) / max(float(getattr(current_ranges, "DOR", DEFAULT_RANGES.DOR)) - float(getattr(current_ranges, "TR", DEFAULT_RANGES.TR)), 1e-6), 0.0, 1.0))
        ...
        if abs(diff) < 5:
            self._store_template_action_plan(
                ...
                hdg_cmd=8,
                spd_cmd=int(spd_cmd),
                reason=f"expand_offset={expand_offset:.1f} egress_offset={egress_offset:.1f} recompress_offset={recompress_offset:.1f}",
                extra={
                    "current_heading_deg": float(current_hdg),
                    "heading_delta_deg": float(diff),
                    "side_sign": float(side_sign),
    ...
    def _execute_front_back(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        self._store_template_action_plan(
            ...
            hdg_cmd=int(hdg_cmd),
            spd_cmd=int(spd_cmd),
            reason=f"role={'lead' if is_lead else 'trail'} node={node}",
            extra={
                "current_heading_deg": float(current_hdg),
                "heading_delta_deg": float(diff),
                "is_lead": bool(is_lead),
    ...
    def _execute_high_low_attack(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        self._store_template_action_plan(
            ...
            target_id=getattr(target_threat, "track_id", ""),
            distance_km=float(distance),
            base_heading_deg=float(target_hdg),
            target_heading_deg=float(target_hdg),
            alt_cmd=int(alt_cmd),
            hdg_cmd=int(hdg_cmd),
            spd_cmd=int(spd_cmd),
            reason=f"role={'lead_level' if is_lead else 'wing_high_cover'} node={node}",
            extra={
                "current_heading_deg": float(current_hdg),
                "heading_delta_deg": float(diff),
                "is_lead": bool(is_lead),
    ...
    def _execute_tactical_turn(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        if agent_id.startswith('A'):
            ...
            if self._should_cap_abort_tactical_turn(
                env,
                agent_id,
                threat=threat,
                current_time=self.step_count * 0.2,
        ...
        self._store_template_action_plan(
            ...
            node="TURN_COLD",
            target_id=getattr(threat, "track_id", "") if 'threat' in locals() and threat is not None else "",
            distance_km=float("nan"),
            base_heading_deg=float(current_hdg),
            target_heading_deg=float(target_hdg),
            alt_cmd=7,
            hdg_cmd=int(hdg_cmd),
            spd_cmd=3,
            reason="forced_turn_cold_180deg",
            extra={
                "current_heading_deg": float(current_hdg),
                "heading_delta_deg": float(diff),
            },
    ...
    def _get_evade_action(self, env, agent_id: str) -> Tuple[int, int, int]:
        ...
        # 根据距离选择规避方式
        current_ranges = getattr(self, "ranges", DEFAULT_RANGES)
        if distance < float(getattr(current_ranges, "MAR", DEFAULT_RANGES.MAR)):
            # 极近距：俯冲脱离
            return self._execute_dive_escape(env, agent_id)
        elif distance < float(getattr(current_ranges, "DR", DEFAULT_RANGES.DR)):
            # 近距：Notch机动
    ...
    def _execute_beam_maneuver(self, env, agent_id: str, threat) -> Tuple[int, int, int]:
        ...
        if abs(diff) < 10:
            selected_hdg = float((current_hdg + diff) % 360.0)
            self._store_template_action_plan(
                agent_id,
                tactic="evade_beam",
                node="BEAM",
                target_id=getattr(threat, "track_id", ""),
                distance_km=float(np.sqrt((threat.x - x) ** 2 + (threat.y - y) ** 2)),
                base_heading_deg=float(threat_bearing),
                target_heading_deg=selected_hdg,
                alt_cmd=7,
                hdg_cmd=8,
                spd_cmd=3,
                reason="beam_perpendicular_to_threat",
                extra={
                    "current_heading_deg": float(current_hdg),
                    "heading_delta_deg": float(diff),
                    "threat_bearing_deg": float(threat_bearing),
                },
            )
            return 7, 8, 3
    ...
    def _execute_notch_maneuver(self, env, agent_id: str, threat) -> Tuple[int, int, int]:
        ...
        if abs(diff) < 10:
            ...
            self._store_template_action_plan(
                ...
                hdg_cmd=8,
                spd_cmd=3,
                reason="notch_doppler_blind_zone",
                extra={
                    "current_heading_deg": float(current_hdg),
                    "heading_delta_deg": float(diff),
                    "threat_bearing_deg": float(threat_bearing),
                    "notch_angle_deg": float(notch_angle),
                },
            )
            return 7, 8, 3
