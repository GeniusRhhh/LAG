...
class TacticSelector:
    ...
    def select_tactic(
        ...
        # ─── 🔧 调试用：强制指定战术（取消注释下面一行即可） ───
        self._force_tactic = TacticType.T_FB  # 可选: T_DS / T_PA / T_HL / T_FB / T_SBS
        if hasattr(self, '_force_tactic'):
            threats = threats if threats is not None else picture.get_all_threats()
            center = self._get_formation_center(friendly_positions)
            nearest = self._get_nearest_threat(threats, center) if threats else None
            if nearest:
                shooter, support = self._select_best_shooter(nearest, friendly_positions)
                return TacticAssignment(tactic=self._force_tactic, target_id=nearest.track_id,
                                        shooter_id=shooter, support_id=support, priority=2)
        # ─── 正常战术选择逻辑 ───
        threats = threats if threats is not None else picture.get_all_threats()
        if not threats:
            return None
