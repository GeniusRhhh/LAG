...
class StateContext:
    ...
    radar_threat_level: float = 0.0
    fuel_critical: bool = False
    mission_time: float = 0.0
    hostile_alive_count: int = 0
    nearest_target_id: str = ""
    nearest_target_zone: str = ""
    nearest_target_speed_mps: float = 0.0
    nearest_target_heading_deg: float = 0.0
    nearest_altitude_delta_km: float = 0.0
    closure_rate_mps: float = 0.0
    time_to_merge_s: float = float("inf")
...
class CAPStateMachine:
    ...
    def _should_evade(self, ctx: StateContext) -> bool:
        if ctx.is_missile_incoming:
            return True
        if (
            bool(getattr(ctx, "ecm_active", False))
            and int(getattr(ctx, "rwr_level", 0) or 0) >= 5
            and float(getattr(ctx, "radar_threat_level", 0.0) or 0.0) >= 1.0
            and ctx.min_threat_distance < self.ranges.MAR
        ):
            return True
