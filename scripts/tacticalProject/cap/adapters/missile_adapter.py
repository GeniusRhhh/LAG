...
class MissileAdapter:
    ...
    def _get_coop_prelaunch_state(self, env, target_id: str, current_time: float) -> tuple[bool, dict]:
        owner_task = getattr(env, "task", None)
        if owner_task is None or not hasattr(owner_task, "check_external_prelaunch_gate"):
            return False, {}
        try:
            passed, details = owner_task.check_external_prelaunch_gate(target_id, current_time)
    ...
    def should_launch(self, agent_id: str, target_id: str, distance: float,
        ...
        if env is not None:
            ...
            coop_prelaunch_ready, _ = self._get_coop_prelaunch_state(env, target_id, current_time)
        ...
        if env is not None:
            ...
            if is_friendly and (not priority_target) and (not is_second_attack):
                if alive_enemy_count >= 3 and shooter_inventory <= 3:
                    return False
                if shooter_inventory <= 2 and distance > 38.0:
                    return False
                if team_inventory <= (alive_enemy_count + 3) and distance > 42.0:
                    return False
                if effective_target_missiles >= 1 and distance > 46.0:
                    return False
            if is_friendly:
                reserve_floor = max(6, alive_enemy_count * 2 + 2)
                if team_inventory <= reserve_floor:
                    if distance > 52.0:
                        return False
                    if (not priority_target) and distance > 38.0:
                        return False
                if (
                    ...
                    return False
                if alive_enemy_count >= 3 and shooter_inventory <= 2 and not (priority_target and distance <= 60.0):
                    return False
                if alive_enemy_count >= 2 and shooter_inventory <= 1 and not (priority_target and distance <= 50.0):
                    return False
        ...
        if is_second_attack:
            ...
            if is_friendly:
                ...
                if priority_target:
                    ...
                    high_km = 60.0
                if coop_prelaunch_ready:
                    low_km = max(26.0, low_km - (2.0 if priority_target else 1.0))
                    high_km += 4.0 if priority_target else 2.0
            else:
                low_km = 30.0
                high_km = 60.0
                if coop_prelaunch_ready:
                    low_km = 28.0
                    high_km = 66.0 if priority_target else 62.0
            ...
            if is_friendly:
                ...
                if target_risk_zone == "MEDIUM":
                    ...
                    high_km = 92.0
                if coop_prelaunch_ready:
                    low_km = max(70.0, low_km - (2.0 if priority_target else 1.0))
                    high_km += 4.0 if priority_target else 2.0
            else:
    ...
    def needs_relay(self, missile_id: str, guide_agent_alive: bool,
        ...
        relay_entry_km = max(mar_km + 8.0, 43.0)
        terminal_entry_km = max(mar_km + 12.0, 47.0)

        # 末制导保持照射，不再一进 TR 就立即疯狂请求接力
        if status.state == MissileState.TERMINAL:
            if status.distance_to_target <= 18.0:
                return True
            return guide_distance_to_target <= terminal_entry_km

        if status.distance_to_target <= 20.0:
            return True

        if status.distance_to_target <= 35.0 and guide_distance_to_target <= relay_entry_km:
            return True

        if status.time_of_flight >= 40.0 and guide_distance_to_target <= max(relay_entry_km + 6.0, 50.0):
            return True
