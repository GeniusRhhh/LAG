...
class EnemyWaveState:
    ...
    turn_entry_distance_km: float = 40.0
    resume_distance_km: float = 72.0
    ...
    regroup_timeout_s: float = 10.0
    regroup_min_hold_s: float = 6.0
    turn_south_min_hold_s: float = 5.0
    north_depart_margin_km: float = 12.0
    hard_escape_until: float = 0.0
    hard_escape_release_km: float = 0.0
    hard_escape_regroup_y_km: float = 0.0
    pressure_level: int = 0
    pressure_tag: str = "balanced"
    enemy_alive_total: int = 4
    friendly_alive_total: int = 4
    local_advantage: int = 0
    hard_standoff_distance_km: float = 40.0
    ...
    winchester_occupation: bool = False
...
class EnemyAIAdapter:
    ...
    def __init__(self):
        ...
        self._turnback_distance_km = float(os.getenv("ENEMY_TURNBACK_DISTANCE_KM", "40"))
        self._hard_standoff_distance_km = float(os.getenv("ENEMY_HARD_STANDOFF_DISTANCE_KM", "40"))
        self._resume_distance_km = float(os.getenv("ENEMY_REATTACK_DISTANCE_KM", "72"))
        self._pullback_distance_km = float(os.getenv("ENEMY_PULLBACK_DISTANCE_KM", "28"))
        self._regroup_timeout_s = float(os.getenv("ENEMY_REGROUP_TIMEOUT_S", "10"))
        self._regroup_min_hold_s = float(os.getenv("ENEMY_REGROUP_MIN_HOLD_S", "6"))
        self._turn_phase_s = float(os.getenv("ENEMY_TURN_PHASE_S", "5"))
        self._reattack_hold_s = float(os.getenv("ENEMY_REATTACK_HOLD_S", "2"))
        self._turn_south_min_hold_s = float(os.getenv("ENEMY_TURN_SOUTH_MIN_HOLD_S", "5"))
        self._regroup_depart_margin_km = float(os.getenv("ENEMY_REGROUP_DEPART_MARGIN_KM", "12"))
        self._regroup_release_distance_km = float(os.getenv("ENEMY_REGROUP_RELEASE_DISTANCE_KM", "72"))
        self._turn_south_depart_margin_km = float(os.getenv("ENEMY_TURN_SOUTH_DEPART_MARGIN_KM", "4"))
    ...
    def get_enemy_action(
        ...
        if float(state.hard_standoff_distance_km or 0.0) < 40.0:
            state.hard_standoff_distance_km = 40.0
        if float(state.turn_entry_distance_km or 0.0) < 46.0:
            state.turn_entry_distance_km = 46.0
        if float(state.resume_distance_km or 0.0) < 68.0:
            state.resume_distance_km = 68.0
        """
    ...
    def _get_legacy_enemy_command(
        ...
        cmd = self._get_raw_combat_ai_command(env, agent_id, current_time, task)
    ...
    def _get_raw_combat_ai_command(
        self,
        env,
        agent_id: str,
        current_time: float,
        ...
        try:
            ...
            return self._default_action(env, agent_id)
        return self._clip_command(tuple(int(x) for x in cmd))
    ...
    def get_group_snapshot(self, agent_id: str) -> Dict[str, object]:
        ...
        return {
            ...
            "pressure_level": int(state.pressure_level) if state is not None else 0,
            ...
            "forward_limit_y_km": float(state.forward_limit_y_km) if state is not None else float("nan"),
            "hard_standoff_distance_km": float(state.hard_standoff_distance_km) if state is not None else float("nan"),
            "hard_escape_until": float(state.hard_escape_until) if state is not None else 0.0,
            "hard_escape_release_km": float(state.hard_escape_release_km) if state is not None else 0.0,
            "hard_escape_regroup_y_km": float(state.hard_escape_regroup_y_km) if state is not None else 0.0,
            "local_advantage": int(state.local_advantage) if state is not None else 0,
            "missile_superiority": bool(state.missile_superiority) if state is not None else False,
            "finish_exploit": bool(state.finish_exploit) if state is not None else False,
            "winchester_occupation": bool(state.winchester_occupation) if state is not None else False,
            "observed_time": float(getattr(self, "_last_seen_time", -1.0) or -1.0),
        }

    def get_action_annotation(self, agent_id: str) -> str:
        combat_ai = self._combat_ai
        if combat_ai is None or not hasattr(combat_ai, "get_action_annotation"):
            return "search"
        try:
            annotation = combat_ai.get_action_annotation(agent_id)
            return str(annotation or "search")
        except Exception:
            return "search"

    def get_action_annotation_for_csv(self, agent_id: str) -> Dict[str, str]:
        combat_ai = self._combat_ai
        if combat_ai is not None and hasattr(combat_ai, "get_action_annotation_for_csv"):
            try:
                annotation = combat_ai.get_action_annotation_for_csv(agent_id)
                if isinstance(annotation, dict):
                    return annotation
            except Exception:
                pass
        return {"Action_Intent": self.get_action_annotation(agent_id)}

    def get_action_type_for_csv(self, agent_id: str) -> Dict[str, str]:
        combat_ai = self._combat_ai
        if combat_ai is None or not hasattr(combat_ai, "get_action_type_for_csv"):
            return {"action_type": ""}
        try:
            action_type = combat_ai.get_action_type_for_csv(agent_id)
    ...
    def evaluate_north_phase_status(
        self,
        agent_id: str,
        target_y_km: float = float("nan"),
        target_zone: str = "",
        distance_km: float = float("nan"),
    ) -> Dict[str, object]:
        snapshot = self.get_group_snapshot(agent_id)
        phase = str(snapshot.get("phase", "UNKNOWN") or "UNKNOWN").upper()
        pressure_level = int(snapshot.get("pressure_level", 0) or 0)
        zone_name = str(target_zone or "UNKNOWN").upper()
        depth_km = float(target_y_km) if np.isfinite(target_y_km) else float("nan")
        range_km = float(distance_km) if np.isfinite(distance_km) else float("nan")
        forward_limit_y_km = float(snapshot.get("forward_limit_y_km", float("nan")))
        observed_time = float(snapshot.get("observed_time", -1.0) or -1.0)
        hard_escape_until = float(snapshot.get("hard_escape_until", 0.0) or 0.0)
        hard_escape_release_km = float(snapshot.get("hard_escape_release_km", 0.0) or 0.0)
        hard_escape_regroup_y_km = float(snapshot.get("hard_escape_regroup_y_km", 0.0) or 0.0)

        front_margin_km = 12.0 if pressure_level >= 2 else 9.0 if pressure_level >= 1 else 5.0
        medium_front_cap_km = 120.0 if pressure_level >= 2 else 112.0 if pressure_level >= 1 else 104.0
        medium_range_cap_km = 72.0 if pressure_level >= 2 else 60.0 if pressure_level >= 1 else 48.0

        near_forward_limit = bool(
            np.isfinite(depth_km)
            and np.isfinite(forward_limit_y_km)
            and depth_km <= (forward_limit_y_km + front_margin_km)
        )
        high_zone_override = zone_name == "HIGH"
        medium_front_override = bool(
            zone_name == "MEDIUM"
            and (
                near_forward_limit
                or (np.isfinite(depth_km) and depth_km <= medium_front_cap_km)
                or (pressure_level >= 2 and np.isfinite(range_km) and range_km <= medium_range_cap_km)
            )
        )
        hard_escape_active = bool(
            phase in {"TURN_NORTH", "REGROUP_NORTH"}
            and (
                (observed_time >= 0.0 and observed_time < hard_escape_until)
                or (
                    hard_escape_release_km > 0.0
                    and np.isfinite(range_km)
                    and range_km < (hard_escape_release_km - 1.0)
                )
                or (
                    hard_escape_regroup_y_km > 0.0
                    and np.isfinite(depth_km)
                    and depth_km < (hard_escape_regroup_y_km - 1.0)
                )
            )
        )
        pressing_override = bool((high_zone_override or medium_front_override) and not hard_escape_active)

        disengaged = False
        reason = "not_north_phase"
        if phase == "REGROUP_NORTH":
            if hard_escape_active:
                disengaged = True
                reason = "north_phase_hard_escape"
            else:
                disengaged = not pressing_override
                reason = "regroup_north" if disengaged else "regroup_north_press_override"
        elif phase == "TURN_NORTH":
            if hard_escape_active:
                disengaged = True
                reason = "north_phase_hard_escape"
            elif pressing_override:
                reason = "turn_north_press_override"
            else:
                disengaged = pressure_level < 2
                reason = "turn_north_low_pressure" if disengaged else "turn_north_pressing"
        ...
        result.update({
            ...
            "near_forward_limit": near_forward_limit,
            "hard_escape_active": hard_escape_active,
            "pressing_override": pressing_override,
            "disengaged": disengaged,
            "reason": reason,
        })
        return result

    def _get_validation_profile(self) -> Dict[str, Any]:
        task = getattr(self, "_active_task", None)
        profile = getattr(task, "enemy_validation_profile", None) if task is not None else None
        return profile if isinstance(profile, dict) else {}

    def _select_validation_schedule_overrides(
        self,
        schedule: Any,
        *,
        ...
        for item in schedule:
            if not isinstance(item, dict):
                continue
            try:
                min_distance_gate = item.get("min_distance_km")
                max_distance_gate = item.get("max_distance_km")
                min_time_gate = item.get("min_time_s")
                max_time_gate = item.get("max_time_s")
                phase_gate = item.get("phase")
                phases_gate = item.get("phases")
                wave_gate = item.get("wave_index")
                min_wave_gate = item.get("min_wave_index")
                max_wave_gate = item.get("max_wave_index")

                if min_distance_gate is not None and min_distance is not None and float(min_distance) < float(min_distance_gate):
                    continue
                if max_distance_gate is not None and min_distance is not None and float(min_distance) > float(max_distance_gate):
                    continue
                if min_time_gate is not None and current_time is not None and float(current_time) < float(min_time_gate):
                    continue
                if max_time_gate is not None and current_time is not None and float(current_time) > float(max_time_gate):
                    continue
                if phase_gate is not None and phase_text and str(phase_gate).upper() != phase_text:
                    continue
                if phases_gate is not None:
                    allowed = {
                        str(value).upper()
                        for value in phases_gate
                        if str(value or "").strip()
                    } if isinstance(phases_gate, (list, tuple, set)) else {str(phases_gate).upper()}
                    if phase_text and allowed and phase_text not in allowed:
                        continue
                if wave_gate is not None and wave_index is not None and int(wave_index) != int(wave_gate):
                    continue
                if min_wave_gate is not None and wave_index is not None and int(wave_index) < int(min_wave_gate):
                    continue
                if max_wave_gate is not None and wave_index is not None and int(wave_index) > int(max_wave_gate):
                    continue
            except Exception:
                continue
    ...
    def _get_validation_group_profile(
        self,
        context: Optional[EnemyFormationContext],
        *,
        current_time: Optional[float] = None,
        min_distance: Optional[float] = None,
        phase: Optional[str] = None,
        ...
        groups = profile.get("group_overrides", {})
        if not isinstance(groups, dict):
            return {}
        group_profile = groups.get(str(context.group_id), {})
        if not isinstance(group_profile, dict):
            return {}

        activation_distance = group_profile.get("activation_distance_km")
        if activation_distance is not None and min_distance is not None:
            try:
                if float(min_distance) > float(activation_distance):
                    pre = group_profile.get("pre_activation_overrides", {})
                    if isinstance(pre, dict):
                        merged = dict(group_profile)
                        merged.update(pre)
                        group_profile = merged
            except Exception:
                pass
        result = dict(group_profile)
        schedule_overrides = self._select_validation_schedule_overrides(
            result.get("schedule"),
        ...
        result.update(schedule_overrides)
        return result

    def _get_validation_group_profile_by_id(self, group_id: str) -> Dict[str, Any]:
        profile = self._get_validation_profile()
        groups = profile.get("group_overrides", {})
        if not isinstance(groups, dict):
            return {}
        group_profile = groups.get(str(group_id), {})
        return dict(group_profile) if isinstance(group_profile, dict) else {}

    def _apply_validation_profile_overrides(
        self,
        ...
        *,
        ...
        result = dict(profile)
        validation = self._get_validation_profile()
        state = self._group_states.get(context.group_id) if context is not None else None
        phase = str(state.phase) if state is not None else None
        wave_index = int(state.wave_index) if state is not None else None
        global_overrides = validation.get("global_overrides", {})
        if isinstance(global_overrides, dict):
            for key, value in global_overrides.items():
                result[key] = value
        global_schedule_overrides = self._select_validation_schedule_overrides(
            validation.get("global_schedule"),
            current_time=current_time,
            min_distance=min_distance,
            phase=phase,
            wave_index=wave_index,
        )
        for key, value in global_schedule_overrides.items():
            result[key] = value
        group_overrides = self._get_validation_group_profile(
            context,
            current_time=current_time,
            min_distance=min_distance,
            phase=phase,
        ...
        for key, value in group_overrides.items():
            if key in (
                ...
                "preferred_target",
                "slot_x_bias_km",
                "slot_x_bias_by_phase",
                "soft_scene_heading_pull_deg",
                "soft_scene_heading_pull_deg_by_phase",
                "allow_validation_descend",
                "schedule",
                "activation_distance_km",
                "pre_activation_overrides",
            ):
                continue
            result[key] = value
        try:
            hard = float(result.get("hard_standoff_distance_km", self._hard_standoff_distance_km))
        except Exception:
            hard = float(self._hard_standoff_distance_km)
        hard = max(40.0, hard)
        try:
            turn = float(result.get("turn_entry_distance_km", self._turnback_distance_km))
        except Exception:
            turn = float(self._turnback_distance_km)
        try:
            resume = float(result.get("resume_distance_km", self._resume_distance_km))
        except Exception:
            resume = float(self._resume_distance_km)
        result["hard_standoff_distance_km"] = hard
        result["turn_entry_distance_km"] = max(hard + 6.0, turn)
        result["resume_distance_km"] = max(hard + 28.0, resume)
        return result

    def _get_validation_slot_bias_km(
        self,
        context: Optional[EnemyFormationContext],
        phase: str,
        *,
        ...
        group_profile = self._get_validation_group_profile(
            context,
            current_time=current_time,
            min_distance=min_distance,
            phase=phase,
        ...
        phase_biases = group_profile.get("slot_x_bias_by_phase", {})
        if isinstance(phase_biases, dict):
            value = phase_biases.get(str(phase), phase_biases.get(str(phase).upper()))
            if value is not None:
                try:
                    ...
                    return 0.0
        try:
            return float(group_profile.get("slot_x_bias_km", 0.0) or 0.0)
        except Exception:
            return 0.0

    def _get_validation_altitude_target_m(
        self,
        agent_id: str,
        *,
        context: Optional[EnemyFormationContext],
        ...
        profile = self._get_validation_profile()
        schedule_map = profile.get("altitude_schedule", {})
        if isinstance(schedule_map, dict):
            schedule = schedule_map.get(agent_id)
            if isinstance(schedule, list):
                for item in schedule:
                    ...
                    min_time_gate = item.get("min_time_s")
                    max_time_gate = item.get("max_time_s")
                    if min_distance_gate is not None and float(min_distance) < float(min_distance_gate):
                        continue
                    if max_distance_gate is not None and float(min_distance) > float(max_distance_gate):
                        continue
                    if phase_gate is not None and str(phase_gate).upper() != str(phase).upper():
                        continue
                    if min_time_gate is not None and float(current_time) < float(min_time_gate):
                        continue
                    if max_time_gate is not None and float(current_time) > float(max_time_gate):
                        continue
                    try:
                        return float(item.get("target_altitude_m"))
                    except Exception:
                        continue
        target_map = profile.get("altitude_targets_m", {})
        if isinstance(target_map, dict) and agent_id in target_map:
            try:
                return float(target_map[agent_id])
            except Exception:
                return None
        return None

    def _altitude_cmd_to_target(self, current_altitude_m: float, target_altitude_m: float) -> int:
        diff = float(target_altitude_m) - float(current_altitude_m)
        if diff >= 2200.0:
            return 12
        if diff >= 1200.0:
            return 11
    ...
    def _get_validation_scene_pull_deg(
        self,
        *,
        context: Optional[EnemyFormationContext],
        phase: str,
        ...
        for source in (group_profile, profile):
            if not isinstance(source, dict):
                continue
            phase_map = source.get("soft_scene_heading_pull_deg_by_phase", {})
            if isinstance(phase_map, dict):
                value = phase_map.get(phase_name, phase_map.get(str(phase)))
                if value is not None:
                    try:
                        return max(0.0, float(value))
                    except Exception:
                        return 0.0
            if "soft_scene_heading_pull_deg" in source:
                try:
                    return max(0.0, float(source.get("soft_scene_heading_pull_deg", 0.0) or 0.0))
                except Exception:
                    return 0.0
        return 18.0

    def _blend_advisory_scene_heading_cmd(
        self,
        current_heading: float,
        advisory_hdg_cmd: int,
        scene_target_heading: float,
        *,
        max_scene_pull_deg: float,
    ) -> int:
        max_scene_pull_deg = max(0.0, float(max_scene_pull_deg))
        if max_scene_pull_deg <= 0.0:
            return int(np.clip(advisory_hdg_cmd, 0, len(self.norm_hdg) - 1))
        advisory_target_heading = self._cmd_to_target_heading(current_heading, advisory_hdg_cmd)
        scene_error = self._normalize_heading_error(scene_target_heading - advisory_target_heading)
        blended_target_heading = (advisory_target_heading + float(np.clip(scene_error, -max_scene_pull_deg, max_scene_pull_deg))) % 360.0
        return self._hdg_to_cmd(current_heading, blended_target_heading)

    def _get_validation_altitude_command(
        self,
        agent_id: str,
        *,
        context: Optional[EnemyFormationContext],
        ...
        validation_target_altitude = self._get_validation_altitude_target_m(
            agent_id,
            context=context,
            phase=phase,
            current_time=current_time,
        ...
        if validation_target_altitude is None:
            return None
        ...
        group_profile = self._get_validation_group_profile(
            ...
            min_distance=min_distance,
            phase=phase,
            wave_index=int(self._group_states.get(context.group_id).wave_index) if context is not None and context.group_id in self._group_states else None,
        )
        allow_descend = bool(group_profile.get("allow_validation_descend", profile.get("allow_validation_descend", False)))
        target_cmd = self._altitude_cmd_to_target(current_altitude_m, validation_target_altitude)
        if validation_target_altitude >= (float(current_altitude_m) - 80.0):
            return target_cmd
        if not allow_descend:
            return 7
        if current_speed_mps < (self._energy_turn_speed_mps + 15.0) or v_up_mps < -1.5:
            return 7
        return max(6, target_cmd)

    def _get_dynamic_target_pool(self, agent_id: str) -> Tuple[str, ...]:
        context = self._agent_contexts.get(agent_id)
        if context is None:
            return tuple()
        group_profile = self._get_validation_group_profile(context)
        target_pool = group_profile.get("target_pool")
        if isinstance(target_pool, (list, tuple)):
            return tuple(str(item) for item in target_pool)
    ...
    def _get_dynamic_preferred_target_id(self, agent_id: str) -> Optional[str]:
        ...
        if context is None or not context.target_friendly_ids:
            return None
        group_profile = self._get_validation_group_profile(context)
        preferred_target = group_profile.get("preferred_target")
        if preferred_target:
    ...
    def _update_group_state(
        ...
        if phase == "APPROACH":
            ...
            if min_distance <= hard_standoff_distance_km:
                self._arm_hard_escape(env, context, state, current_time, min_distance)
                state.turn_north_entry_y_km = float(context.mean_y_km)
        ...
        if phase == "REGROUP_NORTH":
            ...
            hard_escape_release_km = float(state.hard_escape_release_km or 0.0)
            hard_escape_regroup_y_km = float(state.hard_escape_regroup_y_km or 0.0)
            hard_escape_active = bool(
                float(state.hard_escape_until or 0.0) > current_time
                or (
                    hard_escape_release_km > 0.0
                    and min_distance < (hard_escape_release_km - 1.0)
                )
                or (
                    hard_escape_regroup_y_km > 0.0
                    and context.mean_y_km < (hard_escape_regroup_y_km - 1.0)
                )
            )
            north_depart_reference_y_km = turn_north_entry_y_km + north_depart_margin_km
            if hard_escape_regroup_y_km > 0.0:
                north_depart_reference_y_km = max(
                    north_depart_reference_y_km,
                    hard_escape_regroup_y_km - 6.0,
                )
            north_departed = context.mean_y_km >= north_depart_reference_y_km
            north_reached_goal_y_km = max(float(state.regroup_y_km), hard_escape_regroup_y_km)
            north_reached = context.mean_y_km >= north_reached_goal_y_km - 2.5
            ...
            release_gate_bias_km = 0.0
            if state.pressure_level >= 2:
                release_gate_bias_km = 7.0
                if missile_superiority:
                    release_gate_bias_km = 10.0
                if state.finish_exploit:
                    release_gate_bias_km = 13.0
            elif state.pressure_level >= 1:
                release_gate_bias_km = 3.0
                if missile_superiority:
                    release_gate_bias_km = 5.0
                if state.finish_exploit:
                    release_gate_bias_km = 7.0
            # REGROUP_NORTH 结束应优先服从当前波次的 resume_distance 和局部安全间隔，
            # 而不是被全局 legacy release 门槛长期卡在北撤直飞。
            release_gate_km = max(
                local_release_gate_km,
                resume_gate_km - release_gate_bias_km,
                hard_escape_release_km,
            )
            ...
            aggressive_regroup_ready = bool(
                ...
                and north_departed
                and (formation_ready or len(context.members) <= 1)
                and (
                    recovery_ready
                    or (not energy_critical and elapsed >= max(5.5, regroup_timeout_s))
                )
            )
            if hard_escape_active:
                dynamic_regroup_ready = False
                dynamic_timeout_ready = False
                aggressive_regroup_ready = False
            blocked_reasons = []
            ...
            if hard_escape_active:
                blocked_reasons.append("hard_escape")
            ...
            self._maybe_log_regroup_diagnostics(
                ...
                formation_metrics,
                {
                    ...
                    "aggressive_regroup_ready": aggressive_regroup_ready,
                    "elapsed_s": elapsed,
            ...
            winchester_occupy_ready = bool(
                state.winchester_occupation
                and regroup_min_hold_ready
                and (north_departed or north_reached or elapsed >= max(3.0, regroup_min_hold_s))
                and not hard_escape_active
                and not recovery_hold_active
                and min_distance >= max(20.0, hard_standoff_distance_km - 6.0)
                and (
                    recovery_ready
                    or (not energy_critical)
                    or elapsed >= max(4.0, regroup_timeout_s - 1.0)
                )
                and (formation_ready or len(context.members) <= 1 or elapsed >= max(4.0, regroup_timeout_s - 0.5))
            )
            pressure_reengage_ready = bool(
                ...
                and (
                    formation_ready
                    or len(context.members) <= 1
                    or elapsed >= max(4.5, regroup_min_hold_s + 0.6)
                )
                and (
                    recovery_ready
                    or (not energy_critical and elapsed >= max(5.0, regroup_timeout_s - 0.8))
                )
            )
            if (
                ...
                return
            if winchester_occupy_ready:
                state.turn_south_entry_y_km = float(context.mean_y_km)
                self._set_group_phase(context.group_id, "TURN_SOUTH", current_time, "winchester_occupy")
                return
            if dynamic_regroup_ready:
                state.turn_south_entry_y_km = float(context.mean_y_km)
                self._set_group_phase(context.group_id, "TURN_SOUTH", current_time, "dynamic_regroup_ready")
                return
            if aggressive_regroup_ready:
                state.turn_south_entry_y_km = float(context.mean_y_km)
                self._set_group_phase(context.group_id, "TURN_SOUTH", current_time, "press_reengage")
                return
            if pressure_reengage_ready:
                state.turn_south_entry_y_km = float(context.mean_y_km)
                self._set_group_phase(context.group_id, "TURN_SOUTH", current_time, "pressure_reengage")
                return
            if (
        ...
        if phase == "TURN_SOUTH":
            ...
            if missile_superiority:
                effective_hard_standoff_km = max(
                    40.0,
                    hard_standoff_distance_km,
                )
            occupation_push = bool(
                state.winchester_occupation
                and state.friendly_missiles_total <= max(1, state.friendly_alive_total)
            )
            if state.finish_exploit:
                if state.enemy_missiles_total > 0:
                    effective_hard_standoff_km = max(40.0, effective_hard_standoff_km)
                else:
                    effective_hard_standoff_km = max(40.0, effective_hard_standoff_km)
            elif occupation_push:
                effective_hard_standoff_km = max(40.0, effective_hard_standoff_km)
            south_depart_margin_km = max(4.0, float(self._turn_south_depart_margin_km))
            if missile_superiority:
            ...
            hard_line_margin_km = 2.0
            if missile_superiority:
                hard_line_margin_km = max(hard_line_margin_km, 2.5)
            if state.finish_exploit:
                hard_line_margin_km = max(hard_line_margin_km, 3.0)
            if len(context.members) <= 1 or state.friendly_alive_total <= 1:
                hard_line_margin_km = max(hard_line_margin_km, 2.5)
            hard_line_floor_km = 40.0
            hard_line_trigger_km = max(hard_line_floor_km, effective_hard_standoff_km + hard_line_margin_km)
            ...
            if state.finish_exploit:
                south_reengage_gate_km = max(
                    ...
                    max(hard_standoff_distance_km + 10.0, resume_gate_km - 4.0),
                )
            elif missile_superiority:
                south_reengage_gate_km = max(
                    float(state.turn_entry_distance_km or self._turnback_distance_km) + 8.0,
                    max(hard_standoff_distance_km + 10.0, resume_gate_km - 2.0),
                )
            elif pressure_level >= 2:
                south_reengage_gate_km = max(
                    float(state.turn_entry_distance_km or self._turnback_distance_km) + 10.0,
                    max(hard_standoff_distance_km + 12.0, resume_gate_km),
                )
            else:
                south_reengage_gate_km = max(
                    float(state.turn_entry_distance_km or self._turnback_distance_km) + 12.0,
                    max(hard_standoff_distance_km + 14.0, resume_gate_km + 2.0),
                )
    ...
    def _set_group_phase(self, group_id: str, new_phase: str, current_time: float, reason: str):
        ...
        if new_phase == "APPROACH" and state.phase != "APPROACH":
            group_profile = self._get_validation_group_profile_by_id(group_id)
            try:
                max_wave_index = int(group_profile.get("max_wave_index", 0) or 0)
            except Exception:
                max_wave_index = 0
            if max_wave_index > 0 and int(state.wave_index) >= max_wave_index:
                return
        ...
        if new_phase not in ("TURN_NORTH", "REGROUP_NORTH"):
            ...
            state.hard_escape_until = 0.0
            state.hard_escape_release_km = 0.0
            state.hard_escape_regroup_y_km = 0.0
    ...
    def _build_pressure_profile(
        ...
        *,
        ...
        battle_time_s = float(current_time) if current_time is not None else 0.0
        missile_superiority = bool(
            (enemy_missiles_total > 0 and friendly_missiles_total <= 0 and enemy_alive_total >= (friendly_alive_total + 1))
            or (local_enemy_missiles > 0 and local_friendly_missiles <= 0 and local_enemy_alive >= (local_friendly_alive + 1))
            or (missile_advantage >= 6 and enemy_alive_total >= (friendly_alive_total + 1))
            or (local_missile_advantage >= 4 and local_enemy_alive >= (local_friendly_alive + 1))
        )
        winchester_occupation = bool(
            battle_time_s >= 780.0
            and
            enemy_missiles_total <= 0
            and enemy_alive_total >= max(3, friendly_alive_total + 2)
        ...
        endgame_force_press = bool(
            ...
            and enemy_alive_total >= (friendly_alive_total + 1)
            and battle_time_s >= 540.0
        )
        if winchester_occupation:
            pressure_level = 2
            pressure_tag = "press_occupy"
        elif (
            total_advantage >= 2
        ...
        if pressure_level >= 2:
            hard_standoff_distance_km = max(40.0, self._hard_standoff_distance_km)
            turn_entry_distance_km = max(hard_standoff_distance_km + 8.0, self._turnback_distance_km)
            resume_distance_km = max(hard_standoff_distance_km + 32.0, self._resume_distance_km - 2.0)
            forward_floor_y_km = 88.0
            regroup_timeout_s = max(7.0, self._regroup_timeout_s - 2.0)
            regroup_min_hold_s = max(5.0, self._regroup_min_hold_s - 1.0)
            turn_south_min_hold_s = max(4.0, self._turn_south_min_hold_s - 1.0)
            north_depart_margin_km = max(10.0, self._regroup_depart_margin_km - 2.0)
            if missile_superiority:
                hard_standoff_distance_km = max(40.0, hard_standoff_distance_km)
                turn_entry_distance_km = max(hard_standoff_distance_km + 7.0, turn_entry_distance_km)
                resume_distance_km = max(hard_standoff_distance_km + 31.0, resume_distance_km)
                forward_floor_y_km = min(forward_floor_y_km, 84.0)
                regroup_timeout_s = max(6.5, regroup_timeout_s - 0.5)
                regroup_min_hold_s = max(4.5, regroup_min_hold_s - 0.5)
                turn_south_min_hold_s = max(3.8, turn_south_min_hold_s - 0.2)
                north_depart_margin_km = max(9.0, north_depart_margin_km - 1.0)
            if finish_exploit:
                hard_standoff_distance_km = max(40.0, hard_standoff_distance_km)
                turn_entry_distance_km = max(hard_standoff_distance_km + 7.0, turn_entry_distance_km)
                resume_distance_km = max(hard_standoff_distance_km + 30.0, resume_distance_km)
                forward_floor_y_km = min(forward_floor_y_km, 82.0)
                regroup_timeout_s = max(6.0, regroup_timeout_s - 0.5)
                regroup_min_hold_s = max(4.0, regroup_min_hold_s - 0.3)
                turn_south_min_hold_s = max(3.6, turn_south_min_hold_s - 0.2)
                north_depart_margin_km = max(8.0, north_depart_margin_km - 0.5)
            if winchester_occupation:
                hard_standoff_distance_km = max(40.0, hard_standoff_distance_km)
                turn_entry_distance_km = max(hard_standoff_distance_km + 6.0, turn_entry_distance_km)
                resume_distance_km = max(hard_standoff_distance_km + 28.0, resume_distance_km)
                forward_floor_y_km = min(forward_floor_y_km, 90.0)
                regroup_timeout_s = max(5.5, regroup_timeout_s - 0.5)
                regroup_min_hold_s = max(4.0, regroup_min_hold_s - 0.5)
                turn_south_min_hold_s = max(3.8, turn_south_min_hold_s - 0.3)
                north_depart_margin_km = max(8.0, north_depart_margin_km - 1.0)
            profile = {
                ...
                "winchester_occupation": winchester_occupation,
            }
            ...
            profile = {
                ...
                "pressure_tag": pressure_tag,
                "hard_standoff_distance_km": hard_standoff_distance_km,
                "turn_entry_distance_km": max(hard_standoff_distance_km + 6.5, self._turnback_distance_km - 0.5),
                "resume_distance_km": max(hard_standoff_distance_km + 29.0, self._resume_distance_km - 2.0),
                "forward_floor_y_km": 90.0,
                "regroup_timeout_s": max(7.0, self._regroup_timeout_s - 2.0),
                "regroup_min_hold_s": max(4.5, self._regroup_min_hold_s - 1.2),
                "turn_south_min_hold_s": max(3.8, self._turn_south_min_hold_s - 1.0),
                "north_depart_margin_km": max(9.0, self._regroup_depart_margin_km - 1.5),
                "enemy_alive_total": enemy_alive_total,
                "friendly_alive_total": friendly_alive_total,
                "local_advantage": local_advantage,
                "enemy_missiles_total": enemy_missiles_total,
        ...
        return self._apply_validation_profile_overrides(
            profile,
            context=context,
            current_time=current_time,
            min_distance=min_distance,
    ...
    def _refresh_wave_geometry(
        ...
        pressure = self._build_pressure_profile(
            env,
            context,
            current_time=float(getattr(self, "_last_seen_time", 0.0) or 0.0),
            min_distance=min_distance,
        )
        ...
        if len(context.members) <= 1:
            turn_entry_distance = float(max(hard_standoff_distance_km + 1.5, turn_entry_distance - 1.0))
            resume_distance = float(max(hard_standoff_distance_km + 24.0, resume_distance - 2.0))
        ...
        if len(context.members) <= 1:
            regroup_extension_km = 6.0 if pressure_level >= 2 else 12.0 if pressure_level >= 1 else 18.0
            regroup_cap_from_initial = context.initial_center_y_km - (24.0 if pressure_level >= 2 else 14.0 if pressure_level >= 1 else 8.0)
            regroup_target = max(
                regroup_floor,
                forward_limit_y_km + (5.0 if pressure_level >= 2 else 8.0 if pressure_level >= 1 else 10.0),
                min(regroup_cap_from_initial, context.mean_y_km + regroup_extension_km),
            )
        else:
            regroup_extension_km = 8.0 if pressure_level >= 2 else 14.0 if pressure_level == 1 else 26.0
            regroup_cap_from_initial = context.initial_center_y_km - (26.0 if pressure_level >= 2 else 18.0 if pressure_level == 1 else 10.0)
            regroup_target = max(
                regroup_floor + (0.0 if pressure_level >= 2 else 2.0 if pressure_level == 1 else 6.0),
                forward_limit_y_km + (6.0 if pressure_level >= 2 else 10.0 if pressure_level == 1 else 16.0),
                min(regroup_cap_from_initial, context.mean_y_km + regroup_extension_km),
            )
        ...
        if state.phase in ("TURN_NORTH", "REGROUP_NORTH"):
            if float(state.hard_escape_release_km or 0.0) > 0.0:
                resume_distance = max(resume_distance, float(state.hard_escape_release_km))
            if float(state.hard_escape_regroup_y_km or 0.0) > 0.0:
                regroup_target = max(regroup_target, float(state.hard_escape_regroup_y_km))
        ...
        state.winchester_occupation = bool(pressure.get("winchester_occupation", False))
        state.regroup_y_km = float(np.clip(
            regroup_target,
    ...
    def _slot_x_for_phase(
        ...
        *,
        ...
        slot_x = float(slot_map.get(agent_id, anchor_x_km))
        slot_x += self._get_validation_slot_bias_km(
            context,
            phase,
            current_time=current_time,
            min_distance=min_distance,
    ...
    def _arm_hard_escape(
        ...
        hard_line_km = float(state.hard_standoff_distance_km or self._hard_standoff_distance_km)
        base_resume_km = float(state.resume_distance_km or self._resume_distance_km)
        escape_depth_bonus_km = max(0.0, hard_line_km - float(min_distance)) * 0.35
        escape_release_km = max(
            base_resume_km + 2.0,
            hard_line_km + (24.0 if len(context.members) <= 1 else 26.0) + escape_depth_bonus_km,
        )
        friendly_mean_y_km = self._get_friendly_mean_y_km(env, context.target_friendly_ids)
        escape_regroup_y_km = max(
            float(state.regroup_y_km or 0.0),
            friendly_mean_y_km + escape_release_km + 6.0,
            float(context.mean_y_km) + (18.0 if len(context.members) <= 1 else 20.0),
        )
        escape_hold_s = 8.0 if len(context.members) <= 1 else 7.0
        if state.missile_superiority and state.pressure_level >= 2:
            escape_hold_s = max(6.0, escape_hold_s - 1.0)
        state.hard_escape_until = max(float(state.hard_escape_until or 0.0), current_time + escape_hold_s)
        state.hard_escape_release_km = max(float(state.hard_escape_release_km or 0.0), escape_release_km)
        state.hard_escape_regroup_y_km = float(np.clip(
            max(float(state.hard_escape_regroup_y_km or 0.0), escape_regroup_y_km),
            self.bounds.y_min + 78.0,
            self.bounds.y_max - 2.0 * self._boundary_margin_km,
        ))
        state.regroup_y_km = max(float(state.regroup_y_km or 0.0), float(state.hard_escape_regroup_y_km))
    ...
    def _get_combat_command(
        ...
        advisory_cmd = self._get_raw_combat_ai_command(env, agent_id, current_time, task)
        ...
        attack_slot_x_km = self._slot_x_for_phase(
            context,
            agent_id,
            "APPROACH",
            current_time=current_time,
        ...
        alt_cmd = max(advisory_alt_cmd, self._altitude_command(current_altitude, "APPROACH"))
        ...
        scene_pull_deg = self._get_validation_scene_pull_deg(
            context=context,
            phase="APPROACH",
            current_time=current_time,
            min_distance=min_distance,
        ...
        validation_alt_cmd = self._get_validation_altitude_command(
            agent_id,
            context=context,
            phase="APPROACH",
            current_time=current_time,
        ...
        if validation_alt_cmd is not None:
            alt_cmd = max(alt_cmd, int(validation_alt_cmd))
            validation_target_altitude = self._get_validation_altitude_target_m(
                agent_id,
                context=context,
                phase="APPROACH",
            ...
            if validation_target_altitude is not None and validation_target_altitude - current_altitude > 250.0:
                vel_cmd = max(vel_cmd, 6)
            elif validation_target_altitude is not None and validation_target_altitude - current_altitude > 80.0:
        ...
        if min_distance <= (turn_entry_distance_km + 3.5):
            ...
            hdg_cmd = self._blend_advisory_scene_heading_cmd(
                current_heading,
                advisory_hdg_cmd,
                target_heading,
                max_scene_pull_deg=scene_pull_deg,
            )
    ...
    def _build_group_command(
        ...
        current_speed = self._get_speed_mps(aircraft)
        v_up_mps = self._get_vertical_speed_mps(aircraft)
        group_distance = self._get_group_min_distance_km(env, context.members, context.target_friendly_ids)
        hard_escape_release_km = float(state.hard_escape_release_km or 0.0)
        hard_escape_regroup_y_km = float(state.hard_escape_regroup_y_km or 0.0)
        hard_escape_active = bool(
            state.phase in ("TURN_NORTH", "REGROUP_NORTH")
            and (
                current_time < float(state.hard_escape_until or 0.0)
                or (
                    hard_escape_release_km > 0.0
                    and group_distance < (hard_escape_release_km - 1.0)
                )
                or (
                    hard_escape_regroup_y_km > 0.0
                    and current_y_km < (hard_escape_regroup_y_km - 1.0)
                )
            )
        )
        slot_x_km = self._slot_x_for_phase(
            context,
            agent_id,
            state.phase,
            current_time=current_time,
        ...
        if hard_escape_active:
            slot_x_km = float(np.clip(
                0.85 * current_x_km + 0.15 * context.anchor_x_km,
                context.sector_x_min_km + 1.0,
                context.sector_x_max_km - 1.0,
            ))
        if state.phase == "APPROACH":
        ...
        if state.phase == "APPROACH":
            ...
            if state.phase == "TURN_NORTH":
                min_speed_cmd = 7 if hard_escape_active else 6
                target_y_km = max(
                    state.regroup_y_km,
                    hard_escape_regroup_y_km,
                    current_y_km + (16.0 if hard_escape_active else 12.0),
                )
                ...
                target_y_km = max(
                    state.regroup_y_km,
                    hard_escape_regroup_y_km,
                    current_y_km + (14.0 if hard_escape_active else 10.0),
                )
            else:
                min_speed_cmd = 6
        ...
        target_heading = self._boundary_correction(current_x_km, current_y_km, target_heading)
        alt_cmd = max(advisory_alt_cmd, self._altitude_command(current_altitude, state.phase))
        vel_cmd = max(advisory_vel_cmd, self._speed_command(current_altitude, min_speed_cmd))
        advisory_target_heading = self._cmd_to_target_heading(current_heading, advisory_hdg_cmd)
        desired_heading_error = abs(self._normalize_heading_error(target_heading - current_heading))
        scene_pull_deg = self._get_validation_scene_pull_deg(
            context=context,
        ...
        boundary_guard = self._needs_boundary_correction(current_x_km, current_y_km)
        distance_discipline_guard = bool(
            state.phase in ("TURN_NORTH", "REGROUP_NORTH")
            and (
                hard_escape_active
                or group_distance < max(
                    float(state.resume_distance_km or self._resume_distance_km),
                    float(state.hard_standoff_distance_km or self._hard_standoff_distance_km) + 20.0,
                )
            )
        )

        if hard_escape_active and desired_heading_error > 8.0:
            hdg_cmd = self._fast_turn_hdg_cmd(
                current_heading,
                target_heading,
                self._preferred_turn_sign(context, "TURN_NORTH"),
            )
        elif hard_escape_active:
            hdg_cmd = self._hdg_to_cmd(current_heading, target_heading)
        elif distance_discipline_guard and desired_heading_error > 18.0:
            hdg_cmd = self._fast_turn_hdg_cmd(
                current_heading,
                target_heading,
                self._preferred_turn_sign(context, "TURN_NORTH"),
            )
        elif distance_discipline_guard:
            hdg_cmd = self._hdg_to_cmd(current_heading, target_heading)
        elif state.phase in ("TURN_NORTH", "TURN_SOUTH") and desired_heading_error > 28.0:
            hdg_cmd = self._fast_turn_hdg_cmd(
                current_heading,
                target_heading,
            ...
            if state.phase == "REGROUP_NORTH":
                formation_metrics = self._get_formation_metrics(env, context)
                if (
                    formation_metrics["max_x_error_km"] > 5.5
                    or formation_metrics["spacing_error_km"] > 4.5
                    or formation_metrics["y_spread_km"] > 6.0
                ):
                    hdg_cmd = self._hdg_to_cmd(current_heading, target_heading)
                else:
                    hdg_cmd = self._blend_advisory_scene_heading_cmd(
                        ...
                        advisory_hdg_cmd,
                        target_heading,
                        max_scene_pull_deg=max(scene_pull_deg, 14.0),
                    )
            else:
                hdg_cmd = self._blend_advisory_scene_heading_cmd(
                    current_heading,
                    advisory_hdg_cmd,
                    target_heading,
                    max_scene_pull_deg=max(
                        scene_pull_deg,
                        24.0 if state.phase in ("TURN_NORTH", "TURN_SOUTH") else scene_pull_deg,
                    ),
                )
