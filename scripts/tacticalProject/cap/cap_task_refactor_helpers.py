...
def _build_enemy_recreate_state(sim: AircraftSimulator, rescue_speed_mps: float) -> dict:
...
def _build_recreate_state_from_snapshot(
    ...
    *,
    ...
    return {
        ...
        "ic_h_sl_ft": float(snapshot["ic_h_sl_ft"]),
...
def _maybe_recreate_enemy_simulator(self, env, agent_id: str) -> bool:
    ...
    severe_emergency = (
        ...
        and (
            vc_mps < 145.0
            or v_up_mps < -10.5
            or (alt_m < 4200.0 and v_up_mps < -7.0)
            or (alt_m < 2600.0 and vc_mps < 170.0)
        )
    )
    ...
    cooldown_bypass = severe_emergency and steps_since_last >= max(40, cooldown_steps // 4)
    ...
    recent_recreate_times = [
        float(ts)
        for ts in state.get("times_s", [])
        if (float(now_step * 0.2) - float(ts)) <= 60.0
    ]
    count_cap_bypass = severe_emergency and steps_since_last >= max(80, cooldown_steps // 3) and len(recent_recreate_times) < 4
    ...
    if healthy_snapshot is not None:
        ...
        new_state = _build_enemy_recreate_state(sim, rescue_speed_mps)
    ...
    state["times_s"].append(float(now_step * 0.2))
    window.clear()
...
def _maybe_recreate_friendly_simulator(self, env, agent_id: str) -> bool:
    ...
    severe_emergency = (
        len(window) >= max(4, window.maxlen // 3)
        and (
            v_up_mps < -10.5
    ...
    cooldown_steps = _sim_recreate_cooldown_steps(agent_id)
    steps_since_last = now_step - int(state.get("last_step", -10**9))
    cooldown_bypass = severe_emergency and steps_since_last >= max(90, cooldown_steps // 2)
    if steps_since_last < cooldown_steps and not cooldown_bypass:
        return False
    ...
    count_cap_bypass = (
        severe_emergency
        and steps_since_last >= max(140, int(cooldown_steps * 0.75))
    ...
    if healthy_snapshot is not None:
        snapshot_vc = float(healthy_snapshot.get("vc", rescue_speed_mps))
        rescue_speed_mps = float(np.clip(max(rescue_speed_mps, snapshot_vc - 8.0), 165.0, 235.0))
    rescue_speed_mps = _clamp_recreate_speed_gain(
        agent_id,
        vc_mps,
        rescue_speed_mps,
    ...
    log.warning(
        "[%s][T+%07.1fs][friendly_sim_recreate][trigger] alt=%.1fm vc=%.1fm/s v_up=%.2fm/s pitch=%.2fdeg count=%d rescue_speed=%.1fm/s source=%s age=%.1fs anchor=%s severe=%s cap_bypass=%s",
        agent_id,
        float(now_step * 0.2),
        alt_m,
        vc_mps,
    ...
    try:
        ...
        log.warning(
            "[%s][T+%07.1fs][friendly_sim_recreate][applied] alt=%.1fm vc=%.1fm/s pitch=%.2fdeg v_up=%.2fm/s missiles_left=%d",
            agent_id,
            float(now_step * 0.2),
            alt_after,
            vc_after,
