...
class EnemyAIAdapter:
    ...
    def __init__(self, project_name: str = "tactical_simulation"):
        ...
        self._use_new_enemy_ai = os.getenv("CAP_NEW_ENEMY_MANEUVER_AI_ENABLED", "1").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
        ...
        if self._use_new_enemy_ai and NewEnemyManeuverAI is not None:
            self.unified_ai = NewEnemyManeuverAI(project_name=project_name)
        else:
        ...
        logging.info(f"[敌方AI适配器] 初始化完成 - 项目: {project_name}")
    ...
    def get_detailed_action_info(self, agent_id: str) -> Dict[str, Any]:
        """获取详细的动作信息"""
        try:
            if hasattr(self.unified_ai, "get_detailed_action_info"):
                return self.unified_ai.get_detailed_action_info(agent_id)
    ...
    def get_current_tactical_mode(self, agent_id: str) -> str:
        """获取当前战术模式"""
        if hasattr(self.unified_ai, "get_current_tactical_mode"):
            return self.unified_ai.get_current_tactical_mode(agent_id)
        mode = self.unified_ai.tactical_mode.get(agent_id, TacticalMode.NEUTRAL)
        return mode.value
    
    def get_current_phase(self, agent_id: str) -> str:
        """获取当前战术阶段"""
        if hasattr(self.unified_ai, "get_current_phase"):
            return self.unified_ai.get_current_phase(agent_id)
    ...
    def force_tactical_mode(self, agent_id: str, mode: str):
        """强制设置战术模式 - 用于测试"""
        if hasattr(self.unified_ai, "force_tactical_mode"):
            self.unified_ai.force_tactical_mode(agent_id, mode)
            return
    ...
    def force_action(self, agent_id: str, action_name: str, current_time: float = 0.0):
        """强制执行特定动作 - 用于测试"""
        try:
            if hasattr(self.unified_ai, "force_action"):
                self.unified_ai.force_action(agent_id, action_name, current_time)
                return
    ...
    def get_system_status(self) -> Dict[str, Any]:
        """获取系统状态 - 用于监控"""
        if hasattr(self.unified_ai, "get_system_status"):
            return self.unified_ai.get_system_status()
        return {
