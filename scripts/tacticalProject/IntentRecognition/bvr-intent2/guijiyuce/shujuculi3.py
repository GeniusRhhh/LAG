...
# 随机选择3条轨迹作为测试库，排除在滑动窗口处理之外
test_series_indices = np.random.choice(len(all_time_series), size=3, replace=False)
print(f"选择的测试轨迹索引: {test_series_indices}")

# 保存测试轨迹数据到单独的文件
test_trajectories = []
train_time_series = []

for idx, series in enumerate(all_time_series):
    if idx in test_series_indices:
        test_trajectories.append(series)
    else:
        train_time_series.append(series)

# 保存测试轨迹数据
test_save_path = os.path.join(output_folder, "test_trajectories.npz")
np.savez_compressed(
    test_save_path,
    test_trajectories=test_trajectories
)

print(f"已保存 {len(test_trajectories)} 条测试轨迹到 {test_save_path}")
print(f"用于训练的轨迹数量: {len(train_time_series)}")
...
for idx, time_series in enumerate(train_time_series):
    print(f"Applying sliding window to time series {idx + 1}/{len(train_time_series)}: {time_series['label']}")
