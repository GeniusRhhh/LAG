...
class TacticalTask(MultipleCombatTask):
            ...
            if env.current_step % 360 == 0:  # 360步 ≈ 30秒
                ...
                for agent_id in env.agents.keys():
                    if env.agents[agent_id].is_alive:
                        ...
                        alt = pos[2]
                        logging.info(
                            f"[T={current_time:.1f}s][{agent_id}] 位置: ({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.1f}m) "
    ...
    def _use_lowlevel_policy(self, env, agent_id: str, altitude_cmd_id: int, heading_cmd_id: int,
                             velocity_cmd_id: int) -> np.ndarray:
        """
        使用baseline模型生成底层控制
    ...
    def _handle_missile_launches(self, env, current_time: float):
        ...
        for agent_id in list(self.missile_launched.keys()):
            ...
            if heading_error > 90:
                if env.current_step % 60 == 0:
                    logging.warning(
                        f"⚠️ [{agent_id}] 发射条件不满足: 航向{current_heading:.0f}°, 目标方位{target_bearing:.0f}°, 夹角{heading_error:.0f}° >90°")
                # 不立即标记为False，等待调整航向
                continue
        ...
        if agent_id:
            ...
            if old_phase is None and new_phase == TacticalPhase.NLT_MELD:
                if agent_id.endswith('100'):  # 长机
                    logging.info(
                        f"⚡ 我方{agent_id}: 首次进入{new_phase.value}阶段 (距离{adjusted_distance / 1000:.0f}km)")
                ...
                if new_phase == TacticalPhase.NLT_MELD and old_phase not in [None, TacticalPhase.NLT_MELD,
                                                                             TacticalPhase.MELD_MTR]:
                    logging.warning(
                        f"⚠️ [{agent_id}] 战斗已开始({old_phase.value})，拒绝回退到NLT_MELD（距离{adjusted_distance / 1000:.1f}km）")
                    return  # 保持当前阶段不变，不执行NLT决策

                current_time = env.current_step * env.time_interval
                # 只在重要阶段转换时打印（简化）
                if new_phase in [TacticalPhase.MELD_MTR, TacticalPhase.MTR_LR, TacticalPhase.LR_TR,
    ...
    def _determine_phase_by_distance(self, distance):
        ...
        if self.min_distance_reached < self.tactical_distances['DR'] and distance > self.tactical_distances['MTR']:
            if not self.is_second_attack:
                ...
                logging.info(
                    f"🔄 [第二轮攻击] 距离从{self.min_distance_reached / 1000:.1f}km拉大到{distance / 1000:.1f}km，进入MTR2阶段")
    ...
    def _form_initial_formation(self, env):
        ...
        try:
            ...
            logging.info(
                f"   当前误差：横向{lateral_error:.0f}m，纵向{longitudinal_error:.0f}m，高度{altitude_error:.0f}m（总距{distance_error:.0f}m）")
    ...
    def _apply_formation_params(self, env, params: dict):
        ...
        try:
            ...
            if abs(lateral_err) > 3000:
                logging.info(
                    f"🧭 [编队重组] A0200横向{lateral_err:.0f}m，对齐长机航向{target_heading:.1f}°（差{heading_diff:.1f}°）")
            ...
            if abs(alt_err) > ALT_TOLERANCE:
                logging.info(
                    f"📊 [编队重组检测] A0200速度={wingman_speed:.1f}m/s, 高度差={alt_err:.0f}m, 当前高度={current_wingman_alt:.0f}m")
            ...
            if abs(alt_err) > ALT_TOLERANCE or abs(lateral_err) > 1000:
                logging.info(
                    f"✅ [NLT编队执行] 僚机指令 alt_id={alt_idx}（高度差{alt_err:.0f}m）hdg_id={hdg_idx}（横向{lateral_err:.0f}m）vel_id={vel_idx}（速度{wingman_speed:.0f}m/s）")
            else:
    ...
    def _decide_at_nlt(self, env):
        ...
        try:
            ...
            logging.info(
                f"   ✅ 角色分配: 长机={self.tactical_roles.get('lead', 'left')}, 僚机={self.tactical_roles.get('wingman', 'right')}")
    ...
    def _decide_at_meld(self, env):
        ...
        try:
            ...
            if new_tactic != self.selected_tactic:
                ...
                logging.info(
                    f"🔄 MELD角色调整: 长机={new_roles.get('lead', 'left')}, 僚机={new_roles.get('wingman', 'right')}")
    ...
    def _decide_at_mtr(self, env, agent_id: str):
        ...
        try:
            ...
            my_threat = threat_info.get('lead_threat', 0.5) if agent_id.endswith('100') else threat_info.get(
                'wingman_threat', 0.5)
        ...
        try:
            ...
            if is_lr_prime:
                ...
                if rwr_threat_level >= 3:
                    should_abort = True
                    logging.warning(
                        f"⚠️ [LR_PRIME终止] {agent_id} RWR威胁等级{rwr_threat_level}（LOCK/导弹），终止中制导+返航")
                elif my_threat > 0.8:
                    should_abort = True
        ...
        try:
            ...
            # 保守肃清：检查威胁
                ...
                if enemy_intent_type == 'ATTACK_TYPE' and (my_threat > 0.8 or rwr_threat_level >= 3):
                    should_continue = False
                    logging.warning(
                        f"⚠️ [TR决策] {agent_id} 威胁过高({my_threat:.2f})或RWR={rwr_threat_level}，决定脱离")
        ...
        try:
            ...
            if time_in_dr < self.dr_time_window:
                if not self.dr_beam_completed:
                    logging.info(
                        f"⏱️ [DR时间窗] 进度: {time_in_dr:.1f}s/{self.dr_time_window}s，距离: {distance_nm:.1f}NM（目标5NM）")
    ...
    def _get_tactical_command_indices(self, env, agent_id: str) -> tuple:
        ...
        if current_alt > 9000 and current_speed < 150:
            ...
            logging.warning(
                f"⚠️ [{agent_id}] 极高空低速! 强制下降换速度 (高度{current_alt:.0f}m, 速度{current_speed:.1f}m/s)")
        elif current_alt > 8000 and current_speed < 120:
            # 高空极低速，强制下降换速度
            alt_cmd = 3  # 下降-500m
            vel_cmd = 4  # 加速+50m/s
            logging.warning(
                f"⚠️ [{agent_id}] 高空极低速! 强制下降换速度 (高度{current_alt:.0f}m, 速度{current_speed:.1f}m/s)")
    ...
    def _execute_short_skate(self, env, agent_id: str, direction='left', target_heading=None) -> tuple:
        ...
        if agent_id not in self.maneuver_states or self.maneuver_states[agent_id].get('type') != 'short_skate':
            ...
            logging.info(
                f"🔄 【Short Skate】{agent_id}开始{direction}侧返航机动 (初始{initial_heading:.1f}°→目标{target_heading:.1f}°)")
        ...
        if state['phase'] == 'crank':
            ...
            if phase_time < crank_duration:
                ...
                if env.current_step % 30 == 0:
                    logging.info(
                        f"🔄 [Short Skate-Crank] {agent_id} 当前{current_heading:.1f}°→目标{target_heading_now:.1f}° 差{heading_diff:.1f}° 高度{current_alt:.0f}m")
        ...
        if state['phase'] == 'turn_cold':
            ...
            if phase_time < turn_cold_duration:
                ...
                if env.current_step % 30 == 0:
                    logging.info(
                        f"🔄 [Short Skate-TurnCold] {agent_id} 当前{current_heading:.1f}°→目标{target_heading_now:.1f}° 差{heading_diff:.1f}° 高度{current_alt:.0f}m")
        ...
        if state['phase'] == 'escape':
            ...
            if phase_time < escape_duration:
                if env.current_step % 30 == 0:
                    logging.info(
                        f"🔄 [Short Skate-Escape] {agent_id} 加速逃离 航向{current_heading:.1f}° 高度{current_alt:.0f}m")
                return 7, 8, 5  # 保持航向+加速
            else:
                # 完成Short Skate，清理状态
                del self.maneuver_states[agent_id]
            ...
            if wingman_phase in [TacticalPhase.NLT_MELD, TacticalPhase.MELD_MTR, TacticalPhase.MTR_LR]:
                ...
                if last_launch < 0:  # 僚机还未发射
                    if lead_launch > 0 and (current_time - lead_launch) >= 5.0:
                        ...
                        logging.info(
                            f"🚀 [僚机跟随发射] {agent_id} 在长机发射{current_time - lead_launch:.1f}秒后准备发射")
                    elif lead_launch < 0:
    ...
    def _execute_pincer_attack(self, env, agent_id: str) -> tuple:
        ...
        # 僚机动作（右侧包抄，独立阶段判断）- 使用固定目标
            ...
            lead_pos = env.agents['A0100'].get_position() if env.agents['A0100'].is_alive else env.agents[
                agent_id].get_position()
            wing_pos = env.agents[agent_id].get_position()
            lateral_separation = abs(wing_pos[1] - lead_pos[1])  # Y方向距离
            target_lateral_separation = 10000  # 钳形攻势目标横向分离：10km
    ...
    def _execute_tactical_evasion(self, env, agent_id: str) -> tuple:
        ...
        # DR-MAR阶段：根据DR决策结果决定是否重新进攻或返航
            ...
            if self._return_log_counter[agent_id] % 60 == 0:
                logging.info(
                    f"🛡️ [DR_MAR返航] {agent_id} 当前{current_heading:.0f}°→基地{target_heading:.0f}° (X={pos[0] / 1000:.1f}km)")
    ...
    def _establish_rear_formation(self, env, agent_id: str, current_time: float) -> tuple:
        ...
        if current_state == 'LATERAL_ALIGN':
            ...
            if in_stable_zone:
                ...
                logging.info(
                    f"🔄 [{agent_id}]阶段3-只调速度: dx={dx / 1000:.2f}km dy={dy / 1000:.2f}km vel={velocity_cmd_id}")
            ...
            if seriously_off:
                self.formation_state[agent_id] = 'LATERAL_ALIGN'
                logging.warning(
                    f"⚠️ [{agent_id}]严重脱离队形 dx={dx / 1000:.2f}km dy={dy:.0f}m hdg={heading_diff:.1f}° → 重新对齐")
            else:
                # 在稳定区域内：动态微调保持一字型
                ...
                if env.current_step % 60 == 0:
                    logging.info(
                        f"✅ [{agent_id}]队形微调: dx={dx / 1000:.2f}km dy={dy / 1000:.2f}km vel_diff={speed_diff:.0f}fps hdg={heading_cmd_id} vel={velocity_cmd_id}")
    ...
    def _update_integrated_tactical_system(self, env, agent_id: str):
        ...
        try:
            ...
            # 找到我方所有存活飞机
            my_aircraft_list = [env.agents[aid] for aid in env.agents if
                                aid.startswith('A') and env.agents[aid].is_alive]
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        if agent_id not in self.initial_heading:
            ...
            logging.info(
                f"{agent_id} 初始状态: 航向{self.initial_heading[agent_id]:.1f}°, 高度{self.initial_altitude[agent_id]:.1f}m")
        ...
        if env.current_step % 120 == 0 and agent_id.startswith('A'):
            ...
            phase = self.agent_phases.get(agent_id, TacticalPhase.NLT_MELD)
            logging.info(
                f"🔧 [{agent_id}] 阶段:{phase.value} 当前航向:{current_heading:.1f}° 指令:alt={altitude_cmd_id},hdg={heading_cmd_id}({hdg_change:+.1f}°),vel={velocity_cmd_id}")
        ...
        if agent_id in self.beam_maneuver_state:
            ...
            if 'start_time' in state:
                ...
                if elapsed < 5.0:
                    ...
                    if env.current_step % 60 == 0:
                        logging.info(
                            f"📍 [BEAM执行] {agent_id} 敌机{enemy_bearing:.1f}° → {beam_direction} {target_heading:.1f}° (剩{5.0 - elapsed:.1f}s)")
                ...
                # 阶段3：冷却完成，开始新一轮BEAM
                    ...
                    logging.info(
                        f"📍 [BEAM启动] {agent_id} 敌机{enemy_bearing:.1f}° → {beam_direction} {target_heading:.1f}°")
                    return self._turn_to_heading(env, agent_id, target_heading, speed_cmd=3)
        else:
            # 首次启动BEAM
