...
class TacticalAdapter:
    ...
    def initialize_radars(self, env):
        ...
        for role in ['lead', 'wingman']:
            ...
            if aircraft_id in env.agents:
                ...
                logging.info(f"[TacticalAdapter] 初始化友方雷达: {aircraft_id} (N001VE)")
        ...
        for role in ['enemy1', 'enemy2']:
            ...
            if aircraft_id in env.agents:
                ...
                logging.info(f"[TacticalAdapter] 初始化敌方雷达: {aircraft_id} (APG-68)")
    ...
    def convert_maneuver_to_action(self, maneuver: Dict, agent) -> np.ndarray:
        ...
        # 🔥 低速保护：速度过低时自动加大油门（对敌我双方都生效）
        try:
            speed = float(np.linalg.norm(agent.get_velocity()))
            if speed < 220.0:  # 极低速：紧急加速
                throttle = 1.0
            elif speed < 260.0:  # 低速：强制加速
                throttle = max(throttle, 0.95)
            elif speed < 300.0:  # 中低速：适度加速
                throttle = max(throttle, 0.88)
            elif speed < 340.0:  # 正常速度：轻微补偿
                throttle = max(throttle, 0.82)
        except Exception:
            pass
        ...
        if maneuver_type == 'short_skate':
            ...
            throttle = max(throttle, 0.9)  # 保持较高油门
        elif maneuver_type == 'notch_back':
            # 回转+下降
            aileron = 1.0 if heading_diff > 0 else -1.0
            elevator = -0.3
            throttle = max(throttle, 0.85)  # 提高基础油门
        elif maneuver_type == 'beam':
            # Beam机动：保持垂直于威胁方向
            aileron = np.clip(heading_diff / 30.0, -1.0, 1.0)
            throttle = max(throttle, 0.85)  # 提高基础油门

        # 🔥 最终低速兜底：特殊机动可能降低油门，这里再次确保低速时能补偿
        try:
            speed = float(np.linalg.norm(agent.get_velocity()))
            if speed < 220.0:
                throttle = 1.0
            ...
            pass
        if maneuver_type == 'pincer_converge':
