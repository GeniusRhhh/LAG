...
from .intent_recognizer import EnemyIntent, FriendlyIntent, ProjectIntentAdapter
...
class IntegratedTacticalSystem:
    ...
    def __init__(self, my_intent: FriendlyIntent = FriendlyIntent.CONSERVATIVE_CLEAR):
        ...
        self.intent_recognizer = ProjectIntentAdapter()
    ...
    def update_intent_recognition(
        ...
        try:
            ...
            # 识别意图（使用旧版本方法以保持兼容性）
            intent = self.intent_recognizer.recognize_enemy_intent_env(
                env,
                enemy_agent_id,
                my_aircraft_list
            )
