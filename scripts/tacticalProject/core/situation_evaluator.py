...
class SituationEvaluator:
    ...
    def _evaluate_detection(self, my_aircraft, enemy_aircraft) -> float:
        ...
        try:
            ...
            radar_aspect = abs(self._normalize_angle(my_heading - bearing_to_enemy))
            
            # 简化的RCS模型（实际应根据目标类型和角度）
            # 假设正面RCS较小，侧面和尾部较大
            if radar_aspect < 30:
                rcs_factor = 0.5  # 正面RCS小，难以探测
            elif radar_aspect < 150:
                rcs_factor = 1.0  # 侧面RCS大，易于探测
            else:
                rcs_factor = 0.8  # 尾部RCS中等
            
            # 距离因素（雷达探测能力随距离衰减）
            distance_km = distance / 1000.0
            max_detection_range = 150  # km
            distance_factor = 1.0 - (distance_km / max_detection_range)
            
            # 综合探测概率
            detection_score = rcs_factor * distance_factor
