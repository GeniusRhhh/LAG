...
class HighLowSplitV2TacticalTask(MultipleCombatTask):
    """
    上下夹击战术任务V2 - 完全基于拖曳射击的成功架构
    ...
    def __init__(self, config):
        ...
        self.high_low_timeline = {
            ...
            "wingman_attack_delay": 10.0,  # 僚机攻击滞后10秒
    ...
    def _get_leader_command_indices(self, env, agent_id: str):
        ...
        if self.current_phase in [TacticalPhase.NLT_MELD, TacticalPhase.MELD_MTR]:
            ...
            # 长机先行攻击阶段
            self.high_low_timeline["leader_attack_phase"] = current_time
    ...
    def _get_wingman_command_indices(self, env, agent_id: str):
        ...
        if wingman_phase == TacticalPhase.NLT_MELD:
            # 僚机建立高度优势阶段
            return self._establish_high_altitude_advantage(env, agent_id)

        elif wingman_phase == TacticalPhase.MELD_MTR:
            # 僚机保持高度优势，调整位置准备攻击
            return self._maintain_high_altitude_position(env, agent_id, 0.0)
            ...
            # 僚机高空攻击阶段 - 等待长机攻击后再行动
            leader_attack_time = self.high_low_timeline.get("leader_attack_phase")
            if leader_attack_time and (current_time - leader_attack_time) >= self.high_low_timeline["wingman_attack_delay"]:
                # 长机已攻击足够时间，僚机开始俯冲攻击
                self.high_low_timeline["wingman_attack_ready"] = True
    ...
    def _handle_missile_launch(self, env, agent_id: str, current_time: float):
        ...
        # 上下夹击战术：僚机在DOR_DR阶段可以发射导弹
        if agent_id.startswith('A') and self.current_phase == TacticalPhase.DOR_DR:
            if agent_id == "A0100":  # 长机在DOR_DR阶段已经返航，禁止发射
                logging.info(f"{agent_id} 长机返航期间禁止发射导弹")
                return
            elif agent_id == "A0200":  # 僚机在DOR_DR阶段可以发射（上下夹击特殊逻辑）
                logging.debug(f"{agent_id} 僚机DOR_DR阶段，检查发射条件")
                # 继续执行发射逻辑检查
        ...
        # 上下夹击专用发射逻辑
        if agent_id == "A0100":  # 长机：低空攻击者，先行发射
            # 长机在TR_DOR阶段先行发射
            ...
            if should_launch:
                # 记录长机攻击时间
                current_time = env.current_step * env.time_interval
                self.high_low_timeline["leader_missile_launched"] = True
                self.high_low_timeline["leader_attack_phase"] = current_time
                logging.info(f"🎯 A0100长机先行发射: 阶段={self.current_phase.value}, 距离={distance/1000:.1f}km, 时间={current_time:.1f}s")

        elif agent_id == "A0200":  # 僚机：高空攻击者，滞后发射
            # 僚机必须等待长机先发射，且有足够的时间间隔
            leader_launched = self.high_low_timeline.get("leader_missile_launched", False)
            leader_attack_time = self.high_low_timeline.get("leader_attack_phase")
            ...
            if leader_launched and leader_attack_time:
                ...
                if time_since_leader_attack >= self.high_low_timeline["wingman_attack_delay"]:
                    # 僚机发射阶段和距离
                    in_launch_phase = self.current_phase in [TacticalPhase.TR_DOR, TacticalPhase.DOR_DR]
                    in_launch_range = 30000 <= distance <= 45000  # 僚机从高空俯冲，距离可以更近
                    ...
                    if should_launch:
                        logging.info(f"🎯 A0200僚机俯冲攻击发射: 阶段={self.current_phase.value}, "
                                   f"距离={distance/1000:.1f}km, 延迟={time_since_leader_attack:.1f}s")
                    else:
                        logging.debug(f"A0200僚机等待发射: 阶段={self.current_phase.value}, "
                                    f"距离={distance/1000:.1f}km, 延迟={time_since_leader_attack:.1f}s")
                else:
                    logging.debug(f"A0200僚机等待攻击延迟: {time_since_leader_attack:.1f}s < {self.high_low_timeline['wingman_attack_delay']}s")
                    should_launch = False
            else:
