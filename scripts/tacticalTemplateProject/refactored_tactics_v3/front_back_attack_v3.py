...
class FrontBackAttackFinalTask(BaseTacticalTask):
    """
    前后攻击战术任务 - 完全基于拖曳射击的成功架构
    ...
    def __init__(self, config):
        ...
        self.high_low_states = {
            "A0100": {  # 长机 - 保持较低高度
                "target_altitude": 6096.0,  # 保持初始高度
                "altitude_established": True,
                "rtb_direction": "left",
                "role": "low_attacker"  # 低空攻击者
            },
            "A0200": {  # 僚机 - 爬升到高度优势
                "target_altitude": 7296.0,  # 高度优势1200m
                "altitude_established": False,
                "rtb_direction": "right",
                "role": "high_attacker"  # 高空攻击者
            }
        }

        # 前后攻击时间线管理
        self.front_back_timeline = {
            "leader_attack_phase": None,  # 长机攻击阶段
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        # 处理前后攻击战术动作
        result = self._process_front_back_tactics(env, agent_id, current_time)
    ...
    def _process_front_back_tactics(self, env, agent_id, current_time):
        """处理前后攻击战术 - 学习pure_maneuver_task的处理模式"""
        try:
            # 更新战术阶段
    ...
    def _get_leader_command_indices(self, env, agent_id: str):
        ...
        if self.current_phase in [TacticalPhase.NLT_MELD, TacticalPhase.MELD_MTR]:
            ...
            # 长机先行攻击阶段
            self.front_back_timeline["leader_attack_phase"] = current_time

            # 如果已经发射过导弹，执行脱离机动 - 修复触发延迟
            last_launch = self.last_missile_launch_time.get(agent_id, -999)
            if last_launch > 0 and (current_time - last_launch) > 3.0:  # 降低到3秒
                logging.info(f"🔄 {agent_id}长机导弹发射后脱离: 发射时间={last_launch:.1f}s, 当前时间={current_time:.1f}s, 延迟={(current_time - last_launch):.1f}s")
                return self._execute_short_skate_precise(env, agent_id, current_time)
            else:
                # 继续平稳接敌
    ...
    def _get_wingman_command_indices(self, env, agent_id: str):
        ...
        if leader_blue and leader_blue.is_alive:
            ...
            # 调试信息
            if current_time % 15.0 < 0.2:  # 每15秒打印一次
        ...
        if wingman_phase in [TacticalPhase.NLT_MELD, TacticalPhase.MELD_MTR]:
            # 僚机建立后方队形
            if not self.formation_established:
                # 队形建立阶段：清除任何可能的short_skate状态
                if agent_id in self.short_skate_states:
                    logging.info(f"🔧 {agent_id}僚机队形建立期间清除short_skate状态")
            ...
            # 僚机在TR_DOR阶段：此时长机已经在攻击，僚机准备攻击
            last_launch = self.last_missile_launch_time.get(agent_id, -999)
            if last_launch > 0 and (current_time - last_launch) > 3.0:
                # 僚机发射导弹后执行右侧脱离机动
                logging.info(f"🔄 {agent_id}僚机导弹发射后右侧脱离: 发射时间={last_launch:.1f}s, 当前时间={current_time:.1f}s")
                return self._execute_short_skate_precise(env, agent_id, current_time)
            else:
                # 僚机继续保持后方队形，准备发射导弹
                return self._maintain_rear_formation(env, agent_id)
    ...
    def _establish_rear_formation(self, env, agent_id: str):
        ...
        if abs(distance_error) > formation_tolerance:
            ...
            # 队形建立成功
            if not self.formation_established:
                self.formation_established = True
    ...
    def _init_short_skate(self, agent_id, current_time):
        """初始化short_skate机动状态 - 修复前后攻击战术返航方向"""
        # 前后攻击战术专用返航方向：长机左侧，僚机右侧
        if agent_id == "A0100":  # 长机：左侧返航
            crank_angle = -40.0
            turn_cold_angle = -100.0
            logging.info(f"🔄 {agent_id}长机初始化左侧返航: crank={crank_angle}°, turn_cold={turn_cold_angle}°")
        elif agent_id == "A0200":  # 僚机：右侧返航
            crank_angle = 40.0
            turn_cold_angle = 100.0
            logging.info(f"🔄 {agent_id}僚机初始化右侧返航: crank={crank_angle}°, turn_cold={turn_cold_angle}°")
        else:  # 敌方：镜像我方
            crank_angle = 40.0 if agent_id.startswith('B') else -40.0
            turn_cold_angle = 100.0 if agent_id.startswith('B') else -100.0
    ...
    def _execute_short_skate_precise(self, env, agent_id, current_time):
        ...
        if state["initial_heading"] is None:
            ...
            logging.info(f"🔄 {agent_id} Short Skate初始化: 初始航向={current_heading:.1f}°, 初始高度={current_altitude:.0f}m")

        phase_time = current_time - state["phase_start_time"]

        # 高度变化调试信息
        if state["initial_altitude"] is not None:
            altitude_change = current_altitude - state["initial_altitude"]
            if abs(altitude_change) > 200 and current_time % 10.0 < 0.2:  # 每10秒检查一次大幅高度变化
                logging.warning(f"⚠️ {agent_id} Short Skate高度异常变化: 初始={state['initial_altitude']:.0f}m, "
                              f"当前={current_altitude:.0f}m, 变化={altitude_change:+.0f}m, 阶段={state['phase']}")
    ...
    def _maintain_heading_with_climb(self, env, agent_id, target_heading, target_altitude):
        ...
        # 高度控制 - 温和爬升
        altitude_diff = target_altitude - current_altitude
        if altitude_diff > 300:  # 需要明显爬升
            altitude_cmd_id = 9  # +150m 温和爬升
        elif altitude_diff > 100:   # 需要微调爬升
            altitude_cmd_id = 8  # +50m 微调爬升
        elif altitude_diff < -300: # 需要明显下降
            altitude_cmd_id = 5  # -150m 温和下降
        elif altitude_diff < -100:  # 需要微调下降
        ...
        # 速度控制 - 爬升时稍微加速
        velocity_cmd_id = 4 if altitude_diff > 200 else 3
    ...
    def _handle_missile_launch(self, env, agent_id: str, current_time: float):
        ...
        if agent_id == "A0100":  # 长机：前方攻击者，先行发射
            ...
            in_launch_range = 35000 <= distance <= 50000  # 35-50km发射距离
            ...
            if should_launch:
                # 记录长机攻击时间 - 修复时间线记录
                current_time = env.current_step * env.time_interval
                self.front_back_timeline["leader_missile_launched"] = True
                self.front_back_timeline["leader_attack_phase"] = current_time
                logging.info(f"🎯 A0100长机先行发射: 阶段={self.current_phase.value}, 距离={distance/1000:.1f}km, 时间={current_time:.1f}s")
                logging.info(f"📝 长机发射状态更新: leader_missile_launched={self.front_back_timeline['leader_missile_launched']}, leader_attack_phase={self.front_back_timeline['leader_attack_phase']}")

        elif agent_id == "A0200":  # 僚机：后方攻击者，延迟发射
            # 简化僚机发射逻辑 - 修复发射问题
            leader_launched = self.front_back_timeline.get("leader_missile_launched", False)
            leader_attack_time = self.front_back_timeline.get("leader_attack_phase")

            # 僚机发射条件：基于长机发射时间记录
            leader_last_launch = self.last_missile_launch_time.get("A0100", -999)

            if leader_last_launch > 0:  # 长机已经发射过导弹
                time_since_leader_launch = current_time - leader_last_launch
                if time_since_leader_launch >= 5.0:  # 长机发射5秒后，僚机可以发射
                    # 僚机发射阶段和距离
                    in_launch_phase = self.current_phase in [TacticalPhase.TR_DOR, TacticalPhase.DOR_DR]
                    in_launch_range = 20000 <= distance <= 45000  # 僚机发射距离

                    should_launch = in_launch_phase and in_launch_range

                    if should_launch:
                        self.front_back_timeline["wingman_attack_ready"] = True
                        logging.info(f"🎯 A0200僚机后方攻击发射: 阶段={self.current_phase.value}, "
                                   f"距离={distance/1000:.1f}km, 长机发射后延迟={time_since_leader_launch:.1f}s")
                    else:
                        logging.debug(f"A0200僚机等待发射: 阶段={self.current_phase.value}, "
                                    f"距离={distance/1000:.1f}km, 长机发射后延迟={time_since_leader_launch:.1f}s")
                else:
                    logging.debug(f"A0200僚机等待攻击延迟: {time_since_leader_launch:.1f}s < 5.0s")
                    should_launch = False
            else:
                logging.debug(f"A0200僚机等待长机先发射: 长机发射时间={leader_last_launch}")
                should_launch = False
    ...
    def _launch_missile(self, env, agent_id: str, target, current_time: float):
        ...
        try:
            ...
            logging.info(f"🚀 {agent_id} 导弹发射记录: 时间={current_time:.1f}s, 导弹={missile_uid}, 类型={missile_type}")
    ...
    def _establish_high_altitude_advantage(self, env, agent_id: str):
        """僚机建立高度优势"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        current_heading = np.rad2deg(env.agents[agent_id].get_property_value(c.attitude_psi_rad))

        target_altitude = self.high_low_states[agent_id]["target_altitude"]
        altitude_established = abs(current_altitude - target_altitude) < 100  # 100m容差
        self.high_low_states[agent_id]["altitude_established"] = altitude_established
        ...
        if not altitude_established:
            # 需要爬升 - 积极爬升到高度优势
            altitude_diff = target_altitude - current_altitude
            if altitude_diff > 500:  # 需要大幅爬升
                altitude_cmd_id = 11  # +300m 积极爬升
                velocity_cmd_id = 4   # 稍微加速辅助爬升
            elif altitude_diff > 200:  # 需要中等爬升
                altitude_cmd_id = 10  # +150m 中等爬升
            ...
            if abs(heading_diff) > 2.0:  # 爬升时航向控制稍微宽松
                heading_cmd_id = self._convert_heading_to_index(np.deg2rad(heading_diff))
        else:
            # 高度已建立，执行右侧crank机动建立编队间距
            target_heading = 30.0  # 右偏30度建立间距
            heading_diff = target_heading - current_heading
            while heading_diff > 180: heading_diff -= 360
            while heading_diff < -180: heading_diff += 360
    ...
    def _maintain_high_altitude_position(self, env, agent_id: str, target_heading: float):
        """僚机保持高度优势位置"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        current_heading = np.rad2deg(env.agents[agent_id].get_property_value(c.attitude_psi_rad))
    ...
    def _execute_dive_attack(self, env, agent_id: str):
        ...
        # 默认索引
        altitude_cmd_id = 7  # 保持高度（俯冲攻击时暂时保持高度优势）
        heading_cmd_id = 8   # 保持航向
