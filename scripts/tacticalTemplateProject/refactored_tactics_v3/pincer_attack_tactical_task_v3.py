...
重构说明：
- 继承 BaseTacticalTask 获得基础设施（雷达、RWR、模型）
- 保留完整的战术逻辑
...
class PincerAttackTacticalTask(BaseTacticalTask):
    """
    ...
    def __init__(self, config):
        ...
        # 注意：initial_heading和initial_altitude已在基类中初始化
    ...
    def _update_tactical_phase(self, env):
        ...
        if new_phase != self.current_phase:
            ...
            phase_priority = {
                ...
                TacticalPhase.MTR_TR: 3,
                TacticalPhase.TR_DOR: 4,
                TacticalPhase.DOR_DR: 5
            }
            
            current_priority = phase_priority.get(self.current_phase, 0)
            new_priority = phase_priority.get(new_phase, 0)
            
            # 如果新阶段优先级低于当前阶段（即回退），且当前阶段>=TR_DOR，则拒绝回退
            if new_priority < current_priority and current_priority >= 4:  # TR_DOR及以后不允许回退
                logging.error(f"🔒 阶段锁定: 拒绝从{self.current_phase.value}回退到{new_phase.value}（距离={distance/1000:.1f}km）")
                return  # 直接返回，不切换阶段
    ...
    def _get_friendly_leader_command_indices(self, env, agent_id):
        ...
        if self.current_phase == TacticalPhase.NLT_MELD:
            # 阶段1 (90-81km): 长机执行LEFT crank机动 - 固定315°航向（西北方向）
            # 🔥 紧急修复：完全使用固定绝对航向，与僚机对称
            # 僚机使用45°（东北），长机使用315°（西北），形成90°夹角钳形
            return self._maintain_heading_precise(env, agent_id, -100.0)
    ...
    def _get_friendly_wingman_command_indices(self, env, agent_id):
        """友方僚机钳形夹击指令"""
        if self.current_phase == TacticalPhase.NLT_MELD:
            # 阶段1 (90-81km): 僚机执行RIGHT crank机动 - 固定45°航向（东北方向）
            # 使用固定绝对航向，确保展开效果
    ...
    # ========== 精确航向保持和机动方法 ==========
    def _execute_crank_offset(self, env, agent_id, offset_degrees):
        """
        执行Crank机动偏置（相对于敌机方向）
        ...
        Args:
            env: 环境
            agent_id: 飞机ID
            offset_degrees: 相对于敌机方向的偏置角度（正值右偏，负值左偏）
        
        Returns:
            (altitude_cmd, heading_cmd, velocity_cmd)
        """
        # 获取敌机位置
        enemy_id = "B0100" if "B0100" in env._jsbsims and env._jsbsims["B0100"].is_alive else "B0200"
        if enemy_id not in env._jsbsims or not env._jsbsims[enemy_id].is_alive:
            # 敌机不存在，使用默认北向
            return self._maintain_heading_precise(env, agent_id, 0.0)
        
        # 计算朝向敌机的方位角
        my_pos = env.agents[agent_id].get_position()
        enemy_pos = env._jsbsims[enemy_id].get_position()
        ...
        # 归一化到0-360度
        while bearing_to_enemy < 0:
            bearing_to_enemy += 360
        while bearing_to_enemy >= 360:
            bearing_to_enemy -= 360
        
        # 目标航向 = 朝向敌机的方位 + 偏置角度
        target_heading = bearing_to_enemy + offset_degrees
