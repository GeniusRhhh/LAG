#!/usr/bin/env python3
"""
上下夹击战术任务 - 基于拖曳射击项目的成功架构
完全复制拖曳射击的所有成功组件，只修改战术逻辑部分
"""
...
class HighLowAttackTermination(BaseTerminationCondition):
    """上下夹击专用终止条件 - 只有双方全灭才终止"""
...
class HighLowAttackTacticalTask(BaseTacticalTask):
    """
    上下夹击战术任务 - 基于拖曳射击项目的成功架构
    ...
    主要函数调用链：
    normalize_action() → _process_high_low_tactics() → _get_tactical_command_indices()
    ...
    def __init__(self, config):
        """初始化上下夹击任务 - 基于拖曳射击的成功架构"""
        super().__init__(config)

        # 使用自定义终止条件 - 完全替换父类的终止条件
        self.termination_conditions = [
            HighLowAttackTermination(self.config),
        ]
        ...
        # 上下夹击特有：高度管理状态
        # 🔥 修复：增大高度差以形成更明显的上下夹击态势
        self.high_low_states = {
            "A0100": {  # 长机：低空攻击者
                "role": "low_attacker",
                "target_altitude": 5500.0,  # 保持相对低空
                "altitude_established": False,
            },
            "A0200": {  # 僚机：高空攻击者
                "role": "high_attacker",
                "target_altitude": 8500.0,  # 高度优势（3000m差）- 增大高度差
                "altitude_established": False,
            }
        }
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        # 处理上下夹击战术动作
        result = self._process_high_low_tactics(env, agent_id, current_time)
    ...
    def _process_high_low_tactics(self, env, agent_id, current_time):
        """处理上下夹击战术 - 基于拖曳射击的成功架构"""
        try:
            # 更新战术阶段
            ...
            logging.error(f"{agent_id} 上下夹击战术执行错误: {e}")
            return np.array([0.0, 0.0, 0.0, 0.7])
    ...
    def _get_leader_command_indices(self, env, agent_id: str):
        ...
        if self.current_phase in [TacticalPhase.NLT_MELD, TacticalPhase.MELD_MTR, TacticalPhase.MTR_TR]:
            # 上下夹击：长机在前三个阶段都平稳飞行，保持低空
            return self._maintain_low_altitude_approach(env, agent_id, 0.0)

        elif self.current_phase == TacticalPhase.TR_DOR:
            # 上下夹击：长机在MTR-TR阶段末期发射导弹，然后左侧返航
            current_time = env.current_step * env.time_interval
            ...
            if last_launch > 0 and (current_time - last_launch) > 5.0:
                ...
                return self._execute_short_skate_precise_with_side(env, agent_id, current_time, side="left")
            else:
                # 继续低空平稳飞行等待发射时机
                return self._maintain_low_altitude_approach(env, agent_id, 0.0)
    ...
    def _get_wingman_command_indices(self, env, agent_id: str):
        ...
        if leader_blue and leader_blue.is_alive:
            ...
            logging.info(f"🔍 {agent_id}僚机独立时间线: 距离={distance/1000:.1f}km, 僚机阶段={wingman_phase.value}, 全局阶段={self.current_phase.value}")
        else:
            wingman_phase = self.current_phase  # 如果无法计算距离，使用全局阶段

        if wingman_phase == TacticalPhase.NLT_MELD:
            # 🔥 修复：上下夹击僚机在NLT_MELD阶段就开始爬升建立高度优势
            # 同时保持横向分离
            return self._execute_early_climb_with_spacing(env, agent_id)

        elif wingman_phase == TacticalPhase.MELD_MTR:
            # 上下夹击：僚机爬升高度并保持横向分离
            return self._execute_climb_with_spacing(env, agent_id)

        elif wingman_phase == TacticalPhase.MTR_TR:
            # 上下夹击：僚机保持高度优势和横向分离
            return self._maintain_altitude_and_spacing(env, agent_id)
            ...
            if last_launch > 0 and (current_time - last_launch) > 5.0:
                ...
                return self._execute_short_skate_precise_with_side(env, agent_id, current_time, side="right")
            else:
                # 俯冲攻击
                return self._execute_dive_attack(env, agent_id)

        elif wingman_phase == TacticalPhase.DOR_DR:
            # DOR_DR阶段：僚机执行右侧short_skate机动
            current_time = env.current_step * env.time_interval
            return self._execute_short_skate_precise_with_side(env, agent_id, current_time, side="right")
        else:
            return 7, 8, 3
    ...
    def _maintain_heading_precise(self, env, agent_id, target_heading, duration=10.0):
        ...
        # 针对友方飞机（A0100和A0200）的超精确航向控制（目标0°北向）
        if agent_id.startswith("A") and target_heading == 0.0:
            # 超精确控制：0.5度精度，确保友方飞机保持完全一致的正北向
    ...
    def _maintain_low_altitude_approach(self, env, agent_id: str, target_heading: float):
        """长机保持低空接敌"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        ...
        # 高度控制
        altitude_diff = target_altitude - current_altitude
        if abs(altitude_diff) > 100.0:  # 100m容差
            altitude_cmd_id = self._convert_altitude_to_index(altitude_diff)
        else:
            altitude_cmd_id = 7  # 保持高度

        # 航向控制
        heading_cmd_id = self._get_heading_command(env, agent_id, target_heading)

        return altitude_cmd_id, heading_cmd_id, 3  # 保持速度

    def _execute_climb_maneuver(self, env, agent_id: str):
        """执行爬升机动 - 僚机在MELD_MTR阶段爬升，同时保持与长机相同的北向航向"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        target_altitude = self.high_low_states[agent_id]["target_altitude"]
        ...
        if altitude_diff > 500:
            altitude_cmd_id = 13  # +1000m 大幅爬升
        elif altitude_diff > 200:
            altitude_cmd_id = 11  # +300m 中等爬升
        elif altitude_diff > 50:
            altitude_cmd_id = 9   # +50m 微调爬升
        else:
            altitude_cmd_id = 7   # 保持高度
            self.high_low_states[agent_id]["altitude_established"] = True
            logging.info(f"✅ {agent_id}僚机高度优势建立完成: {current_altitude:.0f}m")

        # 航向控制：爬升时必须与长机保持相同的北向航向（0°）
        target_heading = 0.0  # 北向
        heading_cmd_id = self._get_heading_command(env, agent_id, target_heading)

        return altitude_cmd_id, heading_cmd_id, 3  # 爬升，航向同步到北向，保持速度

    def _maintain_altitude_advantage(self, env, agent_id: str):
        """保持高度优势 - 僚机在MTR_TR阶段，同时与长机保持相同航向"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        target_altitude = self.high_low_states[agent_id]["target_altitude"]
        ...
        return altitude_cmd_id, heading_cmd_id, 3  # 高度调整，航向同步，保持速度

    def _execute_dive_attack(self, env, agent_id: str):
        """执行俯冲攻击 - 僚机在TR_DOR阶段，保持与长机相同航向"""
        # 航向控制：俯冲时必须与长机保持相同的北向航向（0°）
        target_heading = 0.0  # 北向
        heading_cmd_id = self._get_heading_command(env, agent_id, target_heading)

        # 俯冲攻击：降低高度，增加速度，保持航向同步
        return 2, heading_cmd_id, 5  # 下降750m，航向同步到北向，增加速度

    def _execute_short_skate_precise_with_side(self, env, agent_id: str, current_time: float, side: str = "left"):
        """执行带方向的short_skate机动"""
        if agent_id not in self.short_skate_states:
            self._init_short_skate_with_side(agent_id, current_time, side)

        # 调用原有的short_skate执行逻辑
        return self._execute_short_skate_precise(env, agent_id, current_time)

    def _init_short_skate_with_side(self, agent_id, current_time, side):
        """初始化带方向的short_skate机动状态"""
        # 根据side参数设置转弯方向
        if side == "left":
            crank_angle = -40.0   # 左转
            turn_cold_angle = -100.0  # 左转
        else:  # right
            crank_angle = 40.0    # 右转
            turn_cold_angle = 100.0   # 右转
    ...
    def _convert_altitude_to_index(self, altitude_diff: float):
        """将高度差转换为指令索引"""
        if altitude_diff > 750:
            return 14  # +1500m
        elif altitude_diff > 500:
            return 13  # +1000m
        elif altitude_diff > 300:
            return 12  # +750m
        elif altitude_diff > 150:
            return 11  # +500m
        elif altitude_diff > 50:
            return 10  # +300m
            ...
            return 2   # -750m
        elif altitude_diff > -750:
            return 1   # -1000m
        else:
            return 0   # -1500m

    def _get_heading_command(self, env, agent_id: str, target_heading: float):
        """获取航向指令"""
        current_heading = np.rad2deg(env.agents[agent_id].get_property_value(c.attitude_psi_rad))
        ...
        # 转换为指令索引
        if abs(heading_diff) < 1.0:  # 1度容差
            return 8  # 保持航向
        else:
            return self._convert_heading_to_index(np.deg2rad(heading_diff))
    ...
    def _maintain_formation_spacing(self, env, agent_id: str, offset_heading: float):
        """保持编队间距 - 僚机保持轻微横向偏移以形成并排编队"""
        current_time = env.current_step * env.time_interval

        # 计算与长机的横向距离
        leader_pos = env.agents["A0100"].get_position()
        wingman_pos = env.agents[agent_id].get_position()

        # 计算横向分离距离（Y轴方向）
        lateral_separation = abs(wingman_pos[1] - leader_pos[1])

        # 目标横向分离距离：5000米（5公里）- 确保俯视图清晰可见
        target_separation = 5000.0

        # 根据当前分离距离调整航向（更宽松的阈值以适应5km目标）
        if lateral_separation < target_separation * 0.7:  # 距离太近，需要增加分离
            # 僚机向右偏航以增加横向分离
            target_heading = offset_heading  # 右偏10度
            logging.debug(f"{agent_id} 增加横向分离: 当前分离={lateral_separation:.0f}m, 目标={target_heading:.1f}°")
        elif lateral_separation > target_separation * 1.3:  # 距离太远，需要减少分离
            # 僚机向左偏航以减少横向分离
            target_heading = -offset_heading * 0.3  # 左偏3度
            logging.debug(f"{agent_id} 减少横向分离: 当前分离={lateral_separation:.0f}m, 目标={target_heading:.1f}°")
        else:
            # 分离距离合适，保持北向
            target_heading = 0.0
            logging.debug(f"{agent_id} 保持编队: 当前分离={lateral_separation:.0f}m, 目标={target_heading:.1f}°")

        # 使用精确航向控制
        return self._maintain_heading_precise(env, agent_id, target_heading)

    def _execute_early_climb_with_spacing(self, env, agent_id: str):
        """执行早期爬升机动并保持编队间距 - 僚机在NLT_MELD阶段"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        target_altitude = self.high_low_states[agent_id]["target_altitude"]

        # 🔥 早期爬升：更激进的高度控制，快速建立高度优势
        altitude_diff = target_altitude - current_altitude
        if altitude_diff > 1500:
            altitude_cmd_id = 14  # +1500m 极大幅爬升（如果有的话）
        ...
        # 根据横向分离调整航向
        if lateral_separation < target_separation * 0.6:
            # 需要增加分离，右偏
            target_heading = 12.0  # 早期阶段更大右偏以快速建立分离
        elif lateral_separation > target_separation * 1.4:
            # 需要减少分离，左偏
            target_heading = -3.0  # 轻微左偏
        else:
            # 分离合适，保持北向
            target_heading = 0.0
        ...
        return altitude_cmd_id, heading_cmd_id, 3  # 爬升，航向调整，保持速度
    
    def _execute_climb_with_spacing(self, env, agent_id: str):
        """执行爬升机动并保持编队间距 - 僚机在MELD_MTR阶段"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        target_altitude = self.high_low_states[agent_id]["target_altitude"]

        # 🔥 修复：更激进的高度控制，快速建立高度优势
        altitude_diff = target_altitude - current_altitude
        if altitude_diff > 1000:
            ...
            altitude_cmd_id = 7   # 保持高度
            self.high_low_states[agent_id]["altitude_established"] = True
            if env.current_step % 100 == 0:  # 降低日志频率
                logging.info(f"✅ {agent_id}僚机高度优势建立完成: {current_altitude:.0f}m")
        ...
        # 根据横向分离调整航向（适应5km目标）
        if lateral_separation < target_separation * 0.7:
            # 需要增加分离，右偏
            target_heading = 8.0  # 爬升时较大右偏以快速建立分离
        elif lateral_separation > target_separation * 1.3:
            # 需要减少分离，左偏
            target_heading = -2.0  # 爬升时轻微左偏
        else:
            # 分离合适，保持北向
            target_heading = 0.0
    ...
    def _maintain_altitude_and_spacing(self, env, agent_id: str):
        """保持高度优势和横向分离 - 僚机在MTR_TR阶段"""
        current_altitude = env.agents[agent_id].get_property_value(c.position_h_sl_m)
        target_altitude = self.high_low_states[agent_id]["target_altitude"]
        ...
        # 根据横向分离调整航向（适应5km目标）
        if lateral_separation < target_separation * 0.8:
            # 需要增加分离，右偏
            target_heading = 4.0  # 中等右偏
        elif lateral_separation > target_separation * 1.2:
            # 需要减少分离，左偏
            target_heading = -2.0  # 轻微左偏
        else:
            # 分离合适，保持北向
            target_heading = 0.0
        ...
        return altitude_cmd_id, heading_cmd_id, 3  # 高度调整，航向调整，保持速度
