...
"""
拖曳射击战术项目专用敌方AI适配器
基于统一敌方战术AI系统，为拖曳射击项目提供专门的敌方AI集成
"""
...
def create_drag_shoot_enemy_ai_integration(tactical_task) -> Optional[EnemyAIAdapter]:
    """
    为拖曳射击战术任务创建统一敌方AI系统集成
    
    Args:
        tactical_task: 拖曳射击战术任务实例
    ...
    try:
        # 创建统一敌方AI适配器
        enemy_ai_adapter = EnemyAIAdapter(project_name="drag_shoot_tactical")
        ...
        if hasattr(tactical_task.unified_enemy_ai, 'set_project_config'):
            project_config = {
                'project_name': 'drag_shoot_tactical',
        ...
        if hasattr(tactical_task, 'unified_enemy_ai') and tactical_task.unified_enemy_ai is not None:
            ...
            logging.error("❌ 拖曳射击统一敌方AI系统集成验证失败")
            return None
            
    except Exception as e:
        logging.error(f"❌ 拖曳射击统一敌方AI系统集成失败: {e}")
        import traceback
        logging.error(f"详细错误信息: {traceback.format_exc()}")
        return None


def verify_drag_shoot_integration(tactical_task) -> bool:
    """
    验证拖曳射击项目的统一敌方AI系统集成状态
    ...
    try:
        ...
        logging.info("✅ 拖曳射击统一敌方AI系统集成验证通过")
        return True
        
    except Exception as e:
        logging.error(f"❌ 拖曳射击集成验证失败: {e}")
        return False
...
if __name__ == "__main__":
    ...
    adapter = create_drag_shoot_enemy_ai_integration(mock_task)
    
    if adapter:
        print("✅ 适配器创建成功")
        success = verify_drag_shoot_integration(mock_task)
