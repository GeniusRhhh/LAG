...
class DragShootTacticalTask(MultipleCombatTask):
    ...
    def _use_lowlevel_policy(self, env, agent_id, altitude_cmd_id, heading_cmd_id, velocity_cmd_id):
        ...
        try:
            ...
            # 🔥 速度补偿：动态调整油门防止速度过低导致坠毁
            try:
                current_velocity = np.linalg.norm(env.agents[agent_id].get_velocity())
                if current_velocity < 220.0:
                    norm_act[3] = max(norm_act[3], 1.0)
                elif current_velocity < 260.0:
                    norm_act[3] = max(norm_act[3], 0.95)
                elif current_velocity < 300.0:
                    norm_act[3] = max(norm_act[3], 0.88)
                elif current_velocity < 340.0:
                    norm_act[3] = max(norm_act[3], 0.82)
            except Exception:
                pass
