...
class SideBySideEnemyAIAdapter:
    """并排射击战术仿真的敌方AI适配器"""
    
    def __init__(self, project_name: str = "side_by_side_shooting"):
        """
        初始化并排射击敌方AI适配器
        ...
        logging.info(f"并排射击敌方AI适配器初始化完成 - 项目: {project_name}")
    ...
    def reset_for_new_episode(self):
        ...
        try:
            ...
            logging.info("并排射击敌方AI适配器已重置")
        except Exception as e:
            logging.error(f"并排射击敌方AI适配器重置失败: {e}")
    ...
    def get_enemy_action(self, env, agent_id: str, current_time: float):
        ...
        try:
            ...
            logging.error(f"并排射击敌方{agent_id}战术指令生成失败: {e}")
            # 返回默认平稳飞行指令
            return 7, 8, 3
    ...
    def _update_action_annotation(self, agent_id: str, current_time: float):
        ...
        try:
            ...
            logging.error(f"并排射击行动注释更新失败 {agent_id}: {e}")
    ...
    def get_action_annotation_for_csv(self, agent_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            logging.error(f"并排射击敌方{agent_id}行动注释获取失败: {e}")
    ...
    def get_action_type_for_csv(self, agent_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            logging.error(f"并排射击敌方{agent_id}CSV动作类型获取失败: {e}")
            return {
                'action_type': ''
            }
...
def create_side_by_side_enemy_ai_integration(tactical_task):
    """为并排射击战术任务创建敌方AI集成"""
    try:
        # 创建适配器
        adapter = SideBySideEnemyAIAdapter("side_by_side_shooting")
        
        # 将适配器附加到战术任务
        tactical_task.unified_enemy_ai = adapter
        
        logging.info("并排射击统一敌方AI系统集成完成")
        return adapter
        
    except Exception as e:
        logging.error(f"并排射击敌方AI集成失败: {e}")
        return None
