...
class JSBSimRunner(Runner):
    @staticmethod
    def _safe_average_episode_reward(buffer) -> float:
        done_count = int((buffer.masks == False).sum())
        if done_count <= 0:
            return float(buffer.rewards.mean())
        return float(buffer.rewards.sum() / done_count)
    ...
    def run(self):
        ...
        log_file = str(self.save_dir) + '/training_log.txt'
        
        # 记录训练开始信息
        with open(log_file, 'w', encoding='utf-8') as f:
            f.write(f"{self.all_args.scenario_name} 训练日志\n")
            f.write(f"开始时间: {training_start_time.isoformat()}\n")
            f.write(f"场景: {self.all_args.scenario_name}\n")
            f.write(f"实验名: {self.experiment_name}\n")
            f.write(f"总训练步数: {self.num_env_steps:,}\n")
            f.write(f"并行环境数: {self.n_rollout_threads}\n")
            f.write(f"学习率: {self.all_args.lr}\n")
            f.write(f"网络结构: {self.all_args.hidden_size}\n")
            f.write("="*60 + "\n\n")
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                avg_reward = self._safe_average_episode_reward(self.buffer)
                ...
                logging.info("\n [train] 进度: {:.1f}% ({}/{})\n"
                             ...
                             "   步数: {:,}/{:,} | FPS: {}\n"
                             "   平均奖励: {:.4f}\n"
                             .format(progress_pct, episode, episodes,
                                     self.all_args.scenario_name, self.experiment_name,
                                     self.total_num_steps, self.num_env_steps, fps,
                                     avg_reward))
                ...
                if len(heading_turns_list):
                    ...
                    logging.info("   航向转弯次数: {:.2f}".format(avg_turns))
                
                # 写入详细日志文件
                log_entry = {
                    'episode': episode,
                    'progress_pct': progress_pct,
                    'total_timesteps': self.total_num_steps,
                    'fps': fps,
                    'average_reward': float(avg_reward),
                    'average_heading_turns': float(np.mean(heading_turns_list)) if heading_turns_list else 0,
                    'timestamp': datetime.datetime.now().isoformat()
                }
                
                with open(str(self.save_dir) + '/training_log.txt', 'a', encoding='utf-8') as f:
                    f.write(f"Episode {episode:4d} | 进度: {progress_pct:5.1f}% | "
                           f"奖励: {avg_reward:8.4f} | FPS: {fps:4d} | "
    ...
    def save(self, episode):
        ...
        if episode % 5 == 0:
            ...
            logging.info(f"✅ 检查点已保存: episode_{episode}")
        ...
        with open(str(self.save_dir) + '/training_progress.json', 'w') as f:
