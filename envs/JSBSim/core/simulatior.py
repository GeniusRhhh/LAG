...
class MissileSimulator(BaseSimulator):
    ...
    def __init__(self,
        ...
        self._fuel_mass = 40.0  # 燃料质量 kg
        self._dm = self._fuel_mass / self._t_boost  # 质量损失率 kg/s
        self._thrust = 15000  # 推力 N
    ...
    def target(self, target: AircraftSimulator):
        ...
        self.frozen_target_id = self.launch_target_id
        self.frozen_target_uid = self.launch_target_uid
        self.frozen_target_real_id = self.launch_target_real_id
    ...
    def run(self):
        ...
        target_alive = bool(getattr(self.target_aircraft, 'is_alive', False))
        if (not target_alive) and distance > max(self._Rc * 1.25, 80.0):
            self.__status = MissileSimulator.MISS
            if not hasattr(self, '_completion_time'):
                self._completion_time = self._t
                logging.debug(f"Missile {self.uid} completed at t={self._completion_time:.1f}s")
    ...
    def _should_miss(self):
        ...
        if (not self.target_aircraft.is_alive) and distance > max(self._Rc * 1.25, 80.0):
            return True
