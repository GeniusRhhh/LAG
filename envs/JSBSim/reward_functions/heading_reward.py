...
class HeadingReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id):
        ...
        heading_r = math.exp(-((delta_heading / 15.0) ** 2))
        ...
        # 增加姿态惩罚
        pitch_penalty = -0.1 if abs(pitch_rad) > 0.35 else 0.0
        roll_penalty = -0.1 if abs(roll_rad) > 0.35 else 0.0

        reward = (heading_r * alt_r * roll_r * pitch_r * speed_r) ** (
                    1 / 5) + heading_bonus + pitch_penalty + roll_penalty
