...
class AltitudeReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id):
        ...
        PH = -0.1 * (1.0 - np.clip(ego_z / self.danger_altitude, 0.0, 1.0)) if ego_z <= self.danger_altitude else 0.0
        Ptarget = 1.0 * np.exp(-((ego_z - self.target_altitude) ** 2) / 0.5)  # 从1.5收紧到0.5，要求更精确的高度控制
        Pvz = 0.0  # 暂时移除垂直速度惩罚
