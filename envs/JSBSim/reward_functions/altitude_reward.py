...
class AltitudeReward(BaseRewardFunction):
    def __init__(self, config):
        ...
        self.safe_altitude = getattr(self.config, f'{self.__class__.__name__}_safe_altitude', 5.0)  # 4.0 → 5.0
    ...
    def get_reward(self, task, env, agent_id):
        ...
        if ego_z <= self.safe_altitude:
            Pv = -0.1 * np.clip(ego_vz / self.Kv * (self.safe_altitude - ego_z) / self.safe_altitude, 0.0, 1.0)  # 惩罚减半
        PH = 0.0
