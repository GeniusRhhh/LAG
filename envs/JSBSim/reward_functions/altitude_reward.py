...
class AltitudeReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id):
        ...
        PH = -0.1 * (1.0 - np.clip(ego_z / self.danger_altitude, 0.0, 1.0)) if ego_z <= self.danger_altitude else 0.0  # 降低惩罚力度
        Ptarget = 1.0 * np.exp(-((ego_z - self.target_altitude) ** 2) / 1.5)  # 增强目标高度奖励
        Pvz = -0.01 * abs(ego_vz) if abs(ego_vz) > 0.6 else 0.0  # 放宽速度惩罚阈值
