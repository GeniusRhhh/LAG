...
from ..core.simulatior import AircraftSimulator, BaseSimulator, is_leader_agent_id  # 导入核心模拟器类
...
class MultipleCombatEnv(BaseEnv):
    ...
    def load_task(self):
        ...
        # 🔥 关键修复：将 env 绑定到 task，确保 task.num_agents 能读取实际飞机数量。
        # 否则 MultipleCombatTask.num_agents 会回退到 4（2v2），导致 4v4 场景 obs_length 仍为 27。
        try:
            self.task.env = self
        except Exception as e:
            logging.warning(f"Failed to bind env to task: {e}")
    ...
    def load_simulator(self):
        ...
        for agent_id, sim in self._jsbsims.items():
            is_leader = is_leader_agent_id(agent_id)  # 每个双机对的第一架为长机（支持4v4的A0300/B0300）
            sim.set_leader(is_leader)  # 设置长机/僚机状态
            logging.info(f"Initialized Agent {agent_id} as {'Leader' if is_leader else 'Wingman'}")

        # 🔥 修复：环境加载后重新计算观测空间（支持4v4）
        actual_num_agents = len(self._jsbsims)
        if hasattr(self.task, 'load_observation_space'):
            # 确保 task 能访问 env，从而拿到真实 num_agents
            try:
                self.task.env = self
            except Exception:
                pass
            # 重新加载观测空间（会使用动态的num_agents）
            self.task.load_observation_space()
            expected_obs_length = 9 + (actual_num_agents - 1) * 6
            if getattr(self.task, 'obs_length', None) != expected_obs_length:
                logging.warning(
                    f"[观测空间异常] obs_length={getattr(self.task, 'obs_length', None)}, "
                    f"expected={expected_obs_length}, num_agents={actual_num_agents}. Re-loading."
                )
                try:
                    self.task.load_observation_space()
                except Exception as e:
                    logging.error(f"Failed to reload observation space: {e}")
            logging.info(f"[观测空间] 已重新计算: obs_length={self.task.obs_length}, num_agents={actual_num_agents}")
    ...
    def _setup_formation_relationships(self):
        """设置红方和蓝方的编队关系以及敌我关系。支持1v1、2v2和4v4配置"""
        ...
        # 检查是否是4v4配置（CAP模式）
        elif len(available_agents) == 8 and all(
                agent in available_agents for agent in ["A0100", "A0200", "A0300", "A0400", "B0100", "B0200", "B0300", "B0400"]):
            # 4v4配置（CAP模式）
            # 红方编队1（左编队：A0100 和 A0200）
            self._jsbsims["A0100"].partners = [self._jsbsims["A0200"]]
            self._jsbsims["A0100"].enemies = [self._jsbsims["B0100"], self._jsbsims["B0200"], self._jsbsims["B0300"], self._jsbsims["B0400"]]
            self._jsbsims["A0200"].partners = [self._jsbsims["A0100"]]
            self._jsbsims["A0200"].enemies = [self._jsbsims["B0100"], self._jsbsims["B0200"], self._jsbsims["B0300"], self._jsbsims["B0400"]]

            # 红方编队2（右编队：A0300 和 A0400）
            self._jsbsims["A0300"].partners = [self._jsbsims["A0400"]]
            self._jsbsims["A0300"].enemies = [self._jsbsims["B0100"], self._jsbsims["B0200"], self._jsbsims["B0300"], self._jsbsims["B0400"]]
            self._jsbsims["A0400"].partners = [self._jsbsims["A0300"]]
            self._jsbsims["A0400"].enemies = [self._jsbsims["B0100"], self._jsbsims["B0200"], self._jsbsims["B0300"], self._jsbsims["B0400"]]

            # 蓝方编队1（B0100 和 B0200）
            self._jsbsims["B0100"].partners = [self._jsbsims["B0200"]]
            self._jsbsims["B0100"].enemies = [self._jsbsims["A0100"], self._jsbsims["A0200"], self._jsbsims["A0300"], self._jsbsims["A0400"]]
            self._jsbsims["B0200"].partners = [self._jsbsims["B0100"]]
            self._jsbsims["B0200"].enemies = [self._jsbsims["A0100"], self._jsbsims["A0200"], self._jsbsims["A0300"], self._jsbsims["A0400"]]

            # 蓝方编队2（B0300 和 B0400）
            self._jsbsims["B0300"].partners = [self._jsbsims["B0400"]]
            self._jsbsims["B0300"].enemies = [self._jsbsims["A0100"], self._jsbsims["A0200"], self._jsbsims["A0300"], self._jsbsims["A0400"]]
            self._jsbsims["B0400"].partners = [self._jsbsims["B0300"]]
            self._jsbsims["B0400"].enemies = [self._jsbsims["A0100"], self._jsbsims["A0200"], self._jsbsims["A0300"], self._jsbsims["A0400"]]
            logging.info("4v4 formation relationships established (CAP mode)")

        else:
            # 未知配置，报错
            logging.error(f"Unsupported agent configuration: {available_agents}")
            raise ValueError(
                f"Unsupported agent configuration. Expected 1v1 (A0100, B0100), 2v2 (A0100, A0200, B0100, B0200), or 4v4 (A0100, A0200, A0300, A0400, B0100, B0200, B0300, B0400), got: {available_agents}")
    def reset(self) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
        ...
        if hasattr(self.task, 'load_observation_space'):
            try:
                ...
                pass
            self.task.load_observation_space()
            logging.debug(f"[观测空间-重置] obs_length={self.task.obs_length}, num_agents={actual_num_agents}")
        ...
        logging.info(f"Environment reset, current_step={self.current_step}, "
                     ...
                     f"time_interval={self.time_interval}, num_agents={actual_num_agents}")
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
        ...
        if isinstance(action, np.ndarray):
            # 动态获取实际智能体数量（支持2v2和4v4）
            actual_num_agents = len(self._jsbsims)
            logging.debug(f"Step action shape: {action.shape}, n_rollout_threads: {self.n_rollout_threads}, actual_num_agents: {actual_num_agents}, num_agents: {self.num_agents}")
            
            if action.ndim == 2 and action.shape[0] == actual_num_agents and action.shape[1] == self.action_space.shape[0]:
                if self.n_rollout_threads == 1:
                    action = action[np.newaxis, :, :]  # 为单线程扩展维度
                else:
                    raise ValueError(
                        f"Expected shape (n_rollout_threads, {actual_num_agents}, {self.action_space.shape[0]}), got {action.shape}")
            elif action.ndim == 3:
                # 检查动作形状是否匹配实际智能体数量
                if action.shape[1] != actual_num_agents:
                    # 如果动作数量不匹配，尝试调整
                    if action.shape[1] == self.num_agents and actual_num_agents != self.num_agents:
                        # 如果任务定义的num_agents和实际不匹配，使用实际的
                        logging.warning(f"Action shape mismatch: task.num_agents={self.num_agents}, actual={actual_num_agents}, using actual")
                    else:
                        raise ValueError(
                            f"Invalid action shape {action.shape}, expected (n_rollout_threads, {actual_num_agents}, {self.action_space.shape[0]})")
            else:
                raise ValueError(
                    f"Invalid action shape {action.shape}, expected (n_rollout_threads, {actual_num_agents}, {self.action_space.shape[0]})")
        ...
        for agent_id in self._jsbsims.keys():
            ...
            # [DEBUG] 追踪实际使用的任务类型 (已注释 - 太频繁)
            # if agent_id == 'B0100':
            #     logging.info(f"[DEBUG] env.step: task类型={type(self.task).__name__}, hasattr normalize_action={hasattr(self.task, 'normalize_action')}")
        ...
        for uid, sim in self._tempsims.items():
            if hasattr(sim, 'is_done') and sim.is_done:
                if uid not in self._missiles_pending_cleanup:
                    ...
                    logging.debug(
                        "Missile %s completed at step %s, marked for delayed cleanup",
                        uid,
                        self.current_step,
                    )
        ...
        for uid, completion_step in list(self._missiles_pending_cleanup.items()):
            if self.current_step >= completion_step + 2:  # 延迟2帧
                if uid in self._tempsims:  # 确保导弹还在
                    ...
                    logging.debug(
                        "Cleaning up missile %s (completed at step %s, current %s)",
                        uid,
                        completion_step,
                        self.current_step,
                    )
        ...
        for uid in missiles_to_remove:
            ...
            if removed_sim:
                logging.info(f"[导弹清理] Removed completed missile: {uid} (delayed cleanup after explosion)")
                # 从发射平台的导弹列表中移除
        ...
        # 添加调试日志（避免f-string导致即使debug关闭也发生昂贵格式化）
        if logging.getLogger().isEnabledFor(logging.DEBUG):
            logging.debug("After task.step, agents: %s", list(self._jsbsims.keys()))
            for agent_id, agent in self._jsbsims.items():
                logging.debug(
                    "Agent %s type: %s, is_alive: %s",
                    agent_id,
                    type(agent),
                    getattr(agent, "is_alive", None),
                )
