...
from ..core.simulatior import BaseSimulator, AircraftSimulator, MissileSimulator, is_leader_agent_id
...
class MultipleCombatTask(SingleCombatTask):
    ...
    def num_agents(self) -> int:
        """返回智能体数量，支持2v2（4个）和4v4（8个）"""
        # 如果环境已经加载，从环境中获取实际智能体数量
        if hasattr(self, 'env') and self.env is not None:
            if hasattr(self.env, '_jsbsims'):
                return len(self.env._jsbsims)
        # 默认返回4（2v2），但实际会在环境加载后动态更新
        return 4
    ...
    def load_observation_space(self):
        ...
        self.obs_length = 9 + (actual_num_agents - 1) * 6
        self.observation_space = spaces.Box(low=-10, high=10., shape=(self.obs_length,))
        self.share_observation_space = spaces.Box(low=-10, high=10., shape=(actual_num_agents * self.obs_length,))
        logging.info(f"Observation space loaded: obs_length={self.obs_length}, num_agents={actual_num_agents}")
    ...
    def get_obs(self, env, agent_id):
        ...
        try:
            ...
            # 如果仍存在非有限值，说明仿真状态已损坏：直接判坠毁，避免后续逻辑/渲染继续传播NaN
            if not np.all(np.isfinite(ego_state)):
                logging.error(f"Non-finite ego_state persists for {agent_id}, forcing crash: {ego_state}")
                try:
            ...
            # 物理限制检查
            # 验证模式：允许少量负高度（数值误差/地形基准差异），避免过早终止战术验证场景
            try:
                verification_mode = os.getenv('CAP_VERIFICATION', '') == '1'
            except Exception:
                verification_mode = False

            ground_tol_m = -100.0 if verification_mode else 0.0
            if ego_state[2] < ground_tol_m:
                logging.error(f"Agent {agent_id} below ground: {ego_state[2]}m")
                env.agents[agent_id].crash()
                return np.zeros(self.obs_length)

            # 处于容差范围内：将高度钳制到0，避免后续归一化出现负值
            if verification_mode and ego_state[2] < 0.0:
                ego_state[2] = 0.0
            ...
            # 其他智能体相对状态
            offset = 9
            all_other_agents = env.agents[agent_id].partners + env.agents[agent_id].enemies
            
            # 🔥 修复：确保不超过观测空间大小（支持4v4）
            max_other_agents = (self.obs_length - 9) // 6  # 最多能容纳的其他智能体数量
            agents_to_process = all_other_agents[:max_other_agents]  # 只处理前N个
            
            for sim in agents_to_process:
                # 检查offset是否超出范围
                if offset + 6 > self.obs_length:
                    logging.warning(f"⚠️ [观测空间] {agent_id} offset超出范围: offset={offset}, obs_length={self.obs_length}")
                    break
    ...
    def reset(self, env):
        ...
        for agent_id in env.agents:
            if agent_id.startswith('A'):  # 友方智能体
                ...
                # 🔥 确保初始速度生效
                try:
                    env.agents[agent_id].set_property_value('ic/vt-kts', 583.0)  # 300m/s ≈ 583kts
                except Exception:
                    pass
                ...
                # 🔥 确保初始速度生效
                try:
                    env.agents[agent_id].set_property_value('ic/vt-kts', 544.0)  # 280m/s ≈ 544kts
                except Exception:
                    pass
...
class HierarchicalMultipleCombatTask(MultipleCombatTask):
    ...
    def reset(self, env):
        ...
        # 设置长机僚机关系
        for agent_id in env.agents.keys():
            is_leader = is_leader_agent_id(agent_id)
