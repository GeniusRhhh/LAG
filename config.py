...
#     group.add_argument("--algorithm-name", type=str, default='ppo', choices=["ppo", "mappo","sac"],
...
#     group.add_argument("--selfplay-algorithm", type=str, default='sp', choices=["sp", "fsp", "pfsp","psro","pbsp"],
...
def _get_prepare_config(parser: argparse.ArgumentParser):
    ...
    Prepare parameters:
        --env-name <str>
            specify the name of environment
        --algorithm-name <str>
            specify the algorithm, including `["ppo", "mappo", "sac"]`
        --experiment-name <str>
            an identifier to distinguish different experiment.
        --seed <int>
            set seed for numpy and torch
        --cuda
            by default False, will use CPU to train; or else will use GPU
        --n-training-threads <int>
            number of training threads working in parallel (default: 1)
        --n-rollout-threads <int>
            number of parallel envs for training rollout (default: 4)
        --num-env-steps <float>
            number of env steps to train (default: 1e7)
        --model-dir <str>
            by default None, set the path to pretrained model
        --use-wandb
            by default False, if set, will log data to wandb server
        --user-name <str>
            [for wandb usage], to specify user's name for collecting training data
        --wandb-name <str>
            [for wandb usage], to specify wandb project name
    """
    group = parser.add_argument_group("Prepare parameters")
    group.add_argument("--env-name", type=str, default='JSBSim',
                       help="specify the name of environment")
    group.add_argument("--algorithm-name", type=str, default='ppo', choices=["ppo", "mappo", "sac"],
                       help="Specify the algorithm (default: ppo)")
    group.add_argument("--experiment-name", type=str, default="check",
                       help="An identifier to distinguish different experiment")
    group.add_argument("--seed", type=int, default=1,
                       help="Random seed for numpy/torch")
    ...
    group.add_argument("--model-dir", type=str, default=None,
                       help="By default None, set the path to pretrained model")
    group.add_argument("--use-wandb", action='store_true', default=False,
                       help="By default False, if set, will log data to wandb server")
    group.add_argument("--user-name", type=str, default='liuqh',
                       help="[for wandb usage], to specify user's name for collecting training data")
    group.add_argument("--wandb-name", type=str, default='liuqh',
                       help="[for wandb usage], to specify wandb project name")
    return parser
...
def _get_replaybuffer_config(parser: argparse.ArgumentParser):
    ...
    Replay Buffer parameters:
        --gamma <float>
            discount factor for rewards (default: 0.99)
        --buffer-size <int>
            the maximum storage in the buffer (default: 1000000 for SAC, 200 for PPO)
        --batch-size <int>
            batch size for training (default: 256 for SAC, ignored for PPO)
        --use-proper-time-limits
            by default False, compute returns considering time limits if set
        --use-gae
            by default True, use generalized advantage estimation (only for PPO)
        --gae-lambda <float>
            gae lambda parameter (default: 0.95, only for PPO)
    """
    group = parser.add_argument_group("Replay Buffer parameters")
    group.add_argument("--gamma", type=float, default=0.99,
                       help="discount factor for rewards (default: 0.99)")
    group.add_argument("--buffer-size", type=int, default=100000,
                       help="maximum storage in the buffer (default: 1000000 for SAC, use smaller for PPO)")
    group.add_argument("--batch-size", type=int, default=256,
                       help="batch size for training (default: 256 for SAC, ignored for PPO)")
    group.add_argument("--use-proper-time-limits", action='store_true', default=False,
                       help="compute returns taking into account time limits")
    group.add_argument("--use-gae", action='store_false', default=True,
                       help="Whether to use generalized advantage estimation (only for PPO)")
    group.add_argument("--gae-lambda", type=float, default=0.95,
                       help="gae lambda parameter (default: 0.95, only for PPO)")
    group.add_argument("--reward-norm", action='store_true', default=True,
                       help="Whether to normalize rewards (default: False, used by SAC)")
    return parser
...
def _get_network_config(parser: argparse.ArgumentParser):
    ...
    Network parameters:
        ...
        --activation-id
            choose 0 to use Tanh, 1 to use ReLU, 2 to use LeakyReLU, 3 to use ELU
        --use-feature-normalization
            by default False, apply LayerNorm to normalize feature extraction inputs if set
        --gain
            by default 0.01, gain of last action layer
    """
    group = parser.add_argument_group("Network parameters")
    group.add_argument("--hidden-size", type=str, default='128 128',
                       help="Dimension of hidden layers for mlp pre-process networks (default: '128 128')")
    ...
    group.add_argument("--use-feature-normalization", action='store_true', default=True,
                       help="Whether to apply LayerNorm to the feature extraction inputs")
    group.add_argument("--gain", type=float, default=0.01,
                       help="The gain of last action layer (default: 0.01)")
    group.add_argument("--use-prior", action='store_true', default=False,
                       help="Whether to use prior human info to update network, use only on missile shoot task")
    return parser
...
def _get_recurrent_config(parser: argparse.ArgumentParser):
    ...
    Recurrent parameters:
        --use-recurrent-policy
            by default True, use Recurrent Policy; if False, use non-recurrent policy
            (Note: SAC typically does not use recurrent policy)
        --recurrent-hidden-size <int>
            dimension of hidden layers for recurrent layers (default: 128)
        --recurrent-hidden-layers <int>
            number of recurrent layers (default: 1)
        --data-chunk-length <int>
            time length of chunks used to train a recurrent policy (default: 10)
    """
    group = parser.add_argument_group("Recurrent parameters")
    group.add_argument("--use-recurrent-policy", action='store_false', default=True,
                       help="Whether to use a recurrent policy (default: True, set False for SAC if not needed)")
    group.add_argument("--recurrent-hidden-size", type=int, default=128,
                       help="Dimension of hidden layers for recurrent layers (default: 128)")
    group.add_argument("--recurrent-hidden-layers", type=int, default=1,
                       help="The number of recurrent layers (default: 1)")
    group.add_argument("--data-chunk-length", type=int, default=10,
                       help="Time length of chunks used to train a recurrent policy (default: 10)")
    return parser
...
def _get_optimizer_config(parser: argparse.ArgumentParser):
    ...
    Optimizer parameters:
        --lr <float>
            learning rate for PPO or fallback for SAC (default: 5e-4)
        --actor-lr <float>
            learning rate for actor network in SAC (default: 3e-4)
        --critic-lr <float>
            learning rate for critic networks in SAC (default: 3e-4)
        --alpha-lr <float>
            learning rate for alpha (entropy coefficient) in SAC (default: 3e-4)
    """
    group = parser.add_argument_group("Optimizer parameters")
    group.add_argument("--lr", type=float, default=5e-4,
                       help="learning rate for PPO or fallback for SAC (default: 5e-4)")
    group.add_argument("--actor-lr", type=float, default=1e-4,  # 修改为 3e-4
                       help="learning rate for actor network in SAC (default: 3e-4)")
    group.add_argument("--critic-lr", type=float, default=1e-4,  # 修改为 3e-4
                       help="learning rate for critic networks in SAC (default: 3e-4)")
    group.add_argument("--alpha-lr", type=float, default=1e-5,  # 修改为 3e-4
                       help="learning rate for alpha in SAC (default: 3e-4)")
    return parser
...
def _get_ppo_config(parser: argparse.ArgumentParser):
    ...
    PPO parameters:
        ...
        --clip-param <float>
            ppo clip parameter (default: 0.2)
        --use-clipped-value-loss
            by default False, clip value loss if set
        --num-mini-batch <int>
            number of batches for ppo (default: 1)
        --value-loss-coef <float>
            ppo value loss coefficient (default: 1)
        --entropy-coef <float>
            ppo entropy term coefficient (default: 0.01)
        --use-max-grad-norm
            by default True, use max norm of gradients
        --max-grad-norm <float>
            max norm of gradients (default: 2)
    """
    group = parser.add_argument_group("PPO parameters")
    group.add_argument("--ppo-epoch", type=int, default=10,
                       help="number of ppo epochs (default: 10)")
    group.add_argument("--clip-param", type=float, default=0.2,
                       help="ppo clip parameter (default: 0.2)")
    group.add_argument("--use-clipped-value-loss", action='store_true', default=False,
                       help="By default False, clip value loss if set")
    group.add_argument("--num-mini-batch", type=int, default=1,
                       help="number of batches for ppo (default: 1)")
    group.add_argument("--value-loss-coef", type=float, default=1,
                       help="ppo value loss coefficient (default: 1)")
    group.add_argument("--entropy-coef", type=float, default=0.01,
                       help="entropy term coefficient (default: 0.01)")
    group.add_argument("--use-max-grad-norm", action='store_false', default=True,
                       help="By default True, use max norm of gradients")
    group.add_argument("--max-grad-norm", type=float, default=2,
                       help="max norm of gradients (default: 2)")
    return parser
...
def _get_sac_config(parser: argparse.ArgumentParser):
    ...
    SAC parameters:
        ...
        --init-alpha <float>
            initial entropy coefficient for SAC (default: 1.0)
        --warmup-steps <int>
            number of warmup steps before training starts (default: 10000)
        --target-entropy <float>
            target entropy for SAC (default: -4.0)
    """
    group = parser.add_argument_group("SAC parameters")
    group.add_argument("--tau", type=float, default=0.005,
                       help="soft update coefficient for target networks (default: 0.005)")
    group.add_argument("--init-alpha", type=float, default=1.0,  # 修改为 1.0
                       help="initial entropy coefficient for SAC (default: 1.0)")
    group.add_argument("--warmup-steps", type=int, default=1000,
                       help="number of warmup steps before training starts (default: 10000)")
    group.add_argument("--target-entropy", type=float, default=-2.0,  # 新增
                       help="target entropy for SAC (default: -4.0)")
    return parser
...
def _get_selfplay_config(parser: argparse.ArgumentParser):
    ...
    Selfplay parameters:
        --use-selfplay
            by default False, use selfplay algorithms if set
        --selfplay-algorithm <str>
            specify the selfplay algorithm, including `["sp", "fsp", "pfsp", "psro", "pbsp"]`
        --n-choose-opponents <int>
            number of different opponents chosen for rollout (default: 1)
        --init-elo <float>
            initial ELO for policy performance (default: 1000.0)
    ...
    group = parser.add_argument_group("Selfplay parameters")
    group.add_argument("--use-selfplay", action='store_true', default=False,
                       help="By default False, use selfplay algorithms if set")
    group.add_argument("--selfplay-algorithm", type=str, default='sp', choices=["sp", "fsp", "pfsp", "psro", "pbsp"],
                       help="Specify the selfplay algorithm (default: 'sp')")
    group.add_argument("--n-choose-opponents", type=int, default=1,
                       help="number of different opponents chosen for rollout (default: 1)")
    group.add_argument("--init-elo", type=float, default=1000.0,
                       help="initial ELO for policy performance (default: 1000.0)")
...
def _get_save_config(parser: argparse.ArgumentParser):
    ...
    group = parser.add_argument_group("Save parameters")
    group.add_argument("--save-interval", type=int, default=1,
                       help="time duration between continuous twice models saving (default: 1)")
    return parser
...
def _get_eval_config(parser: argparse.ArgumentParser):
    ...
    Eval parameters:
        --use-eval
            by default False, start evaluation alongside training if set
        --n-eval-rollout-threads <int>
            number of parallel envs for evaluating rollout (default: 1)
        --eval-interval <int>
    ...
    group = parser.add_argument_group("Eval parameters")
    group.add_argument("--use-eval", action='store_true', default=False,
                       help="By default False, start evaluation alongside training if set")
    group.add_argument("--n-eval-rollout-threads", type=int, default=1,
                       help="Number of parallel envs for evaluating rollout (default: 1)")
    group.add_argument("--eval-interval", type=int, default=25,
                       help="time duration between continuous twice evaluation progress (default: 25)")
    group.add_argument("--eval-episodes", type=int, default=32,
                       help="number of episodes of a single evaluation (default: 32)")
    return parser
...
def _get_render_config(parser: argparse.ArgumentParser):
    """
    Render parameters:
        --render-opponent-index <str>
            the index of opponent policy in the opponent pool (default: 'latest')
        --render-index <str>
            the index of ego policy (default: 'latest')
    """
    group = parser.add_argument_group("Render parameters")
    group.add_argument("--render-opponent-index", type=str, default='latest',
                       help="the index of opponent policy in the opponent pool (default: 'latest')")
    group.add_argument("--render-index", type=str, default='latest',
                       help="the index of ego policy (default: 'latest')")
    return parser
